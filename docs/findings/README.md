...
| [0b-is-admin-was-inverted.md](0b-is-admin-was-inverted.md) | **`is_admin` returned true for a download-only token.** It treated the *absence* of a path scope as a privilege, so seven of eight fixtures were administrators and a scoped token with upload rights was not. Never called by anything; now deleted |
...
| [0b-a-test-that-asked-the-internet.md](0b-a-test-that-asked-the-internet.md) | **A merge-gate test resolved `invalid.com`, a registered domain**, getting SERVFAIL in 0.2-10s (once 680.6s, failing) while asserting the `Host not found` that only NXDOMAIN produces. `host.invalid` is reserved by RFC 6761: 0.06-0.08s, five for five. The comment about the flakiness sat on the wrong suite and claimed an exclusion that never existed |
| [0b-where-the-ci-minutes-go.md](0b-where-the-ci-minutes-go.md) | **SyncTests passes on 4 and 8 threads, 3x faster, and stays at 1.** The step is 2.9 min of a 21-min job whose build is 13.6; two minutes is not worth a concurrency change to the merge gate on one platform's evidence. No compiler cache exists anywhere -- recorded as the next lever, with its tradeoff, rather than taken quietly. Also: this suite's check count varies 3.2x, traced to one test that checks once per socket read completion while moving 64 MiB -- so the count is the kernel's to decide, and pinning the random seed does not fix it |
| [0b-the-rename-scanned-only-source.md](0b-the-rename-scanned-only-source.md) | **The rename check scanned C++ and CMake; binaries are named in scripts.** Eighteen survivors across eight files -- a tool that could not run, a `.gitignore` that stopped ignoring the test binaries, a Windows manifest pointing at nothing. The new scan then passed three canaries because `grep` lacked `-E` |
| [0b-a-check-a-contributor-cannot-fail.md](0b-a-check-a-contributor-cannot-fail.md) | **CONTRIBUTING.md documented the copyright check feeding itself its own output**, so it passed whatever it found -- delete ten notices and it reports `PASS: 550 intact`. CI always passed the literal 560. The wrong invocation was the one aimed at outside contributors, on the same page that says removing a notice gets a patch rejected |
| [0b-a-failure-that-was-not-counted.md](0b-a-failure-that-was-not-counted.md) | **The suite printed "All 1660 tests passed" and exited 0 with a failing test in it**, on two runs out of four at two threads. One thread's concurrent-phase counters -- checks *and* failures -- were discarded before being folded into the totals the exit status reads. CoreTests runs at two threads in the merge gate |
| [0b-a-hang-should-look-like-a-failure.md](0b-a-hang-should-look-like-a-failure.md) | **A timed-out job is reported as `cancelled`** -- the same word as a concurrency cancellation. A hanging `ObjectStoreTests` hid behind that for two hours. A *step*-level timeout reports `failure` instead; measured, then applied |
...
| [0b-a-load-test-that-never-built.md](0b-a-load-test-that-never-built.md) | **The load test could never have compiled.** It included a `load_tester.hpp` that exists nowhere in the repository or its history, and `CMakeLists.txt` never named it -- so the file that would have failed was never built. The fourth thing found in that state |
| [0b-copying-a-live-database.md](0b-copying-a-live-database.md) | **`cp -R` of a live data directory yields an openable snapshot.** Five copies taken a second apart under continuous write load all opened, none corrupt, each holding a prefix of the writes. Measured, because the engine's design only made it plausible |
| [0b-a-signature-nothing-could-produce.md](0b-a-signature-nothing-could-produce.md) | **`can_sign()` returned true for a key that could not sign.** The private-key half of `PKey` -- `load_private` and `sign` -- was declared and implemented by no shipping backend, so a caller who checked before signing got a link error. v0.3.0 could verify tokens and never issue one |
| [0b-a-number-that-named-nothing.md](0b-a-number-that-named-nothing.md) | **A check reported a subtraction instead of a name.** "expected 231, found 230" is true and useless: the check held both lists when it counted them, so finding which header took five CI runs and four wrong hypotheses. Replaced with a manifest -- whose canary then caught `comm` misreporting, because the lists were sorted in one locale and compared in another |
| [0b-the-apple-surface-came-from-one-header.md](0b-the-apple-surface-came-from-one-header.md) | **The macOS package's extra headers were not a platform difference.** One internal header, `sync/impl/sync_client.hpp`, dragged two Apple-only headers into the public surface; nothing installed included it. Removing it and its closure makes the surface identical on both platforms -- and fixes a package that could never be built for Emscripten |
| [0b-a-header-that-could-not-be-used.md](0b-a-header-that-could-not-be-used.md) | **A shipped public header with no include guard, no implementation, and no includers.** `audit_serializer.hpp` could not be included twice or linked against, and was the only reason a vendored nlohmann/json was published at `include/external/json/` -- into every consumer's include namespace. The check meant to catch that read one CMakeLists, not the tree |
| [0b-eighty-three-headers-nobody-was-told-about.md](0b-eighty-three-headers-nobody-was-told-about.md) | **Of 243 installed headers, a consumer could reach 160.** The other 83 were public API no public header mentioned. The last 22 were only findable once the entry points were written down at all -- nothing said what a consumer of `Merge`, `Sync` or `QueryParser` should include, so "is this reachable?" had no answer. 201 install now, all reachable |
