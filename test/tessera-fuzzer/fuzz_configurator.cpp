...
void FuzzConfigurator::setup_realm_config()
{
    ...
    m_config.scheduler = tessera::util::Scheduler::make_dummy();
    if (m_use_encryption) {
...
const tessera::Realm::Config& FuzzConfigurator::get_config() const
{
    return m_config;
}
...
void FuzzConfigurator::init(const std::string& input)
{
    ...
    tessera::test_util::RealmPathInfo test_context{db_name};
...
void FuzzConfigurator::print_cnf()
{
    ...
    m_log << "// Test case generated in " TESSERA_VER_CHUNK " on " << m_fuzzer.get_current_time_stamp() << ".\n";
    m_log << "// TESSERA_MAX_BPNODE_SIZE is " << TESSERA_MAX_BPNODE_SIZE << "\n";
    m_log << "// ----------------------------------------------------------------------\n";
