// This file is new work rather than inherited from realm-core, so it carries no
// Realm Inc. copyright notice -- adding one would have raised the count that
// tools/check-copyright-notices.sh pins, which exists to preserve the notices
// Apache 2.0 section 4(b) requires be retained, not to collect new ones.
