...
// Windows doesn't use fifos
    ...
    SECTION("should be able to set a FIFO fallback path") {
        ...
        tessera::util::try_make_dir(fallback_dir);
        TestFile config;
        ...
        REQUIRE(tessera::util::try_remove_dir_recursive(fallback_dir));
    }
...
#if !TESSERA_USE_UV && !TEST_SCHEDULER_UV // uv scheduler does not support background threads
    ...
    for (int i = 0; i < 2; ++i) {
        SECTION(close_function_names[i]) {
            ...
            SECTION("before write lock is acquired") {
                ...
                tessera::test_util::BowlOfStonesSemaphore sema;
...
TEST_CASE("KeyPathMapping generation") {
    TestFile config;
    tessera::query_parser::KeyPathMapping mapping;
    ...
    SECTION("class aliasing") {
        ...
        config.schema = schema;
        auto realm = Realm::get_shared_realm(config);
        tessera::populate_keypath_mapping(mapping, *realm);
