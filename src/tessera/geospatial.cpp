...
static bool type_is_valid(tessera::StringData str_type)
{
...
std::optional<GeoPoint> Geospatial::point_from_obj(const Obj& obj, ColKey type_col, ColKey coords_col)
{
    if (!type_col) {
        ...
        TESSERA_ASSERT_EX(type_col.get_type() == ColumnType(ColumnType::Type::String), type_col.get_type());
        TESSERA_ASSERT(!type_col.is_collection());
    }
    if (!coords_col) {
        coords_col = obj.get_table()->get_column_key(StringData(c_geo_point_coords_col_name));
    }
    else {
        TESSERA_ASSERT_EX(coords_col.get_type() == ColumnType(ColumnType::Type::Double), coords_col.get_type());
        TESSERA_ASSERT(coords_col.is_list());
    }
...
static Status coord_to_point(double lng, double lat, S2Point& out)
{
    ...
    TESSERA_ASSERT_EX(ll.is_valid(), util::format("coords invalid after normalization, lng = %1, lat = %2", lng, lat));
    out = ll.ToPoint();
    return Status::OK();
}
