...
#ifndef TESSERA_GROUP_HPP
#define TESSERA_GROUP_HPP
...
#include <tessera/impl/output_stream.hpp>
...
public:
    ...
    /// specified name, or `tessera::not_found` if this group does not contain a
    ...
    {
        TESSERA_ASSERT(class_name.size() <= max_class_name_length);
...
{
    TESSERA_ASSERT(table_type != Table::Type::Embedded);
    check_attached();
...
{
    TESSERA_ASSERT_DEBUG(m_notify_handler);
...
inline ref_type Group::get_history_ref(const Array& top) noexcept
{
    ...
    if (has_history) {
        ...
        TESSERA_ASSERT(top.size() > s_hist_version_ndx);
...
public:
    ...
    {
        TESSERA_ASSERT_DEBUG(notification_handler());
