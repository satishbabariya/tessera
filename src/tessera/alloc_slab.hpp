...
#ifndef TESSERA_ALLOC_SLAB_HPP
#define TESSERA_ALLOC_SLAB_HPP
...
{
    TESSERA_ASSERT_3(m_attach_mode, ==, attach_UsersBuffer);
    TESSERA_ASSERT(m_data);
...
void SlabAlloc::for_all_free_entries(Func f) const
{
    ...
    for (const auto& e : m_slabs) {
        ...
        TESSERA_ASSERT(bb->block_before_size == 0);
        while (1) {
