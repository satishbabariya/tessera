...
#ifndef TESSERA_NULL_HPP
#define TESSERA_NULL_HPP
...
struct null {
    ...
    static T get_null_float()
    {
        ...
        TESSERA_ASSERT_DEBUG(std::isnan(d));
        TESSERA_ASSERT_DEBUG(!is_signaling(d));
        return d;
    }
    ...
    {
        TESSERA_ASSERT(std::isnan(static_cast<double>(v)));
        typename std::conditional<std::is_same<T, float>::value, uint32_t, uint64_t>::type i;
