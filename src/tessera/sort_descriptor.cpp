...
{
    TESSERA_ASSERT_EX(m_ascending.empty() || m_ascending.size() == m_column_keys.size(), m_ascending.size(),
                    m_column_keys.size());
...
{
    TESSERA_ASSERT(!column_lists.empty());
    TESSERA_ASSERT_EX(column_lists.size() == ascending.size(), column_lists.size(), ascending.size());
    ...
    for (size_t i = 0; i < column_lists.size(); ++i) {
        ...
        auto sz = columns.size();
        TESSERA_ASSERT_EX(!columns.empty(), i);
...
{
    TESSERA_ASSERT(!m_column_keys.empty());
...
{
    TESSERA_ASSERT(index < m_descriptors.size());
...
tessera::util::Optional<size_t> DescriptorOrdering::get_min_limit() const
{
    tessera::util::Optional<size_t> min_limit;
...
{
    return std::any_of(m_descriptors.begin(), m_descriptors.end(), [](const std::unique_ptr<BaseDescriptor>& desc) {
        TESSERA_ASSERT(desc.get()->is_valid());
        return (desc->get_type() == DescriptorType::Limit &&
