...
#ifndef TESSERA_GLOBAL_KEY_HPP
#define TESSERA_GLOBAL_KEY_HPP
...
struct GlobalKey {
    ...
    constexpr GlobalKey(tessera::util::None = tessera::util::none)
        : m_lo(-1)
        , m_hi(-1)
    {
    }
    ...
    {
        TESSERA_ASSERT(m_hi <= 0x3fffffff);
...
template <>
struct hash<tessera::GlobalKey> {
    size_t operator()(tessera::GlobalKey oid) const
    {
