...
#ifndef TESSERA_TABLE_HPP
#define TESSERA_TABLE_HPP
...
#define TESSERA_EXERCISE_OBJECT_ID_COLLISION 0
...
public:
    ...
    /// <tessera/object-store/object_schema.hpp>.
    ...
    {
        if (TESSERA_UNLIKELY(!valid_column(col_key)))
    ...
    /// index of this table within the group. Otherwise it returns tessera::npos.
...
public:
    ...
    ColKey operator*()
    {
        if (m_pos < m_table->get_column_count()) {
            TESSERA_ASSERT(m_table->m_spec.get_key(m_pos) == m_table->spec_ndx2colkey(m_pos));
...
inline StringData Table::get_column_name(ColKey column_key) const
{
    ...
    TESSERA_ASSERT_3(spec_ndx, <, get_column_count());
