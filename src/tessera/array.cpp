...
#include <tessera/impl/array_writer.hpp>
...
//
// `N_c` must always be a power of `TESSERA_MAX_BPNODE_SIZE`.
//
...
ref_type Array::do_write_shallow(_impl::ArrayWriterBase& out) const
{
    ...
    TESSERA_ASSERT_3(new_ref % 8, ==, 0);                                    // 8-byte alignment
    return new_ref;
}
...
{
    TESSERA_ASSERT_3(begin, <=, end);
    TESSERA_ASSERT_3(end, <=, m_size);
    TESSERA_ASSERT_3(dest_begin, <=, m_size);
    TESSERA_ASSERT_3(end - begin, <=, m_size - dest_begin);
    TESSERA_ASSERT(!(dest_begin >= begin && dest_begin < end)); // Required by std::copy
    ...
    tessera::safe_copy_n(begin_2, end_2 - begin_2, dest_begin_2);
}
...
{
    TESSERA_ASSERT(value >= 0);
    TESSERA_ASSERT(&m_alloc == &Allocator::get_default());
...
{
    TESSERA_ASSERT_DEBUG(ndx <= m_size);
    ...
    if (do_expand) {
        ...
        TESSERA_ASSERT_DEBUG(width > m_width);
...
{
    TESSERA_ASSERT(is_attached());
    TESSERA_ASSERT_3(new_size, <=, m_size);
...
void Array::do_ensure_minimum_width(int_fast64_t value)
{
    ...
    TESSERA_ASSERT_3(width, >, m_width);
...
{
    TESSERA_TEMPEX(return sum, m_width, (start, end));
}
...
int64_t Array::sum(size_t start, size_t end) const
{
    ...
    TESSERA_ASSERT_EX(end <= m_size && start <= end, start, end, m_size);
    ...
    if (w == 1 || w == 2 || w == 4) {
        ...
        for (size_t t = 0; t < chunks; t++) {
            if (w == 1) {
#if 0
#if defined(USE_SSE42) && defined(_MSC_VER) && defined(TESSERA_PTR_64)
                s += __popcnt64(data[t]);
#elif !defined(_MSC_VER) && defined(USE_SSE42) && defined(TESSERA_PTR_64)
...
{
    TESSERA_ASSERT(width != 0 && (width & (width - 1)) == 0); // Is a power of two
...
MemRef Array::clone(MemRef mem, Allocator& alloc, Allocator& target_alloc)
{
    ...
    if (!get_hasrefs_from_header(header)) {
        ...
        tessera::safe_copy_n(src_begin, src_end - src_begin, dst_begin);
    ...
    TESSERA_ASSERT_3(get_wtype_from_header(header), ==, wtype_Bits);
...
MemRef Array::create(Type type, bool context_flag, WidthType width_type, size_t size, int_fast64_t value,
                     Allocator& alloc)
{
    TESSERA_ASSERT_7(value, ==, 0, ||, width_type, ==, wtype_Bits);
    TESSERA_ASSERT_7(size, ==, 0, ||, width_type, !=, wtype_Ignore);
    ...
    if (value != 0) {
        ...
        TESSERA_TEMPEX(fill_direct, width, (data, begin, end, value));
    }
...
void Array::update_width_cache_from_header() noexcept
{
    ...
    TESSERA_TEMPEX(m_vtable = &VTableForWidth, width, ::vtable);
...
void Array::verify() const
{
    ...
    TESSERA_ASSERT(m_width == 0 || m_width == 1 || m_width == 2 || m_width == 4 || m_width == 8 || m_width == 16 ||
    ...
    TESSERA_ASSERT_3(ref_in_parent, ==, m_ref);
#endif
}

size_t Array::lower_bound_int(int64_t value) const noexcept
{
    TESSERA_TEMPEX(return lower_bound, m_width, (m_data, m_size, value));
}

size_t Array::upper_bound_int(int64_t value) const noexcept
{
    TESSERA_TEMPEX(return upper_bound, m_width, (m_data, m_size, value));
}
