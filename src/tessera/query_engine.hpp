...
#ifndef TESSERA_QUERY_ENGINE_HPP
#define TESSERA_QUERY_ENGINE_HPP
...
#if TESSERA_X86_OR_X64_TRUE && defined(_MSC_FULL_VER) && _MSC_FULL_VER >= 160040219
#include <immintrin.h>
#endif
...
public:
    ...
    bool do_consume_condition(ParentNode& node) override
    {
        ...
        TESSERA_ASSERT(this->m_condition_column_key == other.m_condition_column_key);
        if (m_needles.empty()) {
    ...
    size_t find_first_local(size_t start, size_t end) override
    {
        TESSERA_ASSERT(this->m_table);
        size_t s = tessera::npos;
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        TESSERA_ASSERT(this->m_condition_column_key);
...
public:
    ...
    size_t find_first_local(size_t start, size_t end) override
    {
        ...
        auto find = [&](bool nullability) {
            ...
            for (size_t s = start; s < end; ++s) {
                ...
                TESSERA_ASSERT(!(null::is_null_float(v) && !nullability));
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        TESSERA_ASSERT(m_condition_column_key);
...
public:
    ...
    void reset_cache()
    {
        ...
        TESSERA_ASSERT_DEBUG(m_condition_column_key.is_list());
    }
...
public:
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        ...
        return state.describe_column(ParentNode::m_table, this->m_condition_column_key) + " " +
               ...
               (this->m_value_is_null ? util::serializer::print_value(tessera::null())
...
public:
    ...
    {
        TESSERA_ASSERT(column.get_type() == col_type_Mixed);
        get_ownership();
        m_condition_column_key = column;
    }
...
public:
    ...
    size_t find_first_local(size_t start, size_t end) override
    {
        ...
        for (size_t i = start; i < end; i++) {
            ...
            if constexpr (tessera::is_any_v<TConditionFunction, BeginsWith, BeginsWithIns, EndsWith, EndsWithIns, Like,
...
public:
    ...
    std::string describe(util::serializer::SerialisationState& state) const override
    {
        TESSERA_ASSERT(m_condition_column_key1 && m_condition_column_key2);
...
public:
        ...
        : m_target_keys(target_keys)
    {
        ...
        TESSERA_ASSERT(column_type == col_type_Link);
        TESSERA_ASSERT(!m_target_keys.empty());
    }
...
#endif // TESSERA_QUERY_ENGINE_HPP
