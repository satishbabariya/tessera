...
Mixed TableView::aggregate(ColKey column_key, size_t* result_count, ObjKey* return_key) const
{
    ...
    TESSERA_ASSERT(m_table->valid_column(column_key));
...
{
    TESSERA_ASSERT(m_query);
    TESSERA_ASSERT(m_query->m_table);
    TESSERA_ASSERT(m_query->m_table == q.m_table);
