...
#ifndef TESSERA_OBJ_HPP
#define TESSERA_OBJ_HPP
...
inline Obj& Obj::set(ColKey col_key, uint_fast64_t value, bool is_default)
{
    ...
    if (TESSERA_UNLIKELY(util::int_cast_with_overflow_detect(value, value_2))) {
        TESSERA_TERMINATE("Unsigned integer too big.");
    }
...
template <>
inline Obj& Obj::set(ColKey col_key, tessera::null, bool is_default)
{
...
inline Obj& Obj::set_all(Head v, Tail... tail)
{
    ...
    if (get_primary_key_column()) {
        TESSERA_ASSERT(colkey2spec_ndx(get_primary_key_column()) == 0);
        start_index = 1;
    }
