...
int Mixed::compare(const Mixed& b) const noexcept
{
    ...
    switch (type) {
        ...
        default:
            ...
            TESSERA_ASSERT_RELEASE(false && "Compare not supported for this column type");
            break;
    }

    // Comparing rank of types as a fallback makes it possible to sort of a list of Mixed
    TESSERA_ASSERT(sorting_rank[m_type] != sorting_rank[b.m_type]);
...
int64_t Mixed::export_to_type() const noexcept
{
    ...
    TESSERA_ASSERT(get_type() == type_Int);
    return int_val;
}
...
size_t Mixed::hash() const
{
    ...
    switch (get_type()) {
        ...
        case type_Mixed:
        case type_Link:
            TESSERA_ASSERT_RELEASE(false && "Hash not supported for this column type");
            break;
    }
...
StringData Mixed::get_index_data(std::array<char, 16>& buffer) const noexcept
{
    ...
    switch (type) {
        case type_Int: {
            ...
            tessera::safe_copy_n(c, sizeof(int64_t), buffer.data());
        ...
        case type_Float: {
            ...
            tessera::safe_copy_n(src, sizeof(float), buffer.data());
        ...
        case type_Double: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            tessera::safe_copy_n(src, sizeof(double), buffer.data());
        ...
        case type_Timestamp: {
            ...
            tessera::safe_copy_n(s_buf, sizeof(s), buffer.data());
            tessera::safe_copy_n(ns_buf, sizeof(ns), buffer.data() + sizeof(s));
        ...
        case type_Decimal: {
            ...
            const char* src = reinterpret_cast<const char*>(&v2);
            tessera::safe_copy_n(src, sizeof(v2), buffer.data());
        ...
        case type_TypedLink: {
            ...
            tessera::safe_copy_n(src, sizeof(k1), buffer.data());
            src = reinterpret_cast<const char*>(&k2);
            tessera::safe_copy_n(src, sizeof(k2), buffer.data() + sizeof(k1));
    ...
    TESSERA_ASSERT_RELEASE(false && "Index not supported for this column type");
    return {};
}
