...
{
    return size() ? m_values->find_first(value) : tessera::not_found;
}
...
inline std::shared_ptr<T> Dictionary::do_get_collection(const PathElement& path_elem)
{
    ...
    auto get_shared = [&]() -> std::shared_ptr<CollectionParent> {
        ...
        if (weak.expired()) {
            TESSERA_ASSERT_DEBUG(m_level == 1);
...
{
    return find_any_key(key) != tessera::npos;
}
...
{
    TESSERA_ASSERT(m_dictionary_top);
    TESSERA_ASSERT(ndx != tessera::npos);
...
{
    return update() ? m_values->find_first(value) : tessera::not_found;
}
...
{
    size_t ndx = update() ? m_values->find_first(value) : tessera::npos;
    return (ndx == tessera::npos) ? Mixed{} : do_get_key(ndx);
}
...
void Dictionary::verify() const
{
    m_keys->verify();
    m_values->verify();
    TESSERA_ASSERT(m_keys->size() == m_values->size());
}
...
{
    TESSERA_ASSERT_EX(col_key.get_type() == col_type_Link, col_key.get_type());
}

DictionaryLinkValues::DictionaryLinkValues(const Dictionary& source)
    : m_source(source)
{
    TESSERA_ASSERT_EX(source.get_value_data_type() == type_Link, source.get_value_data_type());
}
