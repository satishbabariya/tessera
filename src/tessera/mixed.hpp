...
#ifndef TESSERA_MIXED_HPP
#define TESSERA_MIXED_HPP
...
{
    TESSERA_ASSERT(get_type() == type_Bool);
    return bool_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_Float);
    return float_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_Double);
    return double_val;
}
...
inline StringData Mixed::get<StringData>() const noexcept
{
    if (is_null())
        return StringData();
    TESSERA_ASSERT(get_type() == type_String);
    return string_val;
}
...
inline BinaryData Mixed::get<BinaryData>() const noexcept
{
    if (is_null())
        return BinaryData();
    TESSERA_ASSERT(get_type() == type_Binary);
    return binary_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_Timestamp);
    return date_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_Decimal);
    return decimal_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_ObjectId);
    return id_val;
}
...
{
    TESSERA_ASSERT(get_type() == type_UUID);
    return uuid_val;
}
...
inline ObjKey Mixed::get<ObjKey>() const noexcept
{
    ...
    TESSERA_ASSERT(get_type() == type_Link);
...
{
    TESSERA_ASSERT(get_type() == type_TypedLink);
    return link_val;
}
...
{
    auto type = get_type();
    TESSERA_ASSERT_EX(type == type_Geospatial, type);
...
namespace std {
template <>
struct hash<::tessera::Mixed> {
    inline size_t operator()(const ::tessera::Mixed& m) const noexcept
    {
        return m.hash();
    }
};
