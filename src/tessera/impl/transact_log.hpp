...
#ifndef TESSERA_IMPL_TRANSACT_LOG_HPP
#define TESSERA_IMPL_TRANSACT_LOG_HPP

#include <tessera/binary_data.hpp>
#include <tessera/collection.hpp>
...
private:
    ...
    TESSERA_COLD TESSERA_NORETURN void parser_error() const;
...
inline void TransactLogBufferStream::transact_log_reserve(size_t size, char** inout_new_begin, char** out_new_end)
{
    ...
    TESSERA_ASSERT(*inout_new_begin >= data);
    TESSERA_ASSERT(*inout_new_begin <= (data + m_buffer.size()));
                                                         ...
                                                         char** out_new_end)
{
    ...
    *out_new_begin = tessera::safe_copy_n(data, size, *out_new_begin);
}
...
{
    TESSERA_ASSERT(free_begin <= free_end);
...
{
    TESSERA_ASSERT_DEBUG(m_transact_log_free_begin <= ptr);
    TESSERA_ASSERT_DEBUG(ptr <= m_transact_log_free_end);
...
char* TransactLogEncoder::encode_int(char* ptr, T value)
{
    ...
    if (negative) {
        ...
        TESSERA_DIAG_PUSH();
        TESSERA_DIAG_IGNORE_UNSIGNED_MINUS();
        value = -(value + 1);
        TESSERA_DIAG_POP();
    }
...
void TransactLogParser::parse_one(InstructionHandler& handler)
{
    ...
    switch (instr) {
        ...
        case instr_DictionaryInsert: {
            ...
            TESSERA_ASSERT(type == int(type_String));
...
#endif // TESSERA_IMPL_TRANSACT_LOG_HPP
