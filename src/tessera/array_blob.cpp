...
ref_type Array::blob_replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    TESSERA_ASSERT(get_context_flag());
    size_t sz = blob_size();
    TESSERA_ASSERT_3(begin, <=, end);
    TESSERA_ASSERT_3(end, <=, sz);
    TESSERA_ASSERT(data_size == 0 || data);

    TESSERA_ASSERT((begin == 0 || begin == sz) && end == sz); // Only append or total replace supported
    if (begin == sz && end == sz) {
        // Append
...
ref_type ArrayBlob::replace(size_t begin, size_t end, const char* data, size_t data_size, bool add_zero_term)
{
    ...
    // Insert the data
    modify_begin = tessera::safe_copy_n(data, data_size, modify_begin);
    if (add_zero_term)
        *modify_begin = 0;
