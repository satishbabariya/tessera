...
public:
    ...
    {
        TESSERA_ASSERT(get_node_size() == 1);
...
private:
    ...
    size_t get_elems_per_child() const
    {
        ...
        TESSERA_ASSERT_DEBUG(!m_offsets.is_attached());
...
ref_type BPlusTreeLeaf::bptree_insert(size_t ndx, State& state, InsertFunc func)
{
    ...
    TESSERA_ASSERT_DEBUG(leaf_size <= TESSERA_MAX_BPNODE_SIZE);
    if (ndx > leaf_size)
        ndx = leaf_size;
    if (TESSERA_LIKELY(leaf_size < TESSERA_MAX_BPNODE_SIZE)) {
        func(this, ndx);
...
void BPlusTree<int64_t>::split_root()
{
    if (m_root->is_leaf()) {
        ...
        new_root->create(TESSERA_MAX_BPNODE_SIZE);

        size_t ndx = 0;
        while (ndx < sz) {
            LeafNode new_leaf(this);
            new_leaf.create();
            size_t to_move = std::min(size_t(TESSERA_MAX_BPNODE_SIZE), sz - ndx);
...
void BPlusTreeInner::bptree_access(size_t n, AccessFunc func)
{
    ...
    if (m_offsets.is_attached()) {
        ...
        TESSERA_ASSERT_3(child_ndx, <, get_node_size());
    }
    else {
...
size_t BPlusTreeInner::bptree_erase(size_t n, EraseFunc func)
{
    ...
    if (erase_node_size == 0) {
    ...
    else if (erase_node_size < TESSERA_MAX_BPNODE_SIZE / 2 && child_ndx < (num_children - 1)) {
        ...
        if (combined_size < TESSERA_MAX_BPNODE_SIZE * 3 / 4) {
...
std::unique_ptr<BPlusTreeInner> BPlusTreeInner::split_root()
{
    ...
    size_t elems_per_child = get_elems_per_child();
    new_root->create(TESSERA_MAX_BPNODE_SIZE * elems_per_child);
    ...
    TESSERA_ASSERT(accumulated_size == tree_size);
...
void BPlusTreeInner::erase_and_destroy_bp_node(size_t ndx)
{
    ...
    TESSERA_ASSERT_DEBUG(m_offsets.size() == get_node_size() - 1);
}
...
void BPlusTreeInner::verify() const
{
    ...
    TESSERA_ASSERT_3(Array::get_type(), ==, Array::type_InnerBptreeNode);

    TESSERA_ASSERT_3(Array::size(), >=, 2);
    size_t num_children = get_node_size();

    // Verify invar:bptree-nonempty-inner
    TESSERA_ASSERT_3(num_children, >=, 1);

    size_t elems_per_child = 0;
    if (m_offsets.is_attached()) {
        TESSERA_ASSERT_DEBUG(m_offsets.size() == num_children - 1);
    }
    else {
    ...
    for (size_t i = 0; i < num_children; i++) {
        ...
        if (m_offsets.is_attached()) {
            if (i < num_children - 1) {
                TESSERA_ASSERT_DEBUG(num_elems == m_offsets.get(i));
            }
        }
        else {
            if (i < num_children - 1) {
                TESSERA_ASSERT_DEBUG(elems_in_child == elems_per_child);
            }
            else {
                TESSERA_ASSERT_DEBUG(elems_in_child <= elems_per_child);
            }
    ...
    TESSERA_ASSERT_DEBUG(get_tree_size() == num_elems);
    m_tree->invalidate_leaf_cache();
#endif
}
...
void BPlusTreeBase::bptree_insert(size_t n, BPlusTreeNode::InsertFunc func)
{
    ...
    if (TESSERA_UNLIKELY(new_sibling_ref)) {
