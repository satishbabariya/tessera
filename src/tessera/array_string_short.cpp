...
#include <tessera/array_string_short.hpp>
...
{
    TESSERA_ASSERT_3(ndx, <, m_size);
    TESSERA_ASSERT_3(value.size(), <, max_width); // otherwise we have to use another column type
    ...
    if (m_width <= value.size()) {
        ...
        if (old_width > 0) {
            ...
            while (new_end != base) {
                TESSERA_ASSERT_3(new_width, <=, max_width);
...
{
    TESSERA_ASSERT_3(ndx, <=, m_size);
    TESSERA_ASSERT(value.size() < max_width); // otherwise we have to use another column type
...
void ArrayStringShort::erase(size_t ndx)
{
    ...
    if (ndx < m_size - 1) {
        ...
        tessera::safe_copy_n(old_begin, old_end - old_begin, new_begin);
    }
...
size_t ArrayStringShort::find_first(StringData value, size_t begin, size_t end) const noexcept
{
    ...
    TESSERA_ASSERT(begin <= m_size && end <= m_size && begin <= end);
    ...
    if (m_nullable ? value.is_null() : value_size == 0) {
        ...
        for (size_t i = begin; i != end; ++i) {
            ...
            if (TESSERA_UNLIKELY(data_i_size == 0))
                return i;
        }
    }
    else {
