...
#ifndef TESSERA_KEYS_HPP
#define TESSERA_KEYS_HPP
...
enum class CollectionType {
    ...
    // values in <tessera/data_type.hpp>
    List = 19,
    Set = 20,
    Dictionary = 21
};
...
template <>
struct hash<tessera::ObjKey> {
    size_t operator()(tessera::ObjKey key) const
    {
        return std::hash<uint64_t>{}(key.value);
    }
};
template <>
struct hash<tessera::ColKey> {
    size_t operator()(tessera::ColKey key) const
    {
        return std::hash<int64_t>{}(key.value);
    }
};
template <>
struct hash<tessera::TableKey> {
    size_t operator()(tessera::TableKey key) const
    {
