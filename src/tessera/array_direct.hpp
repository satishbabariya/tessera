...
#ifndef TESSERA_ARRAY_DIRECT_HPP
#define TESSERA_ARRAY_DIRECT_HPP
...
#define TESSERA_TEMPEX(fun, wid, arg) \
    ...
    else {TESSERA_ASSERT_DEBUG(false); fun<0> arg;}

#define TESSERA_TEMPEX2(fun, targ, wid, arg) \
    ...
    else {TESSERA_ASSERT_DEBUG(false); fun<targ, 0> arg;}

#define TESSERA_TEMPEX3(fun, targ1, wid, targ3, arg) \
    ...
    else {TESSERA_ASSERT_DEBUG(false); fun<targ1, 0, targ3> arg;}

#define TESSERA_TEMPEX4(fun, targ1, targ3, targ4, wid, arg) \
    ...
    else {TESSERA_ASSERT_DEBUG(false); fun<targ1, targ3, targ4, 0> arg;}
// clang-format on
...
{
    if (width == 0) {
        TESSERA_ASSERT_DEBUG(value == 0);
        return;
    }
    else if (width == 1) {
        TESSERA_ASSERT_DEBUG(0 <= value && value <= 0x01);
        ...
        *p = uchar((*p & ~(0x01 << bit_ndx)) | (int(value) & 0x01) << bit_ndx);
        ...
        TESSERA_ASSERT_DEBUG(0 <= value && value <= 0x03);
        ...
        *p = uchar((*p & ~(0x03 << bit_ndx)) | (int(value) & 0x03) << bit_ndx);
        ...
        TESSERA_ASSERT_DEBUG(0 <= value && value <= 0x0F);
        ...
        *p = uchar((*p & ~(0x0F << bit_ndx)) | (int(value) & 0x0F) << bit_ndx);
        ...
        TESSERA_ASSERT_DEBUG(std::numeric_limits<int8_t>::min() <= value &&
                           value <= std::numeric_limits<int8_t>::max());
        *(reinterpret_cast<int8_t*>(data) + ndx) = int8_t(value);
    }
    else if (width == 16) {
        TESSERA_ASSERT_DEBUG(std::numeric_limits<int16_t>::min() <= value &&
                           value <= std::numeric_limits<int16_t>::max());
        *(reinterpret_cast<int16_t*>(data) + ndx) = int16_t(value);
    }
    else if (width == 32) {
        TESSERA_ASSERT_DEBUG(std::numeric_limits<int32_t>::min() <= value &&
                           value <= std::numeric_limits<int32_t>::max());
        *(reinterpret_cast<int32_t*>(data) + ndx) = int32_t(value);
    }
    else if (width == 64) {
        TESSERA_ASSERT_DEBUG(std::numeric_limits<int64_t>::min() <= value &&
...
{
    TESSERA_TEMPEX(set_direct, width, (data, ndx, value));
}
...
{
    TESSERA_TEMPEX(return get_direct, width, (data, ndx));
}
...
{
    TESSERA_TEMPEX(return get_two, width, (data, ndx));
}
