...
std::ostream& operator<<(std::ostream& out, const Bson& b)
{
    switch (b.type()) {
        ...
        case Bson::Type::Datetime: {
            auto d = static_cast<tessera::Timestamp>(b);
...
struct LinearMap {
    ...
    iterator erase(iterator)
    {
        ...
        TESSERA_TERMINATE("LinearMap::erase() should never be called");
    }
...
Bson dom_elem_to_bson(const Json& json)
{
    switch (json.type()) {
        ...
        case Json::value_t::discarded:
            TESSERA_TERMINATE("should never see discarded");
    }
    TESSERA_TERMINATE("unknown json value type");
}
...
static constexpr std::pair<std::string_view, FancyParser> bson_fancy_parsers[] = {
    ...
    {"$date",
     +[](const Json& json) {
         ...
         return Bson(tessera::Timestamp(millis_since_epoch / 1000,
...
[[maybe_unused]] bool check_sort_on_startup = [] {
    TESSERA_ASSERT(std::is_sorted(std::begin(bson_fancy_parsers), std::end(bson_fancy_parsers), parser_comp));
    return false;
}();
#endif
