...
public:
    ...
    {
        TESSERA_ASSERT_DEBUG(m_refs.load(std::memory_order_relaxed) == (count - 1));
...
protected:
    ...
    template <typename>
    friend class ::tessera::util::bind_ptr;
    ...
    void bind_ptr() const noexcept
    {
        ...
        TESSERA_ASSERT_DEBUG(ref_count != 0);
    }
...
struct SharedStateBase : public FutureRefCountable {
    ...
    void wait() noexcept
    {
        ...
        if (TESSERA_UNLIKELY(
            ...
            TESSERA_ASSERT_DEBUG(old_state == SSBState::Finished);
            return;
        }
    ...
    void transition_to_finished() noexcept
    {
        ...
        TESSERA_ASSERT_DEBUG(old_state == SSBState::Waiting);
        ...
        for (auto ssb = m_continuation.get(); ssb;
             ssb = ssb->m_state.load(std::memory_order_acquire) == SSBState::Waiting ? ssb->m_continuation.get()
                                                                                     : nullptr) {
            depth++;
            TESSERA_ASSERT(depth < kMaxDepth);
        }
#endif
        if (m_callback) {
            m_callback(this);
    ...
    {
        TESSERA_ASSERT_DEBUG(m_state.load() < SSBState::Finished);
...
struct SharedStateImpl final : public SharedStateBase {
    ...
    {
        TESSERA_ASSERT_DEBUG(m_state.load() < SSBState::Finished);
        TESSERA_ASSERT_DEBUG(other.m_state.load() == SSBState::Finished);
        TESSERA_ASSERT_DEBUG(m_owned_by_promise.load());
    ...
    void disown() const
    {
        TESSERA_ASSERT(m_owned_by_promise.exchange(false));
    }

    void claim() const
    {
        TESSERA_ASSERT(!m_owned_by_promise.exchange(true));
    }
...
public:
    ...
    ~Promise()
    {
        if (TESSERA_UNLIKELY(m_shared_state)) {
    ...
    {
        TESSERA_ASSERT_DEBUG(!status.is_ok());
        set_impl([&](auto& shared_state) {
 ...
 */
template <typename T>
class TESSERA_NODISCARD future_details::Future {
public:
...
private:
    ...
    auto general_impl(SuccessFunc&& success, FailFunc&& fail, NotReady&& notReady) noexcept
    {
        ...
        auto guard = util::make_scope_exit([&]() noexcept {
            auto old_state = SSBState::Init;
            if (TESSERA_UNLIKELY(!m_shared->m_state.compare_exchange_strong(old_state, SSBState::Waiting,
    ...
    {
        TESSERA_ASSERT(!m_shared->m_callback && !m_shared->m_continuation);
 ...
 */
template <>
class TESSERA_NODISCARD future_details::Future<void> {
public:
