...
#ifndef TESSERA_UTIL_SCRATCH_ALLOCATOR_HPP
#define TESSERA_UTIL_SCRATCH_ALLOCATOR_HPP
...
{
    TESSERA_ASSERT(m_current_arena == nullptr);
}
...
{
    TESSERA_ASSERT(&current_arena == m_current_arena);
    TESSERA_ASSERT(checkpoint <= m_position);
...
inline void* ScratchMemory::allocate(const ScratchArena& current_arena, size_t size)
{
    ...
    TESSERA_ASSERT(size % alignment == 0);
...
#endif // TESSERA_UTIL_SCRATCH_ALLOCATOR_HPP
