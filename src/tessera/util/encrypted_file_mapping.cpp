...
{
    To to;
    if (TESSERA_UNLIKELY(int_cast_with_overflow_detect(from, to))) {
...
{
    TESSERA_ASSERT_DEBUG(!int_cast_has_overflow<To>(from));
    ...
    , m_dst_buffer(new char[encryption_page_size])
{
    ...
    TESSERA_ASSERT_RELEASE_EX(ret == 0 && "BCryptOpenAlgorithmProvider()", ret);

    ret = BCryptSetProperty(hAesAlg, BCRYPT_CHAINING_MODE, (PBYTE)BCRYPT_CHAIN_MODE_CBC,
                            sizeof(BCRYPT_CHAIN_MODE_CBC), 0);
    TESSERA_ASSERT_RELEASE_EX(ret == 0 && "BCryptSetProperty()", ret);

    ret = BCryptGenerateSymmetricKey(hAesAlg, &m_aes_key_handle, nullptr, 0, (PBYTE)key, 32, 0);
    TESSERA_ASSERT_RELEASE_EX(ret == 0 && "BCryptGenerateSymmetricKey()", ret);
#else
...
{
    TESSERA_ASSERT(new_data_size >= 0);
...
IVTable& AESCryptor::get_iv_table(FileDesc fd, SizeType data_pos, IVLookupMode mode) noexcept
{
    ...
    TESSERA_ASSERT(idx < m_iv_buffer.capacity()); // required space should have been preallocated
...
{
    TESSERA_ASSERT(page_ndx < m_iv_buffer.capacity());
...
void AESCryptor::write(FileDesc fd, SizeType pos, const char* src, WriteMarker* marker) noexcept
{
    ...
    do {
        ...
        // they're different.
    } while (TESSERA_UNLIKELY(iv.hmac1 == iv.hmac2));
...
void AESCryptor::crypt(EncryptionMode mode, SizeType pos, char* dst, const char* src, const char* stored_iv) noexcept
{
    ...
    TESSERA_ASSERT(err == kCCSuccess);
    TESSERA_ASSERT(bytesEncrypted == encryption_page_size);
#elif defined(_WIN32)
    ULONG cbData;
    int i;
    ...
    if (mode == mode_Encrypt) {
        ...
        TESSERA_ASSERT_RELEASE_EX(i == 0 && "BCryptEncrypt()", i);
        TESSERA_ASSERT_RELEASE_EX(cbData == encryption_page_size && "BCryptEncrypt()", cbData);
    }
    else if (mode == mode_Decrypt) {
        i = BCryptDecrypt(m_aes_key_handle, (PUCHAR)src, encryption_page_size, nullptr, (PUCHAR)iv, sizeof(iv),
                          (PUCHAR)dst, encryption_page_size, &cbData, 0);
        TESSERA_ASSERT_RELEASE_EX(i == 0 && "BCryptDecrypt()", i);
        TESSERA_ASSERT_RELEASE_EX(cbData == encryption_page_size && "BCryptDecrypt()", cbData);
    }
    else {
        TESSERA_UNREACHABLE();
    }
...
EncryptedFileMapping::~EncryptedFileMapping()
{
    ...
    for (auto& e : m_page_state) {
        TESSERA_ASSERT(is_not(e, Writable));
    }
    ...
    TESSERA_ASSERT(it != m_file.mappings.end());
...
{
    TESSERA_ASSERT_EX(addr >= m_addr, addr, m_addr);
...
{
    TESSERA_ASSERT_DEBUG(local_ndx < m_page_state.size());
...
{
    TESSERA_ASSERT_EX(local_ndx < m_page_state.size(), local_ndx, m_page_state.size());
    // Precondition: this method must never be called for a page which
    // is already up to date.
    TESSERA_ASSERT(is_not(m_page_state[local_ndx], UpToDate));
...
bool EncryptedFileMapping::check_possibly_stale_page(size_t local_ndx) noexcept
{
    ...
    for (auto& m : m_file.mappings) {
        ...
        if (is(state, StaleIV)) {
            TESSERA_ASSERT(is_not(state, UpToDate));
            clear(state, StaleIV);
            if (!did_change)
                set(state, UpToDate);
        }
...
{
    TESSERA_ASSERT_EX(local_ndx < m_page_state.size(), local_ndx, m_page_state.size());
    TESSERA_ASSERT(is_not(m_page_state[local_ndx], Dirty));
    TESSERA_ASSERT(is_not(m_page_state[local_ndx], Writable));
...
void EncryptedFileMapping::mark_pages_for_iv_check()
{
    for (auto& state : m_page_state) {
        if (is(state, UpToDate) && is_not(state, Dirty | Writable)) {
            TESSERA_ASSERT(is_not(state, StaleIV));
...
{
    TESSERA_ASSERT(is(m_page_state[local_ndx], Writable));
    TESSERA_ASSERT(is(m_page_state[local_ndx], UpToDate));
    TESSERA_ASSERT(is_not(m_page_state[local_ndx], StaleIV));
    TESSERA_ASSERT(offset + size <= encryption_page_size);
...
{
#ifdef TESSERA_DEBUG
    TESSERA_ASSERT(local_ndx < m_page_state.size());
...
{
    CheckedLockGuard lock(m_file.mutex);
    TESSERA_ASSERT(size > 0);
    TESSERA_ASSERT(m_access == File::access_ReadWrite);
    ...
    while (size > 0) {
        TESSERA_ASSERT(local_ndx < m_page_state.size());
        TESSERA_ASSERT(is(m_page_state[local_ndx], PageState::Writable));
...
{
    CheckedLockGuard lock(m_file.mutex);
    TESSERA_ASSERT_EX(new_size % encryption_page_size == 0, new_size, encryption_page_size);
...
{
    CheckedLockGuard lock(m_file.mutex);
    TESSERA_ASSERT(new_file_offset % encryption_page_size == 0);
    TESSERA_ASSERT(new_size % encryption_page_size == 0);
