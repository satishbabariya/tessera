...
#ifndef TESSERA_UTIL_CF_PTR_HPP
#define TESSERA_UTIL_CF_PTR_HPP
...
public:
    ...
    {
        TESSERA_ASSERT(!m_ref || m_ref != rg.m_ref);
        if (m_ref)
            CFRelease(m_ref);
        m_ref = rg.m_ref;
        rg.m_ref = nullptr;
    ...
    {
        TESSERA_ASSERT(!m_ref || m_ref != ref);
        if (m_ref)
            CFRelease(m_ref);
        m_ref = ref;
    }
