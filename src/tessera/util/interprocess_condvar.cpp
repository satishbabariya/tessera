...
#ifdef TESSERA_CONDVAR_EMULATION
#ifndef _WIN32
#include <unistd.h>
#include <poll.h>
#endif
...
void notify_fd(int fd)
{
    while (true) {
        ...
        TESSERA_ASSERT_EX(ret == -1 && errno == EAGAIN, errno);
        millisleep(1);
        continue;
    }
}
...
#endif // TESSERA_CONDVAR_EMULATION
    ...
    for (;;) {
        ...
        int r;
        {
            if (tp) {
                ...
                TESSERA_ASSERT_DEBUG(!util::int_cast_has_overflow<int64_t>(milliseconds));
...
void InterprocessCondVar::Event::wait(DWORD millis) noexcept
{
    ...
    TESSERA_ASSERT_RELEASE(ret != WAIT_FAILED);
    TESSERA_ASSERT_RELEASE(ret == WAIT_OBJECT_0 || ret == WAIT_TIMEOUT);
}
void InterprocessCondVar::Event::set() noexcept
{
    TESSERA_ASSERT_RELEASE(SetEvent(handle));
}
void InterprocessCondVar::Event::reset() noexcept
{
    TESSERA_ASSERT_RELEASE(ResetEvent(handle));
}
...
void InterprocessCondVar::Mutex::lock() noexcept
{
    ...
    TESSERA_ASSERT_RELEASE(ret != WAIT_FAILED);
    TESSERA_ASSERT_RELEASE(ret == WAIT_OBJECT_0 || ret == WAIT_ABANDONED_0);
}
...
{
    TESSERA_ASSERT_RELEASE(ReleaseMutex(handle));
}

void InterprocessCondVar::update_event_handles()
{
    const int32_t max_process_num = m_shared_part->m_max_process_num;
    TESSERA_ASSERT_RELEASE(max_process_num >= 0);
    TESSERA_ASSERT_RELEASE(max_process_num < 1'000'000); // Sanity check.
