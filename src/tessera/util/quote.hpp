#ifndef TESSERA_UTIL_QUOTE_HPP
#define TESSERA_UTIL_QUOTE_HPP
...
quote:
    ...
    if (TESSERA_LIKELY(sentry)) {
        out.put(dquote); // Throws
        bool follows_hex = false;
        for (C ch : view) {
            if (TESSERA_LIKELY(ctype.is(ctype.print, ch))) {
                if (TESSERA_LIKELY(!follows_hex || !ctype.is(ctype.xdigit, ch))) {
                    if (TESSERA_LIKELY(ch != dquote && ch != bslash))
                        goto put_char;
                    goto escape_char;
                }
            }
