...
#ifndef TESSERA_UTIL_THREAD_HPP
#define TESSERA_UTIL_THREAD_HPP
...
protected:
    ...
    TESSERA_NORETURN static void init_failed(int);
    TESSERA_NORETURN static void attr_init_failed(int);
    TESSERA_NORETURN static void destroy_failed(int) noexcept;
    TESSERA_NORETURN static void lock_failed(int) noexcept;
...
inline void RobustMutex::lock(Func recover_func)
{
    ...
    if (TESSERA_LIKELY(no_thread_has_died))
        return;
    try {
        recover_func(); // Throws
        mark_as_consistent();
...
inline void CondVar::wait(LockGuard& l) noexcept
{
    ...
    if (TESSERA_UNLIKELY(r != 0))
        TESSERA_TERMINATE("pthread_cond_wait() failed");
#endif
}
