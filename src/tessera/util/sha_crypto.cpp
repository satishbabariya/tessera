...
#define TESSERA_USE_BUNDLED_SHA2 1
#elif !TESSERA_HAVE_OPENSSL
#include <sha1.h>
#define TESSERA_USE_BUNDLED_SHA2 1
#endif
 ...
 * TESSERA_USE_BUNDLED_SHA2 macro should be undefined.
...
#undef TESSERA_USE_BUNDLED_SHA2
#endif

#ifdef TESSERA_USE_BUNDLED_SHA2
#include <sha224.hpp>
#include <sha256.hpp>
#include <cstring>
#endif
...
struct Algorithm {
    Algorithm(LPCWSTR alg_id)
    {
        if (BCryptOpenAlgorithmProvider(&hAlg, alg_id, NULL, 0) < 0) {
            throw tessera::util::runtime_error("BCryptOpenAlgorithmProvider() failed");
        }
    }
    ~Algorithm()
    {
...
struct Hash {
    ...
    {
        TESSERA_ASSERT(alg.obj_length() < 512);
        TESSERA_ASSERT(alg.hash_length() == hash_size);
        if (BCryptCreateHash(alg.hAlg, &hHash, hash_object_buffer, 515, NULL, 0, 0) < 0) {
            throw tessera::util::runtime_error("BCryptCreateHash() failed");
        }
    }
    ~Hash()
    {
    ...
    void get_hash(PUCHAR in_buffer, DWORD in_buffer_size, PUCHAR out_buffer)
    {
        if (BCryptHashData(hHash, in_buffer, in_buffer_size, 0) < 0) {
            throw tessera::util::runtime_error("BCryptHashData() failed");
        }
                    ...
                    unsigned char* out_buffer, unsigned int* output_size)
{
    ...
    if (rc == 0) {
        ...
        throw tessera::util::runtime_error("EVP_DigestInit() failed");
    }

    rc = EVP_DigestUpdate(ctx, in_buffer, in_buffer_size);
    if (rc == 0) {
        EVP_MD_CTX_destroy(ctx);
        throw tessera::util::runtime_error("EVP_DigestUpdate() failed");
    }
    ...
    if (rc == 0)
        throw tessera::util::runtime_error("EVP_DigestFinal_ex() failed");
}
#endif
...
void hmac_sha224(Span<const uint8_t> in_buffer, Span<uint8_t, 28> out_buffer, Span<const uint8_t, 32> key)
{
...
#elif defined(TESSERA_USE_BUNDLED_SHA2)
    ...
    TESSERA_ASSERT_DEBUG(hashLen == out_buffer.size());
#else
