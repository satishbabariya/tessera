...
template <>
std::string print_value<>(tessera::Timestamp t)
{
    if (t.is_null()) {
        return "NULL";
    }
...
template <>
std::string print_value<>(tessera::ObjectId oid)
{
    return "oid(" + oid.to_string() + ")";
}

template <>
std::string print_value<>(tessera::ObjKey k)
{
    std::stringstream ss;
    if (!k) {
        ss << "NULL";
...
std::string print_value(tessera::ObjLink link, Group* g)
{
    if (!link) {
        return "NULL";
    }
...
template <>
std::string print_value<>(tessera::UUID uuid)
{
    return "uuid(" + uuid.to_string() + ")";
}

template <>
std::string print_value<>(tessera::TypeOfValue type)
{
...
template <>
std::string print_value<>(const tessera::Geospatial& geo)
{
...
std::string SerialisationState::get_backlink_column_name(ConstTableRef from, ColKey col_key)
{
    ...
    TESSERA_ASSERT_EX(col_type == col_type_Link, col_type);
