...
#ifndef TESSERA_OS_CHECKED_MUTEX_HPP
#define TESSERA_OS_CHECKED_MUTEX_HPP
...
#define TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(x) __attribute__((x))
#else
#define TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(x)
#endif

#define CAPABILITY(x) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(capability(x))

#define RETURN_CAPABILITY(x) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(return_capability(x))

#define SCOPED_CAPABILITY TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(scoped_lockable)

#define GUARDED_BY(x) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(guarded_by(x))

#define REQUIRES(...) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(requires_capability(__VA_ARGS__))

#define ACQUIRE(...) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(acquire_capability(__VA_ARGS__))

#define RELEASE(...) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(release_capability(__VA_ARGS__))

#define EXCLUDES(...) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(locks_excluded(__VA_ARGS__))

#define NO_THREAD_SAFETY_ANALYSIS TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(no_thread_safety_analysis)
#define ASSERT_CAPABILITY(x) TESSERA_THREAD_ANNOTATION_ATTRIBUTE__(assert_capability(x))
...
public:
    ...
    {
        TESSERA_ASSERT(lock.owns_lock());
        TESSERA_ASSERT(lock.native_handle().mutex() == &m_mutex);
...
#endif // TESSERA_OS_CHECKED_MUTEX_HPP
