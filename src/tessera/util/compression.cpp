...
public:
    ...
    {
        return "tessera::util::compression::error";
    }
    std::string message(int err) const override final
    {
        using error = tessera::util::compression::error;
        error e = error(err);
        switch (e) {
...
{
    using Alloc = tessera::util::compression::Alloc;
...
void grow_arena(compression::CompressMemoryArena& compress_memory_arena)
{
    ...
    TESSERA_ASSERT(n != 0);
    TESSERA_ASSERT(n != std::numeric_limits<std::size_t>::max());
...
void inflate_zlib_header(z_stream& strm)
{
    ...
    TESSERA_ASSERT(rc == Z_OK);
    TESSERA_ASSERT(strm.avail_in == 0);
}
                                ...
                                bool has_header)
{
    ...
    do {
        ...
        while (in_offset < compressed_buf.size()) {
            ...
            int rc = inflate(&strm, Z_SYNC_FLUSH);
            TESSERA_ASSERT(rc != Z_STREAM_ERROR && rc != Z_MEM_ERROR);
                                      ...
                                      std::size_t& compressed_size, int compression_level, Alloc* custom_allocator)
{
    ...
    while (rc == Z_OK || rc == Z_BUF_ERROR) {
        TESSERA_ASSERT(strm.next_in + strm.avail_in == uncompressed_ptr + next_in_ndx);
        TESSERA_ASSERT(strm.next_out + strm.avail_out == compressed_ptr + next_out_ndx);
        ...
        TESSERA_ASSERT(rc != Z_STREAM_END || flush == Z_FINISH);
    }
                                                    ...
                                                    util::AppendBuffer<char>& compressed)
{
    ...
    while (uncompressed.size() > 256) {
        ...
        TESSERA_ASSERT(compressed_size);
