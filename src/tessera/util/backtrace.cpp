...
#if !defined(TESSERA_HAVE_BACKTRACE) && (TESSERA_PLATFORM_APPLE || (defined(__linux__) && defined(__GNUC__)))
// we detect the backtrace facility in CMake, but if building outside it we assume
// it's available on Apple or Linux/glibc
#define TESSERA_HAVE_BACKTRACE 1
#endif

#if TESSERA_HAVE_BACKTRACE
#if defined(TESSERA_BACKTRACE_HEADER)
#include TESSERA_BACKTRACE_HEADER
#else
#include <execinfo.h>
#endif
#endif
...
Backtrace Backtrace::capture() noexcept
{
    ...
    if (TESSERA_UNLIKELY(frames <= 1)) {
        ...
        if (TESSERA_UNLIKELY(memory == nullptr)) {
