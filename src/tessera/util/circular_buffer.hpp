...
#ifndef TESSERA_UTIL_CIRCULAR_BUFFER_HPP
#define TESSERA_UTIL_CIRCULAR_BUFFER_HPP
...
public:
    ...
    {
        TESSERA_ASSERT(m_buffer == i.m_buffer);
...
void CircularBuffer<T>::reserve(size_type capacity)
{
    ...
    if (TESSERA_UNLIKELY(int_add_with_overflow_detect(min_allocated_size, 1)))
    ...
    if (TESSERA_UNLIKELY(int_multiply_with_overflow_detect(new_allocated_size, 2)))
...
inline auto CircularBuffer<T>::emplace_front(Args&&... args) -> reference
{
    ...
    TESSERA_ASSERT(m_allocated_size > 0);
    T* memory = get_memory_ptr();
...
inline auto CircularBuffer<T>::circular_inc(size_type index) noexcept -> size_type
{
    ...
    if (TESSERA_LIKELY(index_2 < m_allocated_size))
        return index_2;
    return 0;
}
...
{
    TESSERA_ASSERT(new_allocated_size > 1);
    TESSERA_ASSERT(new_allocated_size > m_size);
...
#endif // TESSERA_UTIL_CIRCULAR_BUFFER_HPP
