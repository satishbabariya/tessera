...
#ifndef TESSERA_UTIL_COMPRESSION_HPP
#define TESSERA_UTIL_COMPRESSION_HPP
...
#ifndef TESSERA_USE_LIBCOMPRESSION
#define TESSERA_USE_LIBCOMPRESSION TESSERA_PLATFORM_APPLE
#endif

namespace tessera::util::compression {
...
template <>
struct is_error_code_enum<tessera::util::compression::error> {
...
public:
    void* alloc(size_t size) noexcept override final
    {
        ...
        if (padding > m_size - offset)
            return nullptr;
        offset += padding;
        TESSERA_ASSERT(offset % alignof(std::max_align_t) == 0);
...
#endif // TESSERA_UTIL_COMPRESSION_HPP
