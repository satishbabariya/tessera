...
#ifndef TESSERA_UTIL_SPAN_HPP
#define TESSERA_UTIL_SPAN_HPP
...
public:
    ...
    constexpr explicit Span(pointer ptr, size_type count)
        : m_data{ptr}
    {
        TESSERA_ASSERT(extent == count);
    }
    constexpr explicit Span(pointer begin, pointer end)
        : m_data{begin}
    {
        TESSERA_ASSERT(extent == std::distance(begin, end));
    }
    ...
    constexpr explicit Span(Container& c)
        : m_data{std::data(c)}
    {
        TESSERA_ASSERT(extent == std::size(c));
    }
    ...
    constexpr explicit Span(const Span<U, dynamic_extent>& other) noexcept
        : m_data{other.data()}
    {
        TESSERA_ASSERT(extent == other.size());
    }
#endif
    ...
    {
        TESSERA_ASSERT(offset <= size());
        TESSERA_ASSERT(count <= size() || count == dynamic_extent);
        if (count == dynamic_extent)
            return {data() + offset, size() - offset};
        TESSERA_ASSERT(count <= size() - offset);
    ...
    {
        TESSERA_ASSERT(idx < size());
        return m_data[idx];
    }
    constexpr reference front() const noexcept
    {
        TESSERA_ASSERT(!empty());
        return m_data[0];
    }
...
public:
    ...
    {
        TESSERA_ASSERT(offset <= m_size);
        TESSERA_ASSERT(count == dynamic_extent || count <= m_size - offset);
    ...
    {
        TESSERA_ASSERT(offset <= m_size);
        TESSERA_ASSERT(count <= m_size || count == dynamic_extent);
        if (count == dynamic_extent)
            return {m_data + offset, m_size - offset};
        TESSERA_ASSERT(count <= m_size - offset);
    ...
    constexpr reference operator[](size_type idx) const noexcept
    {
        TESSERA_ASSERT(idx < m_size);
        return m_data[idx];
    }
    constexpr reference front() const noexcept
    {
        TESSERA_ASSERT(m_size);
        return m_data[0];
    }
    constexpr reference back() const noexcept
    {
