...
TypeOfValue::TypeOfValue(const ColKey& col_key)
{
    ...
    TESSERA_ASSERT_RELEASE(col_type != col_type_Mixed);
    DataType data_type = DataType(col_type);
    TESSERA_ASSERT_RELEASE(data_type.is_valid());
...
std::string TypeOfValue::to_string() const
{
    ...
    while (bit_to_check <= m_attributes) {
        if (m_attributes & bit_to_check) {
            ...
            TESSERA_ASSERT_RELEASE_EX(val, bit_to_check);
            values.push_back(*val);
        }
        bit_to_check <<= 1;
    }
    TESSERA_ASSERT_RELEASE(values.size() > 0);
    if (values.size() == 1) {
        return values[0];
    }
    else {
