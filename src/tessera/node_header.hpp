...
#ifndef TESSERA_NODE_HEADER_HPP
#define TESSERA_NODE_HEADER_HPP
...
public:
    ...
    {
        TESSERA_ASSERT_3(value, <=, max_array_size);
        typedef unsigned char uchar;
        uchar* h = reinterpret_cast<uchar*>(header);
    ...
    {
        TESSERA_ASSERT_3(value, <=, (0xffffff << 3));
        typedef unsigned char uchar;
        uchar* h = reinterpret_cast<uchar*>(header);
    ...
    static size_t calc_byte_size(WidthType wtype, size_t size, uint_least8_t width) noexcept
    {
        ...
        switch (wtype) {
            case wtype_Bits: {
                ...
                TESSERA_ASSERT_3(size, <, 0x1000000);
