...
#ifdef TESSERA_SLAB_ALLOC_DEBUG
#include <cstdlib>
#endif

#include <tessera/util/encrypted_file_mapping.hpp>
#include <tessera/util/errno.hpp>
    ...
    , size(s)
{
    ...
    addr = new char[size];
    TESSERA_ASSERT((reinterpret_cast<size_t>(addr) & 0x7ULL) == 0);
#if TESSERA_ENABLE_ALLOC_SET_ZERO
    ...
    if (is_attached()) {
        ...
        if (m_attach_mode != attach_SharedFile) {
            ...
            if (m_free_space_state != free_space_Invalid) {
                if (TESSERA_COVER_NEVER(!is_all_free())) {
                    print();
#ifndef TESSERA_SLAB_ALLOC_DEBUG
                    std::cerr << "To get the stack-traces of the corresponding allocations,"
                                 "first compile with TESSERA_SLAB_ALLOC_DEBUG defined,"
                                 "then run under Valgrind with --leak-check=full\n";
                    TESSERA_TERMINATE("SlabAlloc detected a leak");
#endif
                }
            }
        }
...
MemRef SlabAlloc::do_alloc(size_t size)
{
    ...
    TESSERA_ASSERT_EX(0 < size, size, get_file_path_for_assertions());
    TESSERA_ASSERT_EX((size & 0x7) == 0, size,
                    get_file_path_for_assertions()); // only allow sizes that are multiples of 8
    TESSERA_ASSERT_EX(is_attached(), get_file_path_for_assertions());
    ...
    TESSERA_ASSERT_RELEASE_EX(size < (1 << section_shift), size, get_file_path_for_assertions());

    // If we failed to correctly record free space, new allocations cannot be
    // carried out until the free space record is reset.
    if (TESSERA_COVER_NEVER(m_free_space_state == free_space_Invalid))
...
#ifdef TESSERA_DEBUG
    if (TESSERA_COVER_NEVER(m_debug_out))
        std::cerr << "Alloc ref: " << ref << " size: " << size << "\n";
#endif

    char* addr = reinterpret_cast<char*>(entry);
    TESSERA_ASSERT_EX(addr == translate(ref), addr, ref, get_file_path_for_assertions());
...
#endif
    TESSERA_ASSERT_EX(ref >= m_baseline, ref, m_baseline, get_file_path_for_assertions());
...
void SlabAlloc::remove_freelist_entry(FreeBlock* entry)
{
    ...
    TESSERA_ASSERT_EX(it != m_block_map.end(), get_file_path_for_assertions());
...
{
    auto bb = bb_before(entry);
    TESSERA_ASSERT_EX(bb->block_after_size < 0, bb->block_after_size, get_file_path_for_assertions());
    auto alloc_size = -bb->block_after_size;
    int max_waste = sizeof(FreeBlock) + sizeof(BetweenBlocks);
    TESSERA_ASSERT_EX(alloc_size >= size && alloc_size <= size + max_waste, alloc_size, size,
                    get_file_path_for_assertions());
    bb->block_after_size = alloc_size;
    bb = bb_after(entry);
    TESSERA_ASSERT_EX(bb->block_before_size < 0, bb->block_before_size, get_file_path_for_assertions());
    TESSERA_ASSERT(-bb->block_before_size == alloc_size);
...
void SlabAlloc::mark_allocated(FreeBlock* entry)
{
    ...
    TESSERA_ASSERT_EX(bb2->block_before_size > 0, bb2->block_before_size, get_file_path_for_assertions());
...
SlabAlloc::FreeBlock* SlabAlloc::allocate_block(int size)
{
    ...
    TESSERA_ASSERT_EX(size_from_block(block) >= size, size_from_block(block), size, get_file_path_for_assertions());
    return block;
}
...
SlabAlloc::FreeBlock* SlabAlloc::grow_slab(int size)
{
    ...
    if (m_slabs.empty()) {
        ...
        TESSERA_ASSERT_DEBUG_EX(curr_ref_end >= m_baseline, curr_ref_end, m_baseline, get_file_path_for_assertions());
        ref = curr_ref_end;
    }
    ref = align_size_to_section_boundary(ref);
    size_t ref_end = ref;
    if (TESSERA_UNLIKELY(int_add_with_overflow_detect(ref_end, new_size))) {
        throw MaximumFileSizeExceeded("AllocSlab slab ref_end size overflow: " + util::to_string(ref) + " + " +
                                      util::to_string(new_size));
    }

    TESSERA_ASSERT(matches_section_boundary(ref));
...
{
    TESSERA_ASSERT_EX(translate(ref) == addr, translate(ref), addr, get_file_path_for_assertions());
    CriticalSection cs(changes);
    ...
    TESSERA_ASSERT_EX(read_only || m_free_space_state == free_space_Dirty, read_only, m_free_space_state,
    ...
    if (read_only) {
        ...
        try {
            TESSERA_ASSERT_RELEASE_EX(ref != 0, ref, get_file_path_for_assertions());
            TESSERA_ASSERT_RELEASE_EX(!(ref & 7), ref, get_file_path_for_assertions());
            auto next = m_free_read_only.lower_bound(ref);
            if (next != m_free_read_only.end()) {
                TESSERA_ASSERT_RELEASE_EX(ref + size <= next->first, ref, size, next->first, next->second,
                                        get_file_path_for_assertions());
            ...
            if (!m_free_read_only.empty() && next != m_free_read_only.begin()) {
                ...
                TESSERA_ASSERT_RELEASE_EX(prev->first + prev->second <= ref, ref, size, prev->first, prev->second,
                                        get_file_path_for_assertions());
        ...
        TESSERA_ASSERT_RELEASE_EX(size < 2UL * 1024 * 1024 * 1024, size, get_file_path_for_assertions());
...
{
    TESSERA_ASSERT_DEBUG(translate(ref) == addr);
    TESSERA_ASSERT_EX(0 < new_size, new_size, get_file_path_for_assertions());
    TESSERA_ASSERT_EX((new_size & 0x7) == 0, new_size,
                    get_file_path_for_assertions()); // only allow sizes that are multiples of 8
    ...
    // fill with zero if TESSERA_ENABLE_ALLOC_SET_ZERO is non-zero.
    ...
    tessera::safe_copy_n(addr, old_size, new_addr);
...
{
    TESSERA_ASSERT(false); // never come here
    return nullptr;
}
...
bool SlabAlloc::align_filesize_for_mmap(ref_type top_ref, Config& cfg)
{
    ...
    TESSERA_ASSERT(!is_file_on_streaming_form());
...
ref_type SlabAlloc::attach_file(const std::string& path, Config& cfg, util::WriteObserver* write_observer)
{
    ...
    TESSERA_ASSERT_EX(!(cfg.is_shared && cfg.read_only), cfg.is_shared, cfg.read_only, get_file_path_for_assertions());
    // session_initiator can be set *only* if we're shared.
    TESSERA_ASSERT_EX(cfg.is_shared || !cfg.session_initiator, cfg.is_shared, cfg.session_initiator,
                    get_file_path_for_assertions());
    // clear_file can be set *only* if we're the first session.
    TESSERA_ASSERT_EX(cfg.session_initiator || !cfg.clear_file, cfg.session_initiator, cfg.clear_file,
                    get_file_path_for_assertions());
    ...
    TESSERA_ASSERT(m_mappings.size());
...
void SlabAlloc::convert_from_streaming_form(ref_type top_ref)
{
    ...
    // session initialization, even if it means writing the database during open.
    {
        ...
        tessera::util::encryption_write_barrier(writable_map, 0);
        writable_map.sync();
        util::encryption_read_barrier(writable_map, 0);
...
ref_type SlabAlloc::attach_buffer(const char* data, size_t size)
{
    ...
    TESSERA_ASSERT_EX(!is_attached(), get_file_path_for_assertions());
    TESSERA_ASSERT_EX(size <= (1UL << section_shift), get_file_path_for_assertions());
...
char* SlabAlloc::translate_memory_pos(ref_type ref) const noexcept
{
    ...
    TESSERA_ASSERT(idx < m_virtual_file_buffer.size());
                                    ...
                                    const std::string& path, bool is_encrypted)
{
    ...
    if (slot_selector == 0 && top_ref == 0xFFFFFFFFFFFFFFFFULL) {
        ...
        if (TESSERA_UNLIKELY(footer->m_magic_cookie != footer_magic_cookie)) {
    ...
    if (TESSERA_UNLIKELY(top_ref % 8 != 0)) {
        throw_header_exception("top ref is not aligned", *header, path);
    }
    if (TESSERA_UNLIKELY(top_ref >= size)) {
...
void SlabAlloc::reset_free_space_tracking()
{
    ...
    while (m_slabs.size() > 1 || (m_slabs.size() == 1 && m_slabs[0].size > minimal_alloc)) {
        ...
        TESSERA_ASSERT(last_translation.mapping_addr == last_slab.addr);
...
void SlabAlloc::update_reader_view(size_t file_size)
{
    ...
    if (!is_in_memory()) {
        TESSERA_ASSERT_EX(file_size % 8 == 0, file_size, get_file_path_for_assertions()); // 8-byte alignment required
        TESSERA_ASSERT_EX(m_attach_mode == attach_SharedFile || m_attach_mode == attach_UnsharedFile, m_attach_mode,
                        get_file_path_for_assertions());
        TESSERA_ASSERT_DEBUG(is_free_space_clean());
        ...
        TESSERA_ASSERT(m_mappings.size() == old_num_mappings);

        {
            ...
            if (old_baseline < old_slab_base) {
                ...
                TESSERA_ASSERT(old_num_mappings > 0);
        ...
        if (replace_last_mapping) {
            ...
            TESSERA_ASSERT(!cur_entry.xover_mapping.is_attached());
...
void SlabAlloc::rebuild_translations(bool requires_new_translation, size_t old_num_sections)
{
    ...
    for (size_t i = old_num_sections; i < num_mappings; ++i) {
        ...
        TESSERA_ASSERT(new_translation_table[i].mapping_addr);
...
void SlabAlloc::get_or_add_xover_mapping(RefTranslation& txl, size_t index, size_t offset, size_t size)
{
    ...
    if (txl.xover_mapping_addr.load(std::memory_order_relaxed)) {
        ...
        TESSERA_ASSERT(offset == txl.lowest_possible_xover_offset.load(std::memory_order_relaxed));
        return;
    }
    MapEntry* map_entry = &m_mappings[index];
    TESSERA_ASSERT(map_entry->primary_mapping.get_addr() == txl.mapping_addr);
...
void SlabAlloc::verify_old_translations(uint64_t youngest_live_version)
{
    ...
    for (auto& m : m_old_mappings) {
        TESSERA_ASSERT(m.mapping.is_attached());
        mappings.insert(m.mapping.get_addr());
    }
    for (auto& m : m_mappings) {
        TESSERA_ASSERT(m.primary_mapping.is_attached());
    ...
    for (auto& t : m_old_translations) {
        TESSERA_ASSERT_EX(youngest_live_version == 0 || t.replaced_at_version < youngest_live_version,
                        youngest_live_version, t.replaced_at_version);
        if (nonempty_attachment()) {
            for (size_t i = 0; i < t.translation_count; ++i)
                TESSERA_ASSERT(mappings.count(t.translations[i].mapping_addr));
        }
    }
#else
...
{
    if (m_attach_mode == attach_SharedFile) {
        TESSERA_ASSERT_EX(new_file_size == round_up_to_page_size(new_file_size), get_file_path_for_assertions());
        ...
        TESSERA_ASSERT_EX(new_file_size <= static_cast<size_t>(m_file.get_size()), new_file_size, m_file.get_size());
...
void SlabAlloc::verify() const
{
    ...
    if (!m_slabs.empty()) {
        ...
        for_all_free_entries([&](size_t ref, size_t size) {
            TESSERA_ASSERT(ref >= first_possible_ref);
            TESSERA_ASSERT(ref + size <= first_impossible_ref);
...
bool SlabAlloc::is_all_free() const
{
    ...
    for (const auto& e : m_slabs) {
        ...
        TESSERA_ASSERT(first->block_before_size == 0);
        auto last = reinterpret_cast<BetweenBlocks*>(e.addr + e.size) - 1;
        TESSERA_ASSERT(last->block_after_size == 0);
