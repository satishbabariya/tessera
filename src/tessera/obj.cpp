...
#include "tessera/cluster_tree.hpp"
...
inline void nullify_linklist(Obj& obj, ColKey origin_col_key, T target)
{
    ...
    TESSERA_ASSERT(ndx != tessera::npos); // There has to be one
...
StableIndex Obj::build_index(ColKey col_key) const
{
    ...
    TESSERA_ASSERT(col_key.get_type() == col_type_Mixed);
    _update_if_needed();
...
T Obj::get(ColKey col_key) const
{
    ...
    TESSERA_ASSERT(type == ColumnTypeTraits<T>::column_id);
...
Geospatial Obj::get(ColKey col_key) const
{
    m_table->check_column(col_key);
    ColumnType type = col_key.get_type();
    TESSERA_ASSERT(type == ColumnTypeTraits<Link>::column_id);
...
bool Obj::is_unresolved(ColKey col_key) const
{
    m_table->check_column(col_key);
    ColumnType type = col_key.get_type();
    TESSERA_ASSERT(type == col_type_Link);
...
int64_t Obj::_get<int64_t>(ColKey::Idx col_ndx) const
{
    ...
    TESSERA_TEMPEX(return get_direct, width, (data, m_row_ndx));
}
...
int64_t Obj::get<int64_t>(ColKey col_key) const
{
    m_table->check_column(col_key);
    ColumnType type = col_key.get_type();
    TESSERA_ASSERT(type == col_type_Int);
...
bool Obj::get<bool>(ColKey col_key) const
{
    m_table->check_column(col_key);
    ColumnType type = col_key.get_type();
    TESSERA_ASSERT(type == col_type_Bool);
...
void Obj::verify_backlink(const Table& origin, ColKey origin_col_key, ObjKey origin_key) const
{
    ...
    TESSERA_ASSERT(backlinks.verify_backlink(m_row_ndx, origin_key.value));
#else
...
void Obj::traverse_path(Visitor v, PathSizer ps, size_t path_length) const
{
    struct BacklinkTraverser : public LinkTranslator {
        ...
        void on_dictionary(Dictionary& dict) final
        {
            ...
            TESSERA_ASSERT(!m_index.is_null());
        }
        void on_list_of_mixed(Lst<Mixed>&) final
        {
            TESSERA_UNREACHABLE(); // we don't support Mixed link to embedded object yet
        }
        void on_set_of_links(LnkSet&) final
        {
            TESSERA_UNREACHABLE(); // sets of embedded objects are not allowed at the schema level
        }
    ...
    if (m_table->is_embedded()) {
        TESSERA_ASSERT(get_backlink_count() == 1);
        ...
        m_table->for_each_backlink_column([&](ColKey col_key) {
            ...
            if (backlinks.size() == 1) {
                ...
                if (attr.test(col_attr_List)) {
                    ...
                    TESSERA_ASSERT(ndx != tessera::not_found);
...
TESSERA_FORCEINLINE void Obj::sync(Node& arr)
{
...
Obj& Obj::set<Mixed>(ColKey col_key, Mixed value, bool is_default)
{
    ...
    if (!value.is_same_type(old_value) || value != old_value) {
        ...
        TESSERA_ASSERT(col_ndx.val + 1 < fields.size());
...
{
    if (value.is_null()) {
        TESSERA_ASSERT(col_key.get_attrs().test(col_attr_Nullable));
...
Obj Obj::create_and_set_linked_object(ColKey col_key, bool is_default)
{
    ...
    TESSERA_ASSERT(t.is_embedded() || !get_table()->is_asymmetric());
    // Incoming links to asymmetric objects are disallowed.
    TESSERA_ASSERT(!t.is_asymmetric());
    ...
    TESSERA_ASSERT(target_key != old_key); // We will always create a new object
    CascadeState state;
...
{
    if (TESSERA_UNLIKELY(val.size() > Table::max_string_size))
        throw LogicError(ErrorCodes::LimitExceeded, "String too big");
}
template <>
inline void check_range(const BinaryData& val)
{
    if (TESSERA_UNLIKELY(val.size() > ArrayBlob::max_binary_size))
...
inline void Obj::nullify_single_link(ColKey col, ValueType target)
{
    ...
    TESSERA_ASSERT(links.get(m_row_ndx) == target);
...
{
    TESSERA_ASSERT(get_alloc().get_storage_version() == m_storage_version);
...
struct EmbeddedObjectLinkMigrator : public LinkTranslator {
    ...
    void on_set_of_mixed(Set<Mixed>& set) final
    {
        ...
        TESSERA_ASSERT(did_erase_pair.second);
    ...
    {
        TESSERA_ASSERT(m_origin_obj.get<Mixed>(col).is_null() ||
...
{
    TESSERA_ASSERT(!m_table->get_primary_key_column());
...
{
    TESSERA_ASSERT(col_key.is_dictionary() || col_key.get_type() == col_type_Mixed);
    checked_update_if_needed();
...
CollectionBasePtr Obj::get_collection_ptr(ColKey col_key) const
{
    ...
    TESSERA_ASSERT(val.is_type(type_Dictionary));
...
void Obj::assign_pk_and_backlinks(Obj& other)
{
    struct LinkReplacer : LinkTranslator {
        ...
        void on_mixed_property(ColKey col) final
        {
            ...
            if (val.is_type(type_Dictionary)) {
                ...
                TESSERA_ASSERT(val.is_null() || val.get_link().get_obj_key() == m_dest_orig.get_key());
    ...
    TESSERA_ASSERT(get_table() == other.get_table());
    ...
    auto copy_links = [this, &other, nb_tombstones](ColKey col) {
        ...
        if (c.get_type() == col_type_Link && !(c.is_dictionary() || c.is_set())) {
            ...
            TESSERA_ASSERT(_get<int64_t>(idx) == 0);
...
template Mixed Obj::get<Mixed>(tessera::ColKey) const;
template UUID Obj::get<UUID>(tessera::ColKey) const;
...
Obj& Obj::set_null(ColKey col_key, bool is_default)
{
    ...
    auto attrs = col_key.get_attrs();
    if (TESSERA_UNLIKELY(!attrs.test(col_attr_Nullable))) {
        throw NotNullable(Group::table_name_to_class_name(m_table->get_name()), m_table->get_column_name(col_key));
    }
...
ref_type Obj::get_collection_ref(StableIndex index, CollectionType type) const
{
    ...
    if (check_index(index)) {
        ...
        throw tessera::IllegalOperation(util::format("Not a %1", type));
    }
...
bool Obj::remove_backlink(ColKey col_key, ObjLink old_link, CascadeState& state) const
{
    ...
    TESSERA_ASSERT(m_table->valid_column(col_key));
