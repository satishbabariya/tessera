...
#ifndef TESSERA_ARRAY_BASIC_TPL_HPP
#define TESSERA_ARRAY_BASIC_TPL_HPP
...
{
    TESSERA_ASSERT(type == NodeHeader::type_Normal);
    TESSERA_ASSERT(!context_flag);
    size_t length = 0;
...
void BasicArray<T>::truncate(size_t to_size)
{
    ...
    TESSERA_ASSERT_3(to_size, <=, m_size);
...
size_t BasicArrayNull<T>::find_first_null(size_t begin, size_t end) const
{
    ...
    TESSERA_ASSERT(begin <= sz && end <= sz && begin <= end);
    while (begin != end) {
...
#endif // TESSERA_ARRAY_BASIC_TPL_HPP
