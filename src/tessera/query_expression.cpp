...
void LinkMap::set_base_table(ConstTableRef table)
{
    ...
    for (size_t i = 0; i < m_link_column_keys.size(); i++) {
        ...
        TESSERA_ASSERT(Table::is_link_type(type) || type == col_type_BackLink);
        ...
        TESSERA_ASSERT(table->valid_column(link_column_key));
...
bool LinkMap::map_links(size_t column, ObjKey key, LinkMapFunction lm) const
{
    ...
    if (column_key.is_collection()) {
        ...
        if (pe.is_all()) {
            ...
            TESSERA_ASSERT(column_key.is_dictionary());
            ...
            TESSERA_ASSERT(column_key.is_list());
        ...
        TESSERA_TERMINATE("Invalid column type in LinkMap::map_links()");
    }
    return true;
}
            ...
            if (leaf.get(row)) {
                ...
                for (; start < end; start++) {
                    ...
                    if (m.is_type(type_TypedLink)) {
                        ...
                        TESSERA_ASSERT(link.get_table_key() == this->m_tables[column + 1]->get_key());
