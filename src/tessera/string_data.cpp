...
TESSERA_FORCEINLINE bool matchlike(const StringData& text, const StringData& pattern,
                                 const StringData* alternate_pattern = nullptr) noexcept
{
    ...
    TESSERA_ASSERT_DEBUG(has_alternate_pattern == bool(alternate_pattern));
    TESSERA_ASSERT_DEBUG(!alternate_pattern || pattern.size() == alternate_pattern->size());
...
bool StringData::matchlike(const tessera::StringData& text, const tessera::StringData& pattern) noexcept
{
...
size_t tessera::murmur2_or_cityhash(const unsigned char* data, size_t len) noexcept
{
    return size_t(Murmur2OrCityHash<>{}(data, len));
}

uint_least32_t tessera::murmur2_32(const unsigned char* data, size_t len) noexcept
{
    // This implementation is copied from libc++.
    // See: https://github.com/llvm-mirror/libcxx/blob/master/include/utility

    TESSERA_ASSERT_DEBUG(len <= std::numeric_limits<uint_least32_t>::max());
...
uint_least64_t tessera::cityhash_64(const unsigned char* data, size_t len) noexcept
{
