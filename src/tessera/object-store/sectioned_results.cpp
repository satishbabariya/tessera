...
void SectionedResults::calculate_sections()
{
    ...
    if (!m_has_performed_initial_evaluation) {
        TESSERA_ASSERT_EX(m_previous_key_to_index.size() == 0, m_previous_key_to_index.size());
        TESSERA_ASSERT_EX(m_previous_index_to_key.size() == 0, m_previous_index_to_key.size());
