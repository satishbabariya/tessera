    ...
    , m_scheduler(m_config.scheduler)
{
    if (version) {
        ...
        TESSERA_ASSERT(*version != VersionID());
    }
    ...
    switch (m_config.schema_mode) {
        ...
        case SchemaMode::Manual:
            ...
            if (version == m_schema_version) {
                ...
                TESSERA_UNREACHABLE(); // changes is non-empty so above line always throws
            }
            return true;
    }
...
void Realm::set_schema_subset(Schema schema)
{
    verify_thread();
    verify_open();
    TESSERA_ASSERT(m_dynamic_schema);
    TESSERA_ASSERT(m_schema_version != ObjectStore::NotVersioned);
...
auto Realm::async_begin_transaction(util::UniqueFunction<void()>&& the_write_block, bool notify_only) -> AsyncHandle
{
    ...
    TESSERA_ASSERT(the_write_block);
    ...
    -> AsyncHandle
{
    ...
    TESSERA_ASSERT(m_transaction->holds_write_mutex());
    TESSERA_ASSERT(!m_notify_only);
    // auditing is not supported
    TESSERA_ASSERT(!audit_context());
...
bool Realm::is_frozen() const
{
    ...
    TESSERA_ASSERT_DEBUG(!result || !m_transaction || m_transaction->is_frozen());
    return result;
}
...
{
    TESSERA_ASSERT(is_frozen());
    TESSERA_ASSERT(m_frozen_version == source.read_transaction_version());
...
KeyPathArray Realm::create_key_path_array(TableKey table_key, size_t num_key_paths, const char** all_key_paths)
{
    ...
    TESSERA_ASSERT(object_schema != m_schema.end());
