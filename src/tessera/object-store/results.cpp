...
#include <tessera/object-store/audit.hpp>
...
size_t Results::do_size()
{
    ...
    TESSERA_COMPILER_HINT_UNREACHABLE();
}
...
const ObjectSchema& Results::get_object_schema() const
{
    ...
    if (!object_schema) {
        TESSERA_ASSERT(m_realm);
        auto it = m_realm->schema().find(get_object_type());
        TESSERA_ASSERT(it != m_realm->schema().end());
...
void Results::ensure_up_to_date(EvaluateMode mode)
{
    if (m_update_policy == UpdatePolicy::Never) {
        TESSERA_ASSERT(m_mode == Mode::TableView);
        return;
    }
...
Mixed Results::get_any(size_t ndx)
{
    ...
    switch (m_mode) {
        ...
        case Mode::Collection:
            ...
            break;
        case Mode::Query:
            TESSERA_UNREACHABLE(); // should always be in TV mode
        case Mode::TableView: {
...
{
    util::CheckedUniqueLock lock(m_mutex);
    TESSERA_ASSERT(m_mode == Mode::Collection);
    ensure_up_to_date();
...
std::pair<StringData, Mixed> Results::get_dictionary_element(size_t ndx)
{
    ...
    TESSERA_ASSERT(typeid(dict) == typeid(Dictionary));
...
void Results::prepare_async(ForCallback force) NO_THREAD_SAFETY_ANALYSIS
{
    ...
    TESSERA_ASSERT(!force || !m_realm->is_frozen());
    if (!force) {
...
{
    TESSERA_ASSERT(!m_realm || !m_realm->is_frozen());
    switch (m_mode) {
        case Mode::Empty:
        case Mode::Table:
            return true;
...
#define TESSERA_RESULTS_TYPE(T)                                                                                        \
    template T Results::get<T>(size_t);                                                                              \
    template util::Optional<T> Results::first<T>();                                                                  \
    template util::Optional<T> Results::last<T>();

TESSERA_RESULTS_TYPE(bool)
TESSERA_RESULTS_TYPE(int64_t)
TESSERA_RESULTS_TYPE(float)
TESSERA_RESULTS_TYPE(double)
TESSERA_RESULTS_TYPE(StringData)
TESSERA_RESULTS_TYPE(BinaryData)
TESSERA_RESULTS_TYPE(Timestamp)
TESSERA_RESULTS_TYPE(ObjectId)
TESSERA_RESULTS_TYPE(Decimal)
TESSERA_RESULTS_TYPE(UUID)
TESSERA_RESULTS_TYPE(Mixed)
TESSERA_RESULTS_TYPE(Obj)
TESSERA_RESULTS_TYPE(util::Optional<bool>)
TESSERA_RESULTS_TYPE(util::Optional<int64_t>)
TESSERA_RESULTS_TYPE(util::Optional<float>)
TESSERA_RESULTS_TYPE(util::Optional<double>)
TESSERA_RESULTS_TYPE(util::Optional<ObjectId>)
TESSERA_RESULTS_TYPE(util::Optional<UUID>)

#undef TESSERA_RESULTS_TYPE
