...
#include <tessera/object-store/impl/external_commit_helper.hpp>
...
#include <tessera/util/fifo_helper.hpp>
...
{
    TESSERA_ASSERT(!version || *version != VersionID());
    if (!config.scheduler)
    ...
    if ((realm = do_get_cached_realm(config))) {
        TESSERA_ASSERT(!version || realm->read_transaction_version() == *version);
        return realm;
    }
...
bool RealmCoordinator::open_db()
{
    ...
    if (m_config.sync_config) {
        TESSERA_ASSERT(m_config.sync_config->user);
    ...
    catch (tessera::FileFormatUpgradeRequired const&) {
...
void RealmCoordinator::advance_schema_cache(uint64_t previous, uint64_t next)
{
    ...
    if (!m_cached_schema)
        return;
    TESSERA_ASSERT(previous <= m_schema_transaction_version_max);
...
#ifdef TESSERA_DEBUG
    std::lock_guard<std::mutex> lock(s_coordinator_mutex);
    TESSERA_ASSERT(s_coordinators_per_path.empty());
#endif
}
...
{
    TESSERA_ASSERT(!m_config.immutable());
    TESSERA_ASSERT(realm.is_in_transaction());
    ...
    VersionID new_version;
    {
        ...
        if (have_notifiers) {
            TESSERA_ASSERT(!m_notifier_skip_version);
            TESSERA_ASSERT(m_notifier_transaction);
            TESSERA_ASSERT_3(m_notifier_transaction->get_transact_stage(), ==, DB::transact_Reading);
            TESSERA_ASSERT_3(m_notifier_transaction->get_version() + 1, ==, new_version.version);
...
void RealmCoordinator::run_async_notifiers()
{
    ...
    if (!m_notifier_transaction) {
        TESSERA_ASSERT(m_notifiers.empty());
        TESSERA_ASSERT(!m_notifier_skip_version);
    ...
    if (skip_version && skip_version->get_version_of_current_transaction() != version) {
        TESSERA_ASSERT(!notifiers.empty());
        TESSERA_ASSERT(version >= skip_version->get_version_of_current_transaction());
