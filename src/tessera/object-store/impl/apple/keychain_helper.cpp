...
#include <tessera/object-store/impl/apple/keychain_helper.hpp>
...
const CFStringRef s_service = CFSTR("io.tessera.sync.keychain");
...
void delete_key(CFStringRef account, CFStringRef service, CFStringRef group)
{
    ...
    TESSERA_ASSERT(status == errSecSuccess || status == errSecItemNotFound);
}
