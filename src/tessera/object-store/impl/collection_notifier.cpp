...
void CollectionNotifier::suppress_next_notification(uint64_t token)
{
    ...
    if (it != end(m_callbacks)) {
        ...
        // this one yet
        TESSERA_ASSERT(it->changes_to_deliver.empty());
        TESSERA_ASSERT(it->accumulated_changes.empty());
        it->skip_next = true;
    }
}
...
{
    TESSERA_ASSERT(m_callbacks.size() > 0);
    ...
    TESSERA_ASSERT(it != end(m_callbacks));
    return it;
}
...
void CollectionNotifier::prepare_handover()
{
    ...
    for (auto& callback : m_callbacks)
        TESSERA_ASSERT(!callback.skip_next);
#endif
}
...
void CollectionNotifier::for_each_callback(Fn&& fn)
{
    ...
    TESSERA_ASSERT_DEBUG(m_callback_count <= m_callbacks.size());
...
void CollectionNotifier::add_changes(CollectionChangeBuilder change)
{
    ...
    for (auto& callback : m_callbacks) {
        if (callback.skip_next) {
            ...
            TESSERA_ASSERT_DEBUG(callback.accumulated_changes.empty());
