...
#include <tessera/object-store/impl/collection_change_builder.hpp>
...
void CollectionChangeBuilder::move(size_t from, size_t to)
{
    ...
    for (auto& move : moves) {
        ...
        TESSERA_ASSERT(!updated_existing_move);
...
#ifdef TESSERA_DEBUG
    for (auto&& move : moves) {
        TESSERA_ASSERT(deletions.contains(move.from));
        TESSERA_ASSERT(insertions.contains(move.to));
    }
#endif
}
...
private:
    ...
    Match find_longest_match(size_t begin1, size_t end1, size_t begin2, size_t end2)
    {
        ...
        auto for_each_b_match = [&](size_t i, auto&& f) {
            ...
            TESSERA_ASSERT(it != end(b) && it->key == ai);
        ...
        for (size_t i = begin1; i < end1; ++i) {
            ...
            for_each_b_match(i, [&](size_t j) {
                ...
                TESSERA_ASSERT(best.i >= begin1 && best.i + best.size <= end1);
                TESSERA_ASSERT(best.j >= begin2 && best.j + best.size <= end2);
            });
        }
        return best;
    }
