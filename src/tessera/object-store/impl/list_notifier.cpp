...
void ListNotifier::run()
{
    ...
    if (m_change.paths.size()) {
        TESSERA_ASSERT(m_collection_parent);
        TESSERA_ASSERT(m_type == PropertyType::Mixed);
        for (auto& p : m_change.paths) {
            if (auto ndx = m_collection_parent->find_index(p); ndx != tessera::not_found)
