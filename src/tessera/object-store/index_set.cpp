...
void ChunkedRangeVector::push_back(value_type value)
{
    if (!empty() && m_data.back().data.size() < max_size) {
        ...
        TESSERA_ASSERT(range.end <= value.first);
...
void ChunkedRangeVector::verify() const noexcept
{
    ...
    size_t prev_end = -1;
    for (auto range : *this) {
        TESSERA_ASSERT(range.first < range.second);
        TESSERA_ASSERT(prev_end == size_t(-1) || range.first > prev_end);
        prev_end = range.second;
    }

    for (auto& chunk : m_data) {
        TESSERA_ASSERT(!chunk.data.empty());
        TESSERA_ASSERT(chunk.data.front().first == chunk.begin);
        TESSERA_ASSERT(chunk.data.back().second == chunk.end);
        TESSERA_ASSERT(chunk.count <= chunk.end - chunk.begin);
        size_t count = 0;
        for (auto range : chunk.data)
            count += range.second - range.first;
        TESSERA_ASSERT(count == chunk.count);
    }
#endif
}
...
size_t IndexSet::count(size_t start_index, size_t end_index) const noexcept
{
    ...
    while (it != end && it.outer()->end <= end_index) {
        TESSERA_ASSERT_DEBUG(it.offset() == 0);
        ret += it.outer()->count;
        it.next_chunk();
    }
...
IndexSet::iterator IndexSet::find(size_t index, iterator begin) noexcept
{
    ...
    TESSERA_ASSERT_DEBUG(inner != it->data.end());
...
void IndexSet::add_shifted_by(IndexSet const& shifted_by, IndexSet const& values)
{
    ...
    for (size_t index : values.as_indexes()) {
        ...
        TESSERA_ASSERT(index >= new_shift);
...
#ifdef TESSERA_DEBUG
    TESSERA_ASSERT((size_t)std::distance(as_indexes().begin(), as_indexes().end()) == expected);
#endif
}
...
void IndexSet::shift_for_insert_at(tessera::IndexSet const& values)
{
...
void IndexSet::remove(tessera::IndexSet const& values)
{
    auto it = begin();
...
{
    TESSERA_ASSERT_DEBUG(!contains(index));
...
IndexSet::iterator IndexSet::do_add(iterator it, size_t index)
{
    ...
    TESSERA_ASSERT(!more_before || index >= std::prev(it)->second);
