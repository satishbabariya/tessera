...
#ifndef TESSERA_SCHEMA_HPP
#define TESSERA_SCHEMA_HPP
...
#define TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE(macro)                                                                     \
...
class SchemaChange {
public:
#define TESSERA_SCHEMA_CHANGE_CONSTRUCTOR(name)                                                                        \
    SchemaChange(schema_change::name value)                                                                          \
        : m_kind(Kind::name)                                                                                         \
    {                                                                                                                \
        name = value;                                                                                                \
    }
    TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE(TESSERA_SCHEMA_CHANGE_CONSTRUCTOR)
#undef TESSERA_SCHEMA_CHANGE_CONSTRUCTOR

    template <typename Visitor>
    auto visit(Visitor&& visitor) const
    {
        switch (m_kind) {
#define TESSERA_SWITCH_CASE(name)                                                                                      \
    case Kind::name:                                                                                                 \
        return visitor(name);
            TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE(TESSERA_SWITCH_CASE)
#undef TESSERA_SWITCH_CASE
        }
...
private:
    enum class Kind {
#define TESSERA_SCHEMA_CHANGE_TYPE(name) name,
        TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE(TESSERA_SCHEMA_CHANGE_TYPE)
#undef TESSERA_SCHEMA_CHANGE_TYPE

    } m_kind;
    union {
#define TESSERA_DEFINE_FIELD(name) schema_change::name name;
        TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE(TESSERA_DEFINE_FIELD)
#undef TESSERA_DEFINE_FIELD
    };
};

#undef TESSERA_FOR_EACH_SCHEMA_CHANGE_TYPE
} // namespace tessera

#endif /* defined(TESSERA_SCHEMA_HPP) */
