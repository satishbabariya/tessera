...
#ifndef TESSERA_OS_OBJECT_ACCESSOR_HPP
#define TESSERA_OS_OBJECT_ACCESSOR_HPP
...
struct ValueUpdater {
    ...
    void operator()(T*)
    {
        ...
        if (!attr_changed) {
            ...
            if constexpr (std::is_same<T, tessera::Mixed>::value) {
                      ...
                      CreatePolicy policy, ObjKey current_obj, Obj* out_row)
{
    ...
    TESSERA_ASSERT(object_schema != realm->schema().end());
                      ...
                      ValueType value, CreatePolicy policy, ObjKey current_obj, Obj* out_row)
{
    ...
    if (object_schema.table_type == ObjectSchema::ObjectType::TopLevelAsymmetric) {
        TESSERA_ASSERT(!policy.update);
        TESSERA_ASSERT(!current_obj);
        TESSERA_ASSERT(object_schema.primary_key_property());
    }
    ...
    if (auto primary_prop = object_schema.primary_key_property()) {
        ...
        if (table->get_primary_key_column() == ColKey{}) {
            TESSERA_ASSERT(realm->is_in_migration());
            if (policy.update) {
...
void List::set(Context& ctx, size_t list_ndx, T&& value, CreatePolicy policy)
{
    ...
    dispatch([&](auto t) {
        ...
        if constexpr (std::is_same_v<U, tessera::Mixed>) {
            if (auto type = _impl::collection_type(new_val)) {
...
#endif // TESSERA_OS_OBJECT_ACCESSOR_HPP
