...
#include <tessera/object-store/sync/impl/sync_client.hpp>
...
{
    TESSERA_ASSERT(m_state != State::Active);
    ...
    if (m_migration_sentinel_query_version) {
        m_flx_subscription_store->get_by_version(*m_migration_sentinel_query_version)
            ...
            .get_async([=, weak_self = weak_from_this()](StatusWith<sync::SubscriptionSet::State> s) {
                ...
                TESSERA_ASSERT(s.get_value() == sync::SubscriptionSet::State::Complete);
...
{
    TESSERA_ASSERT(m_state != State::Dying);
    m_state = State::Dying;
...
{
    TESSERA_ASSERT(m_state != State::Inactive);
...
{
    TESSERA_ASSERT(m_state != State::Paused);
...
{
    TESSERA_ASSERT(m_state != State::WaitingForAccessToken);
...
{
    TESSERA_ASSERT(m_config.sync_config);
    ...
    if (m_migration_store->is_migrated() && m_original_sync_config->flx_sync_requested) {
        ...
        TESSERA_ASSERT(m_migration_sentinel_query_version);
    }
}
    ...
    if (error.status == ErrorCodes::AutoClientResetFailed) {
        ...
        switch (error.server_requests_action) {
            case sync::ProtocolErrorInfo::Action::NoAction:
                TESSERA_UNREACHABLE(); // This is not sent by the MongoDB server
            ...
            case sync::ProtocolErrorInfo::Action::MigrateToFLX:
                ...
                TESSERA_ASSERT(!m_original_sync_config->flx_sync_requested);
                TESSERA_ASSERT(error.migration_query_string && !error.migration_query_string->empty());
...
{
    TESSERA_ASSERT(sync_config->client_resync_mode != ClientResyncMode::Manual);
    ...
    if (sync_config->notify_after_client_reset) {
        config.notify_after_client_reset = [realm_config](VersionID previous_version, bool did_recover) {
            ...
            TESSERA_ASSERT(frozen_before);
            TESSERA_ASSERT(frozen_before->is_frozen());
...
void SyncSession::create_sync_session()
{
    ...
    TESSERA_ASSERT(sync_config.user);
...

    {
        ...
        TESSERA_ASSERT_EX(!sync_route.empty(), "Server URL cannot be empty");
...
sync::SaltedFileIdent SyncSession::get_file_ident() const
{
    ...
    TESSERA_ASSERT(repl);
    TESSERA_ASSERT(dynamic_cast<sync::ClientReplication*>(repl));
...
void SyncSession::update_configuration(SyncConfig new_config)
{
    while (true) {
        ...
        TESSERA_ASSERT(m_state == State::Inactive || m_state == State::Paused);
        TESSERA_ASSERT(!m_session);
        TESSERA_ASSERT(m_config.sync_config->user == new_config.user);
...
{
    TESSERA_ASSERT(!m_flx_subscription_store);
...
void SyncSession::set_write_validator_factory(std::weak_ptr<sync::SubscriptionStore> weak_sub_mgr)
{
    ...
    history.set_write_validator_factory(
        [weak_sub_mgr](Transaction& tr) -> util::UniqueFunction<sync::SyncReplication::WriteValidator> {
            ...
            TESSERA_ASSERT_RELEASE(sub_mgr);
