...
void SyncManager::tear_down_for_testing()
{
...

    {
        ...
        TESSERA_ASSERT_RELEASE(no_sessions);
...
std::shared_ptr<SyncSession> SyncManager::get_session(std::shared_ptr<DB> db, const RealmConfig& config)
{
    ...
    TESSERA_ASSERT_EX(path == config.path, path, config.path);
#else
    auto path = config.path;
#endif
    TESSERA_ASSERT(config.sync_config);
...
{
    TESSERA_ASSERT(!sync_route.empty()); // Cannot be set to empty string
    {
        util::CheckedLockGuard lock(m_mutex);
