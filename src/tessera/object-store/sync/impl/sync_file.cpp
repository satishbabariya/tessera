...
char decoded_char_for(const std::string& percent_encoding, size_t index)
{
    ...
    TESSERA_ASSERT(percent_encoding[index] == '%');
...
{
    TESSERA_ASSERT_DEBUG(template_string.find("XXXXXX") != std::string::npos);
...
{
    TESSERA_ASSERT(absolute_path.length() > 0);
    bool success = true;
    try {
        constexpr bool delete_lockfile = true;
        tessera::DB::delete_files(absolute_path, &success, delete_lockfile);
    }
...
{
    TESSERA_ASSERT(old_path.length() > 0);
    try {
                                              ...
                                              const std::string& realm_file_name, const std::string& partition) const
{
    ...
    // Shorten the Realm path to just `<rootDir>/<hashedAbsolutePath>.tess`
                                            ...
                                            std::optional<std::string> custom_file_name) const
{
    ...
    auto file_name = [&]() -> std::string {
        ...
        if (config.flx_sync_requested) {
            TESSERA_ASSERT_DEBUG(config.partition_value.empty());
