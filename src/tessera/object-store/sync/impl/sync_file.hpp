...
#ifndef TESSERA_OS_SYNC_FILE_HPP
#define TESSERA_OS_SYNC_FILE_HPP
...
// FIXME: Does it make sense to use tessera::StringData arguments for these functions instead of std::string?
...
public:
    ...
    // The default value is `<rootDir>/<appId>/<userId>/<partitionValue>.tess`.
    // If the file cannot be created at this location, for example due to path length restrictions,
    // this function may pass back `<rootDir>/<hashedFileName>.tess`
...
private:
    ...
    static constexpr const char c_metadata_realm[] = "sync_metadata.tess";
    static constexpr const char c_realm_file_suffix[] = ".tess";
...
#endif // TESSERA_OS_SYNC_FILE_HPP
