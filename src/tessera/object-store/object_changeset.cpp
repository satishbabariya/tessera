...
#include <tessera/object-store/object_changeset.hpp>
...
void ObjectChangeSet::verify()
{
    ...
    for (auto it = m_deletions.begin(); it != m_deletions.end(); ++it) {
        TESSERA_ASSERT(m_modifications.find(*it) == m_modifications.end());
        TESSERA_ASSERT(m_insertions.find(*it) == m_insertions.end());
    }
#endif
}
