...
{
    TESSERA_ASSERT(type != PropertyType::Object); // Link columns have to be handled differently
...
ColKey add_column(Group& group, Table& table, Property const& property)
{
    ...
    TESSERA_ASSERT(property.type != PropertyType::LinkingObjects);
...
public:
    ...
    Table& operator()(const ObjectSchema* object_schema)
    {
        ...
        TESSERA_ASSERT(m_current_table);
