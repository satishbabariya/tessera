...
#ifndef TESSERA_OS_DICTIONARY_HPP
#define TESSERA_OS_DICTIONARY_HPP
...
public:
    using Iterator = tessera::Dictionary::Iterator;
...
private:
    ...
    tessera::Dictionary& dict() const noexcept
    {
        TESSERA_ASSERT_DEBUG(dynamic_cast<tessera::Dictionary*>(m_coll_base.get()));
        return static_cast<tessera::Dictionary&>(*m_coll_base);
    }
...
T Dictionary::get(StringData key) const
{
    ...
    if (res.is_null()) {
        if constexpr (std::is_same_v<T, Decimal128>) {
            return Decimal128{tessera::null()};
        }
        else {
            return T{};
        }
...
#endif /* TESSERA_OS_DICTIONARY_HPP */
