...
void DictionaryChangeSet::add(std::vector<Mixed>& arr, const Mixed& key)
{
    ...
    if (key.is_type(type_String)) {
        TESSERA_ASSERT(m_string_store.size() < m_string_store.capacity());
...
Results Dictionary::get_keys() const
{
    ...
    return Results(m_realm,
                   std::make_shared<DictionaryKeyAdapter>(std::dynamic_pointer_cast<tessera::Dictionary>(m_coll_base)));
}
...
public:
    NotificationHandler(tessera::Dictionary& dict, Dictionary::CBFunc cb)
        : m_dict(dict)
        , m_prev_rt(static_cast<Transaction*>(dict.get_table()->get_parent_group())->duplicate())
        , m_prev_dict(std::dynamic_pointer_cast<tessera::Dictionary>(m_prev_rt->import_copy_of(dict)))
        , m_cb(std::move(cb))
    {
    }
    ...
    void operator()(CollectionChangeSet const& c)
    {
        ...
        if (c.collection_root_was_deleted) {
            ...
            TESSERA_ASSERT(m_dict.is_attached());
