...
#ifndef TESSERA_OS_LIST_HPP
#define TESSERA_OS_LIST_HPP
...
private:
    ...
    {
        TESSERA_ASSERT_DEBUG(dynamic_cast<LstBase*>(m_coll_base.get()));
...
auto& List::as() const
{
    TESSERA_ASSERT_DEBUG(dynamic_cast<Lst<T>*>(m_coll_base.get()));
...
{
    TESSERA_ASSERT_DEBUG(dynamic_cast<LnkLst*>(m_coll_base.get()));
...
namespace std {
template <>
struct hash<tessera::List> {
    size_t operator()(tessera::List const&) const;
};
} // namespace std

#endif // TESSERA_OS_LIST_HPP
