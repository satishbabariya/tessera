...
#ifndef TESSERA_QUERY_EXPRESSION_HPP
#define TESSERA_QUERY_EXPRESSION_HPP

#include <tessera/aggregate_ops.hpp>
#include <tessera/array_timestamp.hpp>
#include <tessera/array_binary.hpp>
#include <tessera/array_string.hpp>
#include <tessera/array_backlink.hpp>
#include <tessera/array_list.hpp>
...
#include <tessera/query_value.hpp>
...
#define TESSERA_OLDQUERY_FALLBACK 1
...
public:
    ...
    TESSERA_FORCEINLINE void fun_const(const ValueType& const_value, const ValueBase& right)
    {
        TOperator o;
    ...
    template <class TOperator>
    TESSERA_FORCEINLINE void fun_const(const ValueBase& left, const ValueType& const_value)
    {
        TOperator o;
        // Operate on values one-by-one
        size_t sz = left.size();
    ...
    template <class TOperator>
    TESSERA_FORCEINLINE void fun(const ValueBase& left, const ValueBase& right)
    {
        TOperator o;
        ...
        if (!left.m_from_list && !right.m_from_list) {
            ...
            TESSERA_ASSERT_DEBUG(left.size() > 0);
            ...
            TESSERA_ASSERT_DEBUG(right.size() > 0);
            init(true, left.size());
    ...
    TESSERA_FORCEINLINE static size_t compare(ValueBase& left, ValueBase& right,
                                            ...
                                            util::Optional<ExpressionComparisonType> right_cmp_type)
    {
        ...
        if (!left.m_from_list && !right.m_from_list) {
            ...
            TESSERA_ASSERT_DEBUG(!left_cmp_type || *left_cmp_type == Compare::Any);
            TESSERA_ASSERT_DEBUG(!right_cmp_type || *right_cmp_type == Compare::Any);
        ...
        if (left_size > 2 && right_size > 2) {
            ...
            else if constexpr (tessera::is_any_v<TCond, Greater, GreaterEqual, Less, LessEqual>) {
        ...
        if constexpr (tessera::is_any_v<TCond, BeginsWith, BeginsWithIns, EndsWith, EndsWithIns, Contains, ContainsIns,
                                      Like, LikeIns>) {
...
Query create(L left, const Subexpr2<R>& right)
{
    ...
    constexpr const bool supported_by_old_query_engine =
        ...
        (std::is_same_v<L, R> &&
         tessera::is_any_v<L, double, float, Timestamp, StringData, BinaryData, ObjectId, UUID>);

    if constexpr (TESSERA_OLDQUERY_FALLBACK && supported_by_old_query_engine) {
...
{
#ifdef TESSERA_OLDQUERY_FALLBACK // if not defined, never fallback query_engine; always use query_expression
    ...
    if (left_col && right_col) {
        ...
        TESSERA_ASSERT_DEBUG(t);
        TESSERA_ASSERT_DEBUG(t_right);
...
struct TrueExpression : Expression {
    ...
    {
        TESSERA_ASSERT(start <= end);
        if (start != end)
            return start;

        return tessera::not_found;
    }
...
public:
    ...
    {
        TESSERA_ASSERT_DEBUG(!m_comparison_type || m_from_list);
...
public:
    ...
    {
        TESSERA_ASSERT(m_only_unary_links);
        ObjKey result;
...
public:
    ...
    std::vector<ObjKey> find_all(Mixed value) const final
    {
        ...
        if (m_link_map.get_target_table()->get_primary_key_column() == m_column_key) {
            ...
            TESSERA_ASSERT(index);
            if (value.is_null()) {
                index->find_all(result, tessera::null{});
            }
            else {
                T val = value.get<T>();
...
public:
    ...
    {
        if (links_exist()) {
            TESSERA_ASSERT(!m_leaf);

            if (m_link_map.only_unary_links()) {
                TESSERA_ASSERT(destination.size() == 1);
                TESSERA_ASSERT(!destination.m_from_list);
                ...
                if (link_translation_key) {
                    ...
                    if constexpr (tessera::is_any_v<T, ObjectId, UUID>) {
...
class Columns<BinaryData> : public SimpleQuerySupport<BinaryData> {
    ...
    friend Query operator==(const Columns<BinaryData>& left, tessera::null)
    {
        return create<Equal>(BinaryData(), left);
    }

    friend Query operator==(tessera::null, const Columns<BinaryData>& right)
    {
...
inline std::enable_if_t<!tessera::is_any_v<T, StringData, tessera::null, const char*, std::string>, Query>
...
public:
    ...
    SubColumns<C> column(ColKey column_key) const
    {
        ...
        // having
        TESSERA_ASSERT_DEBUG(!m_comparison_type);
    ...
    void set_cluster(const Cluster* cluster) override
    {
        TESSERA_ASSERT(m_link_map.has_links());
        m_link_map.set_cluster(cluster);
    }
...
public:
    ...
    void evaluate(Subexpr::Index& index, ValueBase& destination) override
    {
        if constexpr (tessera::is_any_v<T, ObjectId, Int, Bool, UUID>) {
    ...
    {
        if constexpr (tessera::is_any_v<T, Int, Float, Double, Decimal128, Mixed>) {
            return max().clone();
        }
        else {
            return {};
    ...
    {
        if constexpr (tessera::is_any_v<T, StringData, BinaryData, Mixed>) {
...
public:
    ...
    {
        TESSERA_ASSERT(m_key_type == type_String);
    }
...
public:
    ...
    void evaluate(Subexpr::Index& index, ValueBase& destination) override
    {
        if (m_dictionary_key_type) {
            ...
            TESSERA_ASSERT_DEBUG(sz > 0 || list_refs.m_from_list);
...
Query compare(const Subexpr2<Link>& left, const Obj& obj)
{
    ...
    if (column) {
        ...
        TESSERA_ASSERT(link_map.get_target_table()->get_key() == obj.get_table()->get_key());
#ifdef TESSERA_OLDQUERY_FALLBACK
...
class Columns : public ObjPropertyExpr<T> {
    constexpr static bool requires_null_column = tessera::is_any_v<T, int64_t, bool>;
...
public:
    ...
    void evaluate_internal(size_t index, ValueBase& destination)
    {
        ...
        if (links_exist()) {
            TESSERA_ASSERT(mpark::holds_alternative<mpark::monostate>(m_leaf));
            if (m_link_map.only_unary_links()) {
            ...
            if constexpr (std::is_same_v<U, int64_t>) {
                ...
                TESSERA_ASSERT_3(ValueBase::chunk_size, ==, 8);
...
public:
    ...
    std::unique_ptr<Subexpr> max_of() override
    {
        if constexpr (tessera::is_any_v<T, Int, Float, Double, Decimal128, Timestamp, Mixed>) {
            return max().clone();
        }
        else {
            return {};
...
public:
    SubQueryCount(const Query& q, const LinkMap& link_map)
        ...
        , m_link_map(link_map)
    {
        TESSERA_ASSERT(q.produces_results_in_table_order());
        TESSERA_ASSERT(m_query.get_table() == m_link_map.get_target_table());
    }
    ...
    std::string description(util::serializer::SerialisationState& state) const override
    {
        TESSERA_ASSERT(m_link_map.get_base_table() != nullptr);
...
public:
    ...
    {
        TESSERA_ASSERT(m_link_map.get_target_table() == m_query.get_table());
    }
...
public:
    ...
    ConstTableRef get_base_table() const override
    {
        ...
        TESSERA_ASSERT(l == nullptr || r == nullptr || l == r);
...
public:
        ...
        , m_right(std::move(right))
    {
        ...
        TESSERA_ASSERT(!(m_left_const_values && m_right_const_values));
    }
    ...
    size_t find_first_with_matches(size_t start, size_t end) const
    {
        ...
        TESSERA_ASSERT(uint64_t(actual_key.value) >= m_cluster->get_offset());
...
#endif // TESSERA_QUERY_EXPRESSION_HPP
