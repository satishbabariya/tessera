...
#ifndef TESSERA_SET_HPP
#define TESSERA_SET_HPP
...
protected:
    ...
    TESSERA_COLD TESSERA_NORETURN void throw_invalid_null()
    {
...
template <class T>
TESSERA_NOINLINE auto Set<T>::find_impl(const T& value) const -> iterator
{
    auto b = this->begin();
...
template <class T>
TESSERA_NOINLINE size_t Set<T>::size() const
{
...
TESSERA_NOINLINE void set_sorted_indices(size_t sz, std::vector<size_t>& indices, bool ascending);
...
{
    TESSERA_ASSERT(!value.is_unresolved());
    update_if_needed();
...
inline util::Optional<Mixed> LnkSet::sum(size_t* return_cnt) const
{
    ...
    TESSERA_TERMINATE("Not implemented");
}
