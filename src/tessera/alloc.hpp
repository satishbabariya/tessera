...
#ifndef TESSERA_ALLOC_HPP
#define TESSERA_ALLOC_HPP
...
#include <tessera/node_header.hpp>
...
#if defined TESSERA_ARCHITECTURE_X86_32 && defined TESSERA_WINDOWS
#define TESSERA_WORKAROUND_MSVC_BUG TESSERA_NOINLINE
#else
#define TESSERA_WORKAROUND_MSVC_BUG
#endif
...
public:
    TESSERA_WORKAROUND_MSVC_BUG inline uint_fast64_t get_content_version() noexcept
    {
...
protected:
    ...
    TESSERA_WORKAROUND_MSVC_BUG inline uint_fast64_t get_instance_version() noexcept
    {
...
{
    // Check that v is divisible by 8 (64-bit aligned).
    TESSERA_ASSERT_DEBUG_EX(v % 8 == 0, v);
...
{
    if (m_is_read_only)
        throw tessera::LogicError(ErrorCodes::WrongTransactionState,
...
inline MemRef Allocator::realloc_(ref_type ref, const char* addr, size_t old_size, size_t new_size)
{
    ...
    if (ref == m_debug_watch)
        TESSERA_TERMINATE("Allocator watch: Ref was reallocated");
#endif
    if (m_is_read_only)
        throw tessera::LogicError(ErrorCodes::WrongTransactionState,
                                "Trying to modify database while in read transaction");
...
#ifdef TESSERA_DEBUG
    if (ref == m_debug_watch)
        TESSERA_TERMINATE("Allocator watch: Ref was freed");
#endif
    TESSERA_ASSERT(!m_is_read_only);
...
{
    TESSERA_ASSERT_DEBUG(ref != 0);
    // TESSERA_ASSERT_DEBUG(m_baseline != 0); // Attached SlabAlloc
...
inline char* Allocator::translate_critical(RefTranslation* ref_translation_ptr, ref_type ref) const noexcept
{
    ...
    if (TESSERA_LIKELY(txl.cookie == 0x1234567890)) {
        ...
        if (TESSERA_LIKELY(offset < lowest_possible_xover_offset)) {
    ...
    tessera::util::terminate("Invalid ref translation entry", __FILE__, __LINE__, txl.cookie, 0x1234567890, ref, idx);
}

inline char* Allocator::translate(ref_type ref) const noexcept
{
    if (auto ptr = m_ref_translation_ptr.load(std::memory_order_acquire); TESSERA_LIKELY(ptr)) {
