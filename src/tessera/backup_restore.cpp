...
{
    return util::format("%1v%2.backup.tess", prefix, version);
}
...
std::string BackupHandler::get_prefix_from_path(const std::string& path)
{
    ...
    // remove a suffix ".tess" but add back the "."
    if (size > 6 && path.substr(size - 6, 6) == ".tess") {
        return path.substr(0, size - 5); // include '.'
    }

    // if no ".tess" suffix, at least ensure a terminating "."
    if (path[size - 1] == '.') {
        return path;
    }
    return path + ".";
