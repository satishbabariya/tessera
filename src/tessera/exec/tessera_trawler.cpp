...
#include <tessera/array_direct.hpp>
...
#include <tessera/collection_parent.hpp>
...
public:
    Node() {}
    Node(tessera::Allocator& alloc, uint64_t ref)
    {
        init(alloc, ref);
    }
    void init(tessera::Allocator& alloc, uint64_t ref);
    bool valid() const
    {
        return m_valid;
    }
    ...
    char* data()
    {
        return tessera::Array::get_data_from_header(m_header);
    }
...
public:
    Array() {}
    Array(tessera::Allocator& alloc, uint64_t ref)
        : Node(alloc, ref)
    {
        init(alloc, ref);
    }
    ...
    {
        return tessera::NodeHeader::get_is_inner_bptree_node_from_header(m_header);
    }
    void init(tessera::Allocator& alloc, uint64_t ref)
    {
        Node::init(alloc, ref);
    ...
    {
        int64_t val = tessera::get_direct(m_data, width(), ndx);
    ...
    std::string get_string(size_t ndx) const
    {
        ...
        if (valid()) {
            ...
            auto w = width();
            TESSERA_ASSERT(ndx * w < sz);
            str = m_data + ndx * w;
        }
        return str;
    }

    uint64_t mem_usage(tessera::Allocator& alloc) const
    {
        uint64_t mem = 0;
...
private:
    ...
    void _mem_usage(tessera::Allocator& alloc, uint64_t& mem) const
    {
        if (m_has_refs) {
...
public:
    Table() = default;
    Table(tessera::Allocator& alloc, uint64_t ref)
        : Array(alloc, ref)
    {
        if (valid()) {
            ...
            if (spec.size() > 5) {
                ...
                for (size_t spec_ndx = 0; spec_ndx < num_spec_cols; ++spec_ndx) {
                    tessera::ColKey col_key{m_column_colkeys.get_val(spec_ndx)};
            ...
            if (size() > 11) {
                ...
                if (pk_col)
                    m_pk_col = tessera::ColKey(pk_col);
            }
            if (size() > 12) {
                auto flags = get_val(12);
                m_table_type = static_cast<tessera::Table::Type>(flags & 0x3);
            }
        }
    }
    std::string get_column_name(tessera::ColKey col_key) const
    {
        return m_column_names.get_string(m_leaf_ndx2spec_ndx[col_key.get_index().val]);
    }
    void print_columns(const Group&) const;
    size_t get_size(tessera::Allocator& alloc) const
    {
        size_t ret = 0;
        if (m_clusters.valid()) {
            if (m_clusters.is_inner_bptree_node()) {
                ...
                if (uint64_t key_ref = m_clusters.get_ref(0)) {
                    auto header = alloc.translate(tessera::to_ref(key_ref));
                    ret = tessera::NodeHeader::get_size_from_header(header);
                }
                else {
...
private:
    ...
    {
        TESSERA_ASSERT(column_ndx <= m_column_names.size());
        ...
        for (size_t i = 0; i != column_ndx; ++i) {
            auto type = tessera::ColumnType(m_column_types.get_val(i));
            if (type == tessera::col_type_Link) {
                subspec_ndx += 1; // index of dest column
            }
            else if (type == tessera::col_type_BackLink) {
    ...
    Array m_clusters;
    tessera::ColKey m_pk_col;
    tessera::Table::Type m_table_type = tessera::Table::Type::TopLevel;
    std::vector<size_t> m_leaf_ndx2spec_ndx;
};

class Group : public Array {
public:
    Group(tessera::Allocator& alloc, uint64_t ref)
        : Array(alloc, ref)
        , m_alloc(alloc)
    {
        m_valid &= (size() <= 12);
    ...
    std::vector<tessera::BinaryData> get_changesets()
    {
        std::vector<tessera::BinaryData> ret;
        if (int(get_val(7)) == 2) {
...
void Table::print_columns(const Group& group) const
{
    ...
    for (unsigned i = 0; i < m_column_names.size(); i++) {
        ...
        auto attr = tessera::ColumnAttr(m_column_attributes.get_val(i));
        std::string type_str;
        tessera::ColKey col_key;
        if (this->m_column_colkeys.valid()) {
            // core6
            col_key = tessera::ColKey(m_column_colkeys.get_val(i));
        }
        if (type == 13) {
            // type_LinkList == 13
            type = 12;
        ...
        if (type == 12) {
            ...
            if (col_key) {
                // core6
                tessera::TableKey opposite_table_key(uint32_t(m_opposite_table.get_val(col_key.get_index().val)));
            ...
            type_str = get_data_type_name(tessera::DataType(type));
        }
        if (col_key) {
            if (col_key.is_list())
                type_str += "[]";
            ...
            if (col_key.is_dictionary()) {
                auto key_type = tessera::DataType(int(m_column_types.get_val(i)) >> 16);
                type_str = std::string("{") + get_data_type_name(key_type) + ", " + type_str + "}";
            }
        }
        if (attr & tessera::col_attr_Nullable)
            type_str += "?";
        if (attr & tessera::col_attr_Indexed)
            type_str += " (indexed)";
...
void Node::init(tessera::Allocator& alloc, uint64_t ref)
{
    m_ref = ref;
    m_header = alloc.translate(tessera::to_ref(ref));
...
static std::vector<Entry> get_nodes(tessera::Allocator& alloc, uint64_t ref)
{
...
{
    tessera::SlabAlloc::Config config;
...
void RealmFile::memory_leaks()
{
    if (m_group->valid()) {
        ...
        if (nodes.size() > 1) {
            ...
            TESSERA_ASSERT(it->length == m_group->get_file_size());
...
public:
    ...
    bool select_table(tessera::TableKey key)
    {
    ...
    bool insert_group_level_table(tessera::TableKey key)
    {
        std::cout << "Create table: " << m_group->get_table_name(key.value) << std::endl;
        return true;
    }

    bool erase_class(tessera::TableKey)
    {
        return true;
    }

    bool rename_class(tessera::TableKey)
    {
        return true;
    }

    bool create_object(tessera::ObjKey key)
    {
        std::cout << "Create object: " << key << std::endl;
        return true;
    }

    bool remove_object(tessera::ObjKey key)
    {
        std::cout << "Remove object: " << key << std::endl;
        return true;
    }

    bool modify_object(tessera::ColKey col_key, tessera::ObjKey key)
    {
    ...
    bool set_link_type(tessera::ColKey)
    {
        return true;
    }

    bool insert_column(tessera::ColKey col_key)
    {
        std::cout << "Add column: " << m_table->get_column_name(col_key) << std::endl;
        return true;
    }

    bool erase_column(tessera::ColKey)
    {
        return true;
    }

    bool rename_column(tessera::ColKey)
    {
        return true;
    }

    bool select_collection(tessera::ColKey col_key, tessera::ObjKey key, const tessera::StablePath&)
    {
    ...
    bool typed_link_change(tessera::ColKey, tessera::TableKey)
    {
        return true;
    }
