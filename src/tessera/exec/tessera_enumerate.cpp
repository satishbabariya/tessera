...
static void enumerate_strings(tessera::SharedRealm realm, double threshold)
{
    auto& group = realm->read_group();
    auto table_keys = group.get_table_keys();
    for (auto table_key : table_keys) {
        tessera::TableRef t = group.get_table(table_key);
        size_t table_size = t->size();
        tessera::util::format(std::cout, "Begin table '%1' of size %2:\n", t->get_name(), table_size);
        if (table_size == 0)
            continue;
        bool found_str_col = false;
        auto do_convert = [&realm, &t](tessera::ColKey col) {
        ...
        t->for_each_public_column([&](tessera::ColKey col_key) {
            if (col_key.get_type() == tessera::col_type_String && !col_key.is_collection()) {
                found_str_col = true;
                tessera::util::format(std::cout, "\tcolumn '%1' ", t->get_column_name(col_key));
                std::cout << std::flush;
                if (t->is_enumerated(col_key)) {
                    ...
                    std::unique_ptr<tessera::DescriptorOrdering> distinct =
                        std::make_unique<tessera::DescriptorOrdering>();
                    distinct->append_distinct(tessera::DistinctDescriptor({{col_key}}));
                    size_t uniques = t->where().count(*distinct.get());
                    double utilization = uniques / double(table_size);
                    tessera::util::format(std::cout, "contains %1 unique values (%2%%) ", uniques, utilization * 100.0);
                    std::cout << std::flush;
            ...
            return tessera::IteratorControl::AdvanceToNext;
        });
        if (!found_str_col) {
        ...
        try {
            ...
            for (int curr_arg = 1; curr_arg < argc; curr_arg++) {
                if (strcmp(argv[curr_arg], "--key") == 0) {
                    ...
                    tessera::util::format(std::cout, "File name '%1' for threshold %2%%\n", argv[curr_arg], threshold);
                    auto start = std::chrono::steady_clock::now();
                    tessera::Realm::Config config;
                    config.path = argv[curr_arg];
                    if (key_ptr) {
                        config.encryption_key.resize(64);
                        memcpy(&config.encryption_key[0], &key_ptr[0], 64);
                    }
                    tessera::SharedRealm realm;
                    try {
                        realm = tessera::Realm::get_shared_realm(config);
                    }
