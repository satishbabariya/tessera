...
template <typename T>
constexpr bool is_status_or_status_with = std::is_same_v<T, ::tessera::Status> || is_status_with<T>;
 ...
 */
template <typename T>
class TESSERA_NODISCARD StatusWith {
...
public:
    ...
    {
        TESSERA_ASSERT_DEBUG(is_ok());
        TESSERA_ASSERT_RELEASE(m_value);
        return *m_value;
    }
