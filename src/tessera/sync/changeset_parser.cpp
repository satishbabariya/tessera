...
struct State {
    ...
    TESSERA_NORETURN void parser_error(std::string_view complaint); // Throws
    TESSERA_NORETURN void parser_error()
    {
...
struct InstructionBuilder : InstructionHandler {
    ...
    void set_intern_string(uint32_t index, StringBufferRange range) final
    {
        ...
        TESSERA_ASSERT(index == strings.size());
...
{
    T value = 0;
    if (TESSERA_LIKELY(_impl::decode_int(*this, value)))
        return value;
...
StringData State::read_string()
{
    ...
    if (size > tessera::Table::max_string_size)
