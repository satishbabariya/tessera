...
InternString Changeset::intern_string(StringData str)
{
    ...
    TESSERA_ASSERT(m_string_buffer.size() < std::numeric_limits<uint32_t>::max());
    TESSERA_ASSERT(m_strings.size() < std::numeric_limits<uint32_t>::max());
    TESSERA_ASSERT(str.size() < std::numeric_limits<uint32_t>::max());
    ...
    if (path) {
        for (auto& element : *path) {
            if (auto subfield = mpark::get_if<InternString>(&element)) {
                ...
                TESSERA_TERMINATE("Invalid path");
            }
        }
    }
    return os;
...
std::ostream& tessera::sync::operator<<(std::ostream& os, const Changeset& changeset)
{
...
void Changeset::verify() const
{
    for (size_t i = 0; i < m_strings.size(); ++i) {
        ...
        TESSERA_ASSERT(range.offset <= m_string_buffer.size());
        TESSERA_ASSERT(range.offset + range.size <= m_string_buffer.size());
    }
    ...
    for (auto instr : *this) {
        ...
        if (auto table_instr = instr->get_if<Instruction::TableInstruction>()) {
            ...
            if (auto object_instr = instr->get_if<Instruction::ObjectInstruction>()) {
                ...
                mpark::visit(util::overload{
                                 [&](const Instruction::AddTable::TopLevelTable& spec) {
                                     TESSERA_ASSERT(is_valid_key_type(spec.pk_type));
            ...
            TESSERA_TERMINATE("Corrupt instruction type");
        }
    }
}
