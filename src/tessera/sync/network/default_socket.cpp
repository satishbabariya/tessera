...
namespace tessera::sync::websocket {
...
{
    TESSERA_ASSERT(i < endpoints.size());
...
{
    TESSERA_ASSERT(m_logger_ptr);                     // Make sure the logger is valid
...
DefaultSocketProvider::~DefaultSocketProvider()
{
    ...
    TESSERA_ASSERT(m_state == State::Stopped);
}
...
void DefaultSocketProvider::event_loop()
{
    ...
    {
        util::CheckedLockGuard lock(m_mutex);
        TESSERA_ASSERT(m_state == State::Starting);
    }
