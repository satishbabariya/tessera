...
#define TESSERA_HAVE_KEYCHAIN_APIS (TARGET_OS_MAC && !TARGET_OS_IPHONE)
...
namespace tessera::sync::network::ssl {
...
template <>
class is_error_condition_enum<tessera::sync::network::ssl::Errors> {
public:
...
public:
    ...
    Want initiate()
    {
        TESSERA_ASSERT(this == m_stream->m_tcp_socket.m_read_oper.get());
...
public:
    ...
    {
        TESSERA_ASSERT(is_complete() || is_canceled());
        bool orphaned = !m_stream;
...
public:
    ...
    Want initiate()
    {
        TESSERA_ASSERT(this == m_stream->m_tcp_socket.m_write_oper.get());
        TESSERA_ASSERT(!is_complete());
        m_stream->m_tcp_socket.m_desc.ensure_nonblocking_mode(); // Throws
        return advance();
    }
...
std::size_t Stream::ssl_perform(Oper oper, std::error_code& ec, Want& want) noexcept
{
    ...
    TESSERA_ASSERT((ret > 0) == (ssl_error == SSL_ERROR_NONE));

    TESSERA_ASSERT(!m_bio_error_code || ssl_error == SSL_ERROR_SYSCALL);
    ...
    switch (ssl_error) {
        ...
        case SSL_ERROR_SYSCALL:
            if (TESSERA_UNLIKELY(sys_error != 0)) {
                ec = util::make_basic_system_error_code(sys_error);
            }
            else if (TESSERA_UNLIKELY(m_bio_error_code)) {
                ec = m_bio_error_code;
            }
            else if (ret == 0) {
...
std::size_t Stream::ssl_perform(Oper oper, std::error_code& ec, Want& want) noexcept
{
    ...
    if (TESSERA_UNLIKELY(m_mock_ssl_perform_error)) {
    ...
    if (result == errSSLWouldBlock) {
        TESSERA_ASSERT(blocking_want != Want::nothing);
        ec = std::error_code();
        want = blocking_want;
        return n;
    }
