...
#include <tessera/util/cf_str.hpp>
...
using namespace tessera::sync::network::ssl;


namespace {

#if TESSERA_INCLUDE_CERTS

const char* root_certs[] = {
#include <tessera/sync/noinst/root_certs.hpp>
};
...
#endif // TESSERA_INCLUDE_CERTS

#if TESSERA_HAVE_OPENSSL && _WIN32
...
private:
    ...
    {
        TESSERA_ASSERT_RELEASE(type == X509_LU_X509);
...
#endif // TESSERA_HAVE_OPENSSL && _WIN32

#if TESSERA_HAVE_OPENSSL && (OPENSSL_VERSION_NUMBER < 0x10100000L || defined(LIBRESSL_VERSION_NUMBER))
...
#endif // TESSERA_HAVE_OPENSSL && (OPENSSL_VERSION_NUMBER < 0x10100000L || defined(LIBRESSL_VERSION_NUMBER))
...
{
    TESSERA_ASSERT(!m_tcp_socket.m_read_oper || !m_tcp_socket.m_read_oper->in_use());
    TESSERA_ASSERT(!m_tcp_socket.m_write_oper || !m_tcp_socket.m_write_oper->in_use());
    m_tcp_socket.m_desc.ensure_blocking_mode(); // Throws
    Want want = Want::nothing;
    ssl_handshake(ec, want);
    TESSERA_ASSERT(want == Want::nothing);
    return ec;
}
...
{
#if TESSERA_USE_SYSTEM_OPENSSL_PATHS
    ERR_clear_error();
...
public:
    ...
    BioMethod()
    {
        const char* name = "tessera::util::Stream::BioMethod";
...
#pragma GCC diagnostic pop
...
#if OPENSSL_VERSION_NUMBER >= 0x10002000L && !defined(LIBRESSL_VERSION_NUMBER)
    {
        ...
        if (ret == 0) {
            ...
            TESSERA_ASSERT_DEBUG(sys_error != 0);
...
int Stream::verify_callback_using_delegate(int preverify_ok, X509_STORE_CTX* ctx) noexcept
{
    ...
    TESSERA_ASSERT(stream->m_ssl_verify_callback);
...
int Stream::bio_write(BIO* bio, const char* data, int size) noexcept
{
    ...
    if (ec) {
        if (TESSERA_UNLIKELY(ec != error::resource_unavailable_try_again)) {
...
int Stream::bio_read(BIO* bio, char* buffer, int size) noexcept
{
    ...
    BIO_clear_retry_flags(bio);
    if (ec) {
        if (TESSERA_UNLIKELY(ec == MiscExtErrors::end_of_input)) {
...
#endif
        default:
            TESSERA_ASSERT_EX(false, "Got BIO_ctrl with unknown command %d", cmd);
    }
    return 0;
}
...
#endif // TESSERA_HAVE_KEYCHAIN_APIS
...
std::pair<OSStatus, std::size_t> Stream::do_ssl_write(const char* data, std::size_t size) noexcept
{
    ...
    TESSERA_ASSERT(size >= m_num_partially_written_bytes);
    ...
    if (result != noErr) {
        ...
        if (TESSERA_LIKELY(result == errSSLWouldBlock)) {
...
#else // !TESSERA_HAVE_OPENSSL && !TESSERA_HAVE_SECURE_TRANSPORT
