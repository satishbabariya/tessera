...
// Linux epoll
#if defined(TESSERA_USE_EPOLL) && !TESSERA_ANDROID
#define TESSERA_NETWORK_USE_EPOLL 1
#else
#define TESSERA_NETWORK_USE_EPOLL 0
#endif
...
#if !defined(TESSERA_HAVE_KQUEUE)
#if !defined(TESSERA_DISABLE_UTIL_NETWORK_KQUEUE)
#define TESSERA_HAVE_KQUEUE 1
#endif
#endif
#endif
#if !defined(TESSERA_HAVE_KQUEUE)
#define TESSERA_HAVE_KQUEUE 0
#endif
...
public:
    ...
    /// `TESSERA_UTIL_NETWORK_EVENT_LOOP_METRICS` was defined during
...
template <>
class is_error_code_enum<tessera::sync::network::ResolveErrors> {
public:
...
inline void Service::Descriptor::ensure_blocking_mode()
{
    ...
    if (TESSERA_UNLIKELY(!m_in_blocking_mode)) {
        bool value = false;
...
public:
    ...
    void recycle() noexcept override final
    {
        TESSERA_ASSERT(in_use());
        TESSERA_ASSERT(!m_service);
...
public:
    ...
    void orphan() noexcept override final
    {
        TESSERA_ASSERT(false); // Never called
    }
...
public:
    ...
    {
        TESSERA_ASSERT(!stream.lowest_layer().m_read_oper || !stream.lowest_layer().m_read_oper->in_use());
    ...
    {
        TESSERA_ASSERT(!stream.lowest_layer().m_write_oper || !stream.lowest_layer().m_write_oper->in_use());
        stream.lowest_layer().m_desc.ensure_blocking_mode(); // Throws
...
public:
    ...
    {
        auto& s = *this;
        TESSERA_ASSERT(this == s.m_stream->lowest_layer().m_read_oper.get());
        TESSERA_ASSERT(!s.is_complete());
        TESSERA_ASSERT(s.m_curr <= s.m_end);
        Want want = Want::nothing;
        if (TESSERA_UNLIKELY(s.m_curr == s.m_end)) {
            ...
            if (want == Want::nothing) {
                if (TESSERA_UNLIKELY(s.m_error_code)) {
    ...
    Want advance() noexcept override final
    {
        auto& s = *this;
        TESSERA_ASSERT(!s.is_complete());
        TESSERA_ASSERT(!s.is_canceled());
        TESSERA_ASSERT(!s.m_error_code);
        TESSERA_ASSERT(s.m_curr < s.m_end);
        TESSERA_ASSERT(!s.m_is_read_some || s.m_curr == m_begin);
        for (;;) {
            ...
            TESSERA_ASSERT(n > 0 || s.m_error_code || want != Want::nothing); // No busy loop, please
...
public:
    ...
    {
        auto& s = *this;
        TESSERA_ASSERT(this == s.m_stream->lowest_layer().m_write_oper.get());
        TESSERA_ASSERT(!s.is_complete());
        TESSERA_ASSERT(s.m_curr <= s.m_end);
        Want want = Want::nothing;
        if (TESSERA_UNLIKELY(s.m_curr == s.m_end)) {
    ...
    Want advance() noexcept override final
    {
        ...
        TESSERA_ASSERT(!s.m_error_code);
        TESSERA_ASSERT(s.m_curr < s.m_end);
        TESSERA_ASSERT(!s.m_is_write_some || s.m_curr == s.m_begin);
        for (;;) {
...
public:
    ...
    Want advance() noexcept override final
    {
        ...
        TESSERA_ASSERT(!s.is_canceled());
        TESSERA_ASSERT(!s.m_error_code);
        TESSERA_ASSERT(s.m_read_ahead_buffer.empty());
        TESSERA_ASSERT(s.m_curr < s.m_end);
        for (;;) {
            ...
            TESSERA_ASSERT(nonempty || s.m_error_code || want != Want::nothing); // No busy loop, please
...
public:
    ...
    {
        auto& s = *this;
        TESSERA_ASSERT(s.is_complete() || s.is_canceled());
        TESSERA_ASSERT(s.is_complete() ==
                     (s.m_error_code || s.m_curr == s.m_end || (s.m_is_read_some && s.m_curr != s.m_begin)));
        TESSERA_ASSERT(s.m_curr >= s.m_begin);
...
public:
    ...
    {
        auto& s = *this;
        TESSERA_ASSERT(s.is_complete() || (s.is_canceled() && !s.m_error_code));
        TESSERA_ASSERT(s.is_canceled() || s.m_error_code ||
...
std::unique_ptr<Oper, Service::LendersOperDeleter> Service::alloc(OwnersOperPtr& owners_ptr, Args&&... args)
{
    ...
    std::size_t size;
    if (TESSERA_LIKELY(addr)) {
        TESSERA_ASSERT(!owners_ptr->in_use());
        ...
        TESSERA_ASSERT(dynamic_cast<UnusedOper*>(owners_ptr.get()));
        static_cast<UnusedOper*>(owners_ptr.get())->UnusedOper::~UnusedOper();
        if (TESSERA_UNLIKELY(size < sizeof(Oper))) {
            owners_ptr.release();
            delete[] static_cast<char*>(addr);
            goto no_object;
        }
...
{
    TESSERA_ASSERT(m_in_use);
    TESSERA_ASSERT(!m_canceled);
    m_canceled = true;
}
...
{
    TESSERA_ASSERT(!m_complete);
    TESSERA_ASSERT(!value || m_in_use);
    m_complete = value;
}
...
public:
    ...
    void recycle_and_execute() override final
    {
        TESSERA_ASSERT(is_complete() || (is_canceled() && !m_error_code));
        TESSERA_ASSERT(is_canceled() || m_error_code || !m_endpoints.empty());
        bool orphaned = !m_resolver;
...
inline void SocketBase::Option<T, opt, U>::get(const SocketBase& sock, std::error_code& ec)
{
    ...
    if (!ec) {
        TESSERA_ASSERT(value_size == sizeof value);
        m_value = T(value);
    }
}
...
public:
    ...
    {
        TESSERA_ASSERT(this == m_socket->m_write_oper.get());
    ...
    Want advance() noexcept override final
    {
        TESSERA_ASSERT(!is_complete());
        TESSERA_ASSERT(!is_canceled());
        TESSERA_ASSERT(!m_error_code);
...
public:
    ...
    Want initiate()
    {
        TESSERA_ASSERT(this == m_acceptor->m_read_oper.get());
        TESSERA_ASSERT(!is_complete());
    ...
    Want advance() noexcept override final
    {
        ...
        TESSERA_ASSERT(!is_canceled());
        TESSERA_ASSERT(!m_error_code);
        TESSERA_ASSERT(!m_socket.is_open());
...
public:
    ...
    {
        TESSERA_ASSERT(is_complete() || (is_canceled() && !m_error_code));
        TESSERA_ASSERT(is_canceled() || m_error_code || m_socket.is_open());
        bool orphaned = !m_acceptor;
        std::error_code ec = m_error_code;
        if (is_canceled())
            ec = util::error::operation_aborted;
...
{
    TESSERA_ASSERT(!m_read_oper || !m_read_oper->in_use());
    if (TESSERA_UNLIKELY(socket.is_open()))
...
inline void Acceptor::async_accept(Socket& sock, Endpoint* ep, H&& handler)
{
    if (TESSERA_UNLIKELY(sock.is_open()))
        throw util::runtime_error("Socket is already open");
...
inline bool ReadAheadBuffer::refill_async(S& stream, std::error_code& ec, Want& want) noexcept
{
    ...
    if (n == 0)
        return false;
    TESSERA_ASSERT(!ec || ec == util::MiscExtErrors::end_of_input);
    TESSERA_ASSERT(n <= size);
    m_begin = m_buffer.get();
    m_end = m_begin + n;
    return true;
