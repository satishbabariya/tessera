...
struct MajorSide : Side {
    ...
    {
        TESSERA_ASSERT(position >= m_changeset->begin());
        TESSERA_ASSERT(position != m_changeset->end());
        m_position = position;
        skip_tombstones();
        TESSERA_ASSERT(position != m_changeset->end());
    ...
    {
        TESSERA_ASSERT(m_position != m_changeset->end());
        do {
            ++m_position;
        } while (m_position != m_changeset->end() && !*m_position);
    }
...
struct MinorSide : Side {
    ...
    {
        if (TESSERA_LIKELY(m_position != end())) {
    ...
    TESSERA_NOINLINE void skip_tombstones_slow() noexcept
    {
    ...
    Instruction& get() noexcept final
    {
        ...
        TESSERA_ASSERT(instr != nullptr);
        return *instr;
    }

    void init_with_instruction(Position position)
    {
        // TESSERA_ASSERT(position >= Position(m_conflict_ranges));
        TESSERA_ASSERT(position != end());
        m_position = position;
        update_changeset_pointer();
        skip_tombstones();
        TESSERA_ASSERT(position != end());
...
#if defined(TESSERA_DEBUG) // LCOV_EXCL_START Debug utilities
...
#endif // LCOV_EXCL_STOP TESSERA_DEBUG
...
struct TransformerImpl {
    ...
    _impl::ChangesetIndex::Ranges* get_conflict_ranges_for_instruction(const Instruction& instr)
    {
        ...
        if (_impl::is_schema_change(instr)) {
            ...
            if (!ranges->empty()) {
#if TESSERA_DEBUG // LCOV_EXCL_START
                if (m_trace) {
                    std::cerr << TERM_RED << "Conflict group: Everything (due to schema change)\n" << TERM_RESET;
                }
#endif // TESSERA_DEBUG LCOV_EXCL_STOP
            }
            return ranges;
        }
        else {
            ...
            TESSERA_ASSERT(num_major_ids <= 2);
            TESSERA_ASSERT(num_major_ids >= 1);
#if TESSERA_DEBUG // LCOV_EXCL_START
            if (m_trace) {
            ...
            if (num_major_ids == 2) {
                ...
                // two object IDs.
                TESSERA_ASSERT(ranges == index.get_modifications_for_object(major_ids[1]));
            }
            return ranges;
        }
    }
    ...
    {
        TESSERA_ASSERT(*m_major_side.m_position); // cannot prepend a tombstone
    ...
    {
        TESSERA_ASSERT(*m_minor_side.m_position); // cannot prepend a tombstone
        ...
        TESSERA_ASSERT(m_minor_end == m_minor_side.end());
    }
    ...
    void transform_prepended_major(size_t num_prepended)
    {
        ...
        TESSERA_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());

#if defined(TESSERA_DEBUG) // LCOV_EXCL_START
        if (m_trace) {
        ...
        for (size_t i = 0; i < num_prepended; ++i) {
            ...
            TESSERA_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());
            m_major_side.init_with_instruction(m_major_side.m_position);
            TESSERA_ASSERT(!m_major_side.was_discarded);
            TESSERA_ASSERT(m_major_side.m_position != m_major_side.m_changeset->end());
            transform_major();
            if (!m_major_side.was_discarded) {
                // Discarding an instruction moves to the next.
...
#if defined(TESSERA_DEBUG) // LCOV_EXCL_START Debug tracing
...
TESSERA_NORETURN void throw_bad_merge(std::string msg)
{
    throw sync::TransformError{std::move(msg)};
}

template <class... Params>
TESSERA_NORETURN void bad_merge(const char* msg, Params&&... params)
{
    throw_bad_merge(util::format(msg, std::forward<Params>(params)...));
}

TESSERA_NORETURN void bad_merge(Side& side, Instruction::PathInstruction instr, const std::string& msg)
{
    std::stringstream ss;
...
struct MergeUtils {
    ...
    {
        TESSERA_ASSERT(left.path.size() != 0);
        TESSERA_ASSERT(left.path.size() < right.path.size());
        TESSERA_ASSERT(mpark::holds_alternative<uint32_t>(left.path.back()));
...
DEFINE_MERGE(Instruction::ArrayInsert, Instruction::Update)
{
    if (same_container(left, right)) {
        TESSERA_ASSERT(right.is_array_update());
...
DEFINE_MERGE(Instruction::Clear, Instruction::Update)
{
    ...
    // The two instructions are at the same level of nesting.
    if (same_path(left, right)) {
        TESSERA_ASSERT(right.value.type != Type::Set);
...
DEFINE_NESTED_MERGE(Instruction::ArrayErase)
{
    if (is_prefix_of(outer, inner)) {
        ...
        if (outer.index() < index) {
            ...
            TESSERA_ASSERT(index != outer.index());
        }
    }
}
...
void TransformerImpl::merge_instructions(MajorSide& their_side, MinorSide& our_side)
{
    ...
    if (their_side.get().get_if<Instruction::Update>()) {
        TESSERA_ASSERT(their_side.m_path_len > 2);
    }
    if (our_side.get().get_if<Instruction::Update>()) {
        TESSERA_ASSERT(our_side.m_path_len > 2);
    }
...
{
    TESSERA_ASSERT(our_changesets.size() != 0);
    TESSERA_ASSERT(their_changesets.size() != 0);
    bool trace = false;
#if TESSERA_DEBUG && !TESSERA_UWP
...
size_t Transformer::transform_remote_changesets(TransformHistory& history, file_ident_type local_file_ident,
                                                ...
                                                util::Logger& logger)
{
    TESSERA_ASSERT(local_file_ident != 0);
...
void parse_remote_changeset(const RemoteChangeset& remote_changeset, Changeset& parsed_changeset)
{
    ...
    // origin.
    TESSERA_ASSERT(remote_changeset.origin_file_ident != 0);
    TESSERA_ASSERT(remote_changeset.remote_version != 0);
