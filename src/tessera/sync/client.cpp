...
ClientImpl::~ClientImpl()
{
    ...
    // are abandoned.
    TESSERA_ASSERT(m_stopped);
    TESSERA_ASSERT(m_unactualized_session_wrappers.empty());
    TESSERA_ASSERT(m_abandoned_session_wrappers.empty());
}
    ...
    post([this] {
        for (auto& p : m_server_slots) {
            ...
            if (m_one_connection_per_session) {
                TESSERA_ASSERT(!slot.connection);
                ...
                TESSERA_ASSERT(slot.alt_connections.empty());
                if (slot.connection) {
...
void ClientImpl::register_unactualized_session_wrapper(SessionWrapper* wrapper)
{
    // Thread safety required.
    {
        ...
        TESSERA_ASSERT(m_actualize_and_finalize);
                                                   ...
                                                   Optional<ProxyConfig> proxy_config, bool& was_created)
{
    ...
    if (server_slot.connection && !m_one_connection_per_session && !proxy_config) {
        ...
        TESSERA_ASSERT(server_slot.alt_connections.empty());
        return *server_slot.connection;
    }

    // Create a new connection
    TESSERA_ASSERT(!server_slot.connection);
...
void ClientImpl::remove_connection(ClientImpl::Connection& conn) noexcept
{
    ...
    TESSERA_ASSERT(i != m_server_slots.end()); // Must be found
    ServerSlot& server_slot = i->second;
    if (!m_one_connection_per_session) {
        TESSERA_ASSERT(server_slot.alt_connections.empty());
        TESSERA_ASSERT(&*server_slot.connection == &conn);
        ...
        TESSERA_ASSERT(j != server_slot.alt_connections.end()); // Must be found
        TESSERA_ASSERT(&*j->second == &conn);
    ...
    {
        util::CheckedLockGuard lk(m_drain_mutex);
        TESSERA_ASSERT(m_num_connections);
...
const std::string& SessionImpl::get_virt_path() const noexcept
{
    ...
    TESSERA_ASSERT_EX(m_state == State::Active || m_state == State::Unactivated, m_state);
...
void SessionImpl::process_pending_flx_bootstrap()
{
    ...
    while (bootstrap_store->has_pending()) {
        ...
        history.integrate_server_changesets(
            *pending_batch.progress, 1.0, pending_batch.changesets, new_version, batch_state, logger, transact,
            [&](const Transaction& tr, util::Span<Changeset> changesets_applied) {
                TESSERA_ASSERT_3(changesets_applied.size(), <=, pending_batch.changesets.size());
        ...
        auto action = call_debug_hook(SyncClientHookEvent::DownloadMessageIntegrated, progress, query_version,
                                      batch_state, pending_batch.changesets.size());
        TESSERA_ASSERT_EX(action == SyncClientHookAction::NoAction, action);
    ...
    TESSERA_ASSERT_3(query_version, !=, -1);
    ...
    TESSERA_ASSERT_EX(action == SyncClientHookAction::NoAction || action == SyncClientHookAction::EarlyReturn, action);
}
...
SubscriptionStore* SessionImpl::get_flx_subscription_store()
{
    ...
    TESSERA_ASSERT_EX(m_state == State::Active, m_state);
...
SyncClientHookAction SessionImpl::call_debug_hook(const SyncClientHookData& data)
{
    ...
    switch (action) {
        case tessera::SyncClientHookAction::SuspendWithRetryableError: {
            ...
            TESSERA_ASSERT_EX(err_processing_err.is_ok(), err_processing_err);
            return SyncClientHookAction::EarlyReturn;
        }
        case tessera::SyncClientHookAction::TriggerReconnect: {
...
{
    if (TESSERA_LIKELY(!m_wrapper.m_debug_hook)) {
        return SyncClientHookAction::NoAction;
    }
    if (TESSERA_UNLIKELY(m_state != State::Active)) {
        return SyncClientHookAction::NoAction;
    }
...
{
    TESSERA_ASSERT_EX(m_state == State::Unactivated || m_state == State::Active, m_state);
...
{
    TESSERA_ASSERT(m_db);
    TESSERA_ASSERT(m_db->get_replication());
    TESSERA_ASSERT(dynamic_cast<ClientReplication*>(m_db->get_replication()));
...
SessionWrapper::~SessionWrapper() noexcept
{
    ...
    TESSERA_ASSERT(m_actualized);
    TESSERA_ASSERT(m_abandoned);
    TESSERA_ASSERT(m_finalized);
    TESSERA_ASSERT(m_closed);
    TESSERA_ASSERT(!m_db);
}
...
void SessionWrapper::on_commit(version_type new_version)
{
    ...
    m_client.post([self = util::bind_ptr{this}, new_version] {
        TESSERA_ASSERT(self->m_actualized);
        if (TESSERA_UNLIKELY(!self->m_sess))
...
void SessionWrapper::cancel_reconnect_delay()
{
    ...
    m_client.post([self = util::bind_ptr{this}] {
        TESSERA_ASSERT(self->m_actualized);
        if (TESSERA_UNLIKELY(self->m_closed)) {
            return;
        }
...
{
    TESSERA_ASSERT(upload_completion || download_completion);
...
void SessionWrapper::report_progress(ReportedProgress& p, DownloadableProgress downloadable)
{
    ...
    auto calculate_progress = [](uint64_t transferred, uint64_t transferable, uint64_t final_transferred) {
        TESSERA_ASSERT_DEBUG_EX(final_transferred <= transferred, final_transferred, transferred, transferable);
        TESSERA_ASSERT_DEBUG_EX(transferred <= transferable, final_transferred, transferred, transferable);
...
std::ostream& operator<<(std::ostream& os, ProxyConfig::Type proxyType)
{
    ...
    TESSERA_TERMINATE("Invalid Proxy Type object.");
}
