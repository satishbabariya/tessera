...
#define TESSERA_DEFINE_REFLECTOR_VISITOR(X) void operator()(const Instruction::X&) const;
    TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_DEFINE_REFLECTOR_VISITOR)
#undef TESSERA_DEFINE_REFLECTOR_VISITOR
};
...
{
    TESSERA_ASSERT(string.value < m_strings.size());
    return m_strings[string.value];
}
...
inline Changeset::iterator Changeset::erase_stable(const_iterator cpos)
{
    ...
    TESSERA_ASSERT(pos.m_inner >= begin);
    TESSERA_ASSERT(pos.m_inner < end);
...
template <bool is_const>
struct iterator_traits<tessera::sync::Changeset::IteratorImpl<is_const>> {
    using difference_type = std::ptrdiff_t;
...
#endif // TESSERA_SYNC_CHANGESET_HPP
