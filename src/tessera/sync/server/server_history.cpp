...
#include <tessera/util/value_reset_guard.hpp>
                               ...
                               version_type& partial_progress_reference_version) const
{
    ...
    if (is_initiated_as_partial_view) {
        ...
        TESSERA_ASSERT(partial_file_ident != 0);
...
void ServerHistory::allocate_file_identifiers(FileIdentAllocSlots& slots, VersionInfo& version_info)
{
    ...
    if (TESSERA_UNLIKELY(m_acc->upstream_status.is_attached())) {
...
{
    TESSERA_ASSERT(!integratable_changesets.empty());
    ...
    for (;;) {
        ...
        if (TESSERA_UNLIKELY(!anything_to_do))
            return false;
        ...
        try {
            ...
            for (file_ident_type client_file_ident : client_file_order) {
                ...
                TESSERA_ASSERT(client_file_ident != g_root_node_file_ident);
                TESSERA_ASSERT(client_file_ident != m_local_file_ident);
                bool excluded =
                    (result.excluded_client_files.find(client_file_ident) != result.excluded_client_files.end());
                if (TESSERA_UNLIKELY(excluded))
                    continue;
                ...
                for (const IntegratableChangeset& ic : list.changesets) {
                    TESSERA_ASSERT(ic.client_file_ident == client_file_ident);
                    ...
                    if (origin_file_ident != 0) {
                        ...
                        if (TESSERA_UNLIKELY(origin_file_ident <= g_root_node_file_ident))
                            goto error;
                        if (TESSERA_UNLIKELY(std::uint_fast64_t(origin_file_ident) >= m_num_client_files))
                            goto error;
                        ...
                        if (TESSERA_UNLIKELY(proxy_file_ident != ic.client_file_ident))
                            goto error;
                    }
    ...
    error:
        TESSERA_ASSERT(current_client_file_ident != 0);
...
{
    TESSERA_ASSERT(!m_acc->upstream_status.is_attached());
                                            ...
                                            ClientType client_type, salt_type& file_ident_salt)
{
    TESSERA_ASSERT(m_acc->upstream_status.is_attached());
    static_assert(g_root_node_file_ident == 1, "");
    if (TESSERA_UNLIKELY(file_ident < 2))
        return false;
    std::size_t max = std::numeric_limits<std::size_t>::max();
    if (TESSERA_UNLIKELY(std::uint_fast64_t(file_ident) > max))
...
{
    TESSERA_ASSERT(file_index >= m_num_client_files);
    TESSERA_ASSERT(proxy_file_ident == 0 || is_valid_proxy_file_ident(proxy_file_ident));
...
bool ServerHistory::ensure_upstream_file_ident(file_ident_type file_ident)
{
    ...
    if (TESSERA_UNLIKELY(std::uint_fast64_t(file_ident) > max))
        throw util::overflow_error{"Client file index"};
    std::size_t file_index = std::size_t(file_ident);
    if (TESSERA_LIKELY(file_index < m_num_client_files)) {
        auto client_type = m_acc->cf_client_types.get(file_index);
        if (TESSERA_UNLIKELY(client_type != int(ClientType::upstream)))
            return false;
        TESSERA_ASSERT(m_acc->cf_ident_salts.get(file_index) == 0);
        TESSERA_ASSERT(m_acc->cf_proxy_files.get(file_index) == 0);
        return true;
    }
    do
        add_client_file(0, 0, ClientType::upstream); // Throws
                                    ...
                                    ClientType client_type)
{
    switch (client_type) {
        ...
        case ClientType::self:
            TESSERA_ASSERT(file_ident_salt == 0);
            TESSERA_ASSERT(proxy_file_ident == 0);
            break;
    ...
    m_acc->cf_ident_salts.insert(tessera::npos, std::int_fast64_t(file_ident_salt));  // Throws
    m_acc->cf_client_versions.insert(tessera::npos, client_version);                  // Throws
    m_acc->cf_rh_base_versions.insert(tessera::npos, recip_hist_base_version);        // Throws
    m_acc->cf_recip_hist_refs.insert(tessera::npos, recip_hist_ref);                  // Throws
    m_acc->cf_proxy_files.insert(tessera::npos, std::int_fast64_t(proxy_file_ident)); // Throws
    m_acc->cf_client_types.insert(tessera::npos, std::int_fast64_t(client_type));     // Throws
    m_acc->cf_last_seen_timestamps.insert(tessera::npos, last_seen_timestamp);        // Throws
    m_acc->cf_locked_server_versions.insert(tessera::npos, locked_server_version);    // Throws
...
{
    TESSERA_ASSERT(is_direct_client(client_type));
    TESSERA_ASSERT(client_type != ClientType::legacy);
    ...
    if (TESSERA_UNLIKELY(expired_due_to_compaction)) {
        logger.debug("Client expired because history has been compacted");
        return BootstrapError::client_file_expired;
    }

    TESSERA_ASSERT_RELEASE(recip_hist_base_version >= m_history_base_version);
...
{
    TESSERA_ASSERT(client_file_ident != 0);
    TESSERA_ASSERT(download_progress.server_version <= end_version);
    ...
    TESSERA_ASSERT(download_progress.server_version >= m_history_base_version);

    std::size_t client_file_index = std::size_t(client_file_ident);
    {
        auto client_type = ClientType(m_acc->cf_client_types.get(client_file_index));
        TESSERA_ASSERT_RELEASE(is_direct_client(client_type));
        std::int_fast64_t last_seen_timestamp = m_acc->cf_last_seen_timestamps.get(client_file_index);
        bool expired = (last_seen_timestamp == 0);
        if (TESSERA_UNLIKELY(expired))
            return false;
    ...
    if (download_progress_2.server_version > m_history_base_version) {
        ...
        TESSERA_ASSERT(cumulative_byte_size_current_2 >= 0);
    }
    if (m_history_size > 0) {
        std::size_t end_ndx = m_history_size - 1;
        cumulative_byte_size_total_2 = m_acc->sh_cumul_byte_sizes.get(end_ndx);
    }
    TESSERA_ASSERT(cumulative_byte_size_current_2 <= cumulative_byte_size_total_2);
...
void ServerHistory::add_upstream_sync_status()
{
    ...
    TESSERA_ASSERT(!m_acc->upstream_status.is_attached());
    TESSERA_ASSERT(m_local_file_ident == g_root_node_file_ident);
...
std::vector<sync::Changeset> ServerHistory::get_parsed_changesets(version_type begin, version_type end) const
{
    ...
    TESSERA_ASSERT(begin > m_history_base_version);
    if (end == version_type(-1)) {
        end = m_history_base_version + m_history_size + 1;
    }
    TESSERA_ASSERT(begin <= end);
...
public:
    ...
    {
        TESSERA_ASSERT(m_changesets);
        TESSERA_ASSERT(server_version > m_base_version);
    ...
    {
        TESSERA_ASSERT(new_base_version > m_base_version);
                                                ...
                                                std::size_t num_changesets, util::Logger& logger)
{
    ...
    TESSERA_ASSERT(remote_file_index < m_num_client_files);
    ...
    if (from_downstream) {
        ...
        bool expired = (last_seen_timestamp == 0);
        TESSERA_ASSERT_RELEASE(!expired);
    }
...

    {
        ...
        for (std::size_t i = 0; i < num_changesets; ++i) {
            ...
            TESSERA_ASSERT(changesets[i].origin_file_ident > 0);
            UploadCursor upload_cursor = {changesets[i].remote_version, changesets[i].last_integrated_local_version};
            TESSERA_ASSERT(upload_cursor.client_version > prev_upload_cursor.client_version);
            TESSERA_ASSERT(are_mutually_consistent(upload_cursor, prev_upload_cursor));
    ...
    if (num_changesets > 0) {
        ...
        for (std::size_t i = 0; i < num_changesets; ++i) {
            TESSERA_ASSERT(get_instruction_encoder().buffer().size() == 0);
                                           ...
                                           UploadCursor upload_progress)
{
    ...
    TESSERA_ASSERT(upload_progress.client_version >= orig_upload_progress.client_version);
    TESSERA_ASSERT(are_mutually_consistent(upload_progress, orig_upload_progress));
...
void ServerHistory::upgrade_history_schema(int stored_schema_version)
{
    ...
    // >= 1`).
    TESSERA_ASSERT(stored_schema_version < get_server_history_schema_version());
    TESSERA_ASSERT(stored_schema_version >= 1);
    int orig_schema_version = stored_schema_version;
    int schema_version = orig_schema_version;
    // NOTE: Future migration steps go here.

    TESSERA_ASSERT(schema_version == get_server_history_schema_version());
...
{
    TESSERA_ASSERT(begin_version <= end_version);
    TESSERA_ASSERT(begin_version >= m_ct_base_version);
    TESSERA_ASSERT(end_version <= m_ct_base_version + m_ct_history_size);
    std::size_t n = to_size_t(end_version - begin_version);
    TESSERA_ASSERT(n == 0 || m_acc);
...
{
    TESSERA_ASSERT(realm_version >= m_version_of_oldest_bound_snapshot);
...
void ServerHistory::verify() const
{
    ...
    TESSERA_ASSERT(m_ct_history_size != 0 || m_ct_base_version == initial_realm_version);

    if (!m_acc) {
        TESSERA_ASSERT(m_local_file_ident == g_root_node_file_ident);
        TESSERA_ASSERT(m_num_client_files == 0);
        TESSERA_ASSERT(m_history_size == 0);
        TESSERA_ASSERT(m_server_version_salt == 0);
        TESSERA_ASSERT(m_history_base_version == 0);
        TESSERA_ASSERT(m_ct_history_size == 0);
        return;
    }
    ...
    TESSERA_ASSERT(m_history_base_version == m_acc->root.get_as_ref_or_tagged(s_history_base_version_iip).get_as_int());
    salt_type base_version_salt = m_acc->root.get_as_ref_or_tagged(s_base_version_salt_iip).get_as_int();
    TESSERA_ASSERT((m_history_base_version == 0) == (base_version_salt == 0));

    TESSERA_ASSERT(m_acc->cf_ident_salts.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_client_versions.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_rh_base_versions.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_recip_hist_refs.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_proxy_files.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_client_types.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_last_seen_timestamps.size() == m_num_client_files);
    TESSERA_ASSERT(m_acc->cf_locked_server_versions.size() == m_num_client_files);

    TESSERA_ASSERT(m_acc->sh_version_salts.size() == m_history_size);
    TESSERA_ASSERT(m_acc->sh_origin_files.size() == m_history_size);
    TESSERA_ASSERT(m_acc->sh_client_versions.size() == m_history_size);
    TESSERA_ASSERT(m_acc->sh_timestamps.size() == m_history_size);
    TESSERA_ASSERT(m_acc->sh_changesets.size() == m_history_size);
    TESSERA_ASSERT(m_acc->sh_cumul_byte_sizes.size() == m_history_size);

    salt_type server_version_salt =
        (m_history_size == 0 ? base_version_salt : salt_type(m_acc->sh_version_salts.get(m_history_size - 1)));
    TESSERA_ASSERT(m_server_version_salt == server_version_salt);

    TESSERA_ASSERT(m_local_file_ident > 0 && std::uint_fast64_t(m_local_file_ident) < m_num_client_files);
    ...
    for (std::size_t i = 0; i < m_history_size; ++i) {
        ...
        TESSERA_ASSERT(salt > 0 && salt <= 0x0'7FFF'FFFF'FFFF'FFFF);
        file_ident_type origin_file_ident = 0;
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(m_acc->sh_origin_files.get(i), origin_file_ident));
        TESSERA_ASSERT(origin_file_ident != m_local_file_ident);
        std::size_t origin_file_index = 0;
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(origin_file_ident, origin_file_index));
        TESSERA_ASSERT(origin_file_index < m_num_client_files);
        version_type client_version = 0;
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(m_acc->sh_client_versions.get(i), client_version));
        bool of_local_origin = (origin_file_ident == 0);
        if (of_local_origin) {
            TESSERA_ASSERT(client_version == 0);
        }
        else {
            ...
            if (!from_reference_file) {
                ...
                switch (ClientType(client_type)) {
                    ...
                    case ClientType::indirect: {
                        ...
                        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(proxy_file, client_file_ident));
                        good_client_type = true;
                        break;
                    }
                    case ClientType::self:
                ...
                TESSERA_ASSERT(good_client_type);
            }
            ClientFile& client_file = client_files[client_file_ident];
            if (from_reference_file) {
                TESSERA_ASSERT(client_version >= client_file.last_integrated_client_version);
            }
            else {
                TESSERA_ASSERT(client_version > client_file.last_integrated_client_version);
            }
    ...
    for (std::size_t i = 0; i < m_num_client_files; ++i) {
        ...
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(client_version, client_version_2));
        file_ident_type proxy_file_2 = 0;
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(proxy_file, proxy_file_2));
        version_type locked_server_version_2 = 0;
        TESSERA_ASSERT(!util::int_cast_with_overflow_detect(locked_server_version, locked_server_version_2));
        if (client_file_ident == 0) {
            // Special entry
            TESSERA_ASSERT(ident_salt == 0);
            TESSERA_ASSERT(proxy_file_2 == 0);
            TESSERA_ASSERT(client_type == 0);
            TESSERA_ASSERT(last_seen_timestamp == 0);
            TESSERA_ASSERT(locked_server_version_2 == 0);
            // Upstream server
            TESSERA_ASSERT(client_version_2 >= last_integrated_client_version);
        }
        else if (client_file_ident == g_root_node_file_ident) {
            // Root node's entry
            TESSERA_ASSERT(ident_salt == 0);
            TESSERA_ASSERT(client_version_2 == 0);
            TESSERA_ASSERT(rh_base_version == 0);
            TESSERA_ASSERT(recip_hist_ref == 0);
            TESSERA_ASSERT(proxy_file_2 == 0);
            TESSERA_ASSERT(client_type == 0);
            TESSERA_ASSERT(last_seen_timestamp == 0);
            TESSERA_ASSERT(locked_server_version_2 == 0);
            TESSERA_ASSERT(!client_file);
            ...
            TESSERA_ASSERT(recip_hist_ref == 0);
            TESSERA_ASSERT(proxy_file_2 == 0);
            TESSERA_ASSERT(client_type == int(ClientType::self));
            TESSERA_ASSERT(last_seen_timestamp == 0);
            TESSERA_ASSERT(locked_server_version_2 == 0);
            TESSERA_ASSERT(!client_file);
            found_self = true;
            ...
            if (proxy_file_2 == 0) {
                ...
                TESSERA_ASSERT(rh_base_version == 0);
                TESSERA_ASSERT(recip_hist_ref == 0);
                TESSERA_ASSERT(client_type == int(ClientType::upstream));
                TESSERA_ASSERT(last_seen_timestamp == 0);
                TESSERA_ASSERT(locked_server_version_2 == 0);
                TESSERA_ASSERT(!client_file);
            }
                ...
                TESSERA_ASSERT(rh_base_version == 0);
                TESSERA_ASSERT(recip_hist_ref == 0);
                TESSERA_ASSERT(client_type == int(ClientType::indirect));
                TESSERA_ASSERT(last_seen_timestamp == 0);
                TESSERA_ASSERT(locked_server_version_2 == 0);
                TESSERA_ASSERT(is_valid_proxy_file_ident(proxy_file_2));
                TESSERA_ASSERT(!client_file);
            }
        }
        else {
            ...
            bool expired = (last_seen_timestamp == 0);
            TESSERA_ASSERT(ident_salt > 0 && ident_salt <= 0x0'7FFF'FFFF'FFFF'FFFF);
            TESSERA_ASSERT(client_version_2 >= last_integrated_client_version);
            TESSERA_ASSERT(!expired || (recip_hist_ref == 0));
            TESSERA_ASSERT(proxy_file_2 == 0);
            TESSERA_ASSERT(is_direct_client(ClientType(client_type)));
            TESSERA_ASSERT(locked_server_version_2 <= current_server_version);
        }
    }
    TESSERA_ASSERT(found_self);

    TESSERA_ASSERT(m_ct_history_size >= 1); // See comment above
    TESSERA_ASSERT(m_acc->ct_history.size() == m_ct_history_size);
#endif // TESSERA_DEBUG
}
...
public:
    ...
    {
        if (TESSERA_UNLIKELY(m_server_history))
...
void ServerHistory::update_from_ref_and_version(ref_type ref, version_type realm_version)
{
    ...
    if (TESSERA_LIKELY(m_acc)) {
        ...
        if (m_acc->upstream_status.is_attached()) {
            TESSERA_ASSERT(m_acc->upstream_status.size() == s_upstream_status_size);
        }
        if (m_acc->partial_sync.is_attached()) {
            TESSERA_ASSERT(m_acc->partial_sync.size() == s_partial_sync_size);
        }
        dag.release();
    }
...
void ServerHistory::create_empty_history()
{
    ...
    TESSERA_ASSERT(m_num_client_files == 0);
    for (int i = 0; i < 2; ++i) {
        m_acc->cf_ident_salts.insert(tessera::npos, 0);            // Throws
        m_acc->cf_client_versions.insert(tessera::npos, 0);        // Throws
        m_acc->cf_rh_base_versions.insert(tessera::npos, 0);       // Throws
        m_acc->cf_recip_hist_refs.insert(tessera::npos, 0);        // Throws
        m_acc->cf_proxy_files.insert(tessera::npos, 0);            // Throws
        m_acc->cf_client_types.insert(tessera::npos, 0);           // Throws
        m_acc->cf_last_seen_timestamps.insert(tessera::npos, 0);   // Throws
        m_acc->cf_locked_server_versions.insert(tessera::npos, 0); // Throws
        ++m_num_client_files;
    }
}
...
{
    TESSERA_ASSERT(server_version >= m_history_base_version);
    ...
    TESSERA_ASSERT(history_entry_index < m_history_size);
...
{
    static_assert(g_root_node_file_ident == 1, "");
    TESSERA_ASSERT(file_ident >= 2);
    TESSERA_ASSERT(std::uint_fast64_t(file_ident) < m_num_client_files);
...
void ServerHistory::add_sync_history_entry(const HistoryEntry& entry)
{
    ...
    m_acc->sh_version_salts.insert(tessera::npos, m_salt_for_new_server_versions); // Throws
    m_acc->sh_origin_files.insert(tessera::npos, client_file);                     // Throws
    m_acc->sh_client_versions.insert(tessera::npos, client_version);               // Throws
    m_acc->sh_timestamps.insert(tessera::npos, timestamp);                         // Throws
    ...
    m_acc->sh_cumul_byte_sizes.insert(tessera::npos, history_byte_size);
...
void ServerHistory::trim_cont_transact_history()
{
    ...
    if (m_version_of_oldest_bound_snapshot > m_ct_base_version) {
        ...
        TESSERA_ASSERT(num_entries_to_erase < m_ct_history_size);
...
{
    TESSERA_ASSERT(server_version > m_history_base_version && server_version <= get_server_version());
...
{
    TESSERA_ASSERT(remote_file_ident != g_root_node_file_ident);
    TESSERA_ASSERT(begin_version >= m_history_base_version);
    TESSERA_ASSERT(begin_version <= end_version);
...
bool ServerHistory::received_from(const HistoryEntry& entry, file_ident_type remote_file_ident) const noexcept
{
    ...
    if (!from_upstream_server) {
        std::size_t remote_file_index = std::size_t(remote_file_ident);
        TESSERA_ASSERT(is_direct_client(ClientType(m_acc->cf_client_types.get(remote_file_index))));
...
void ServerHistory::fixup_state_and_changesets_for_assigned_file_ident(Transaction& group, file_ident_type file_ident)
{
    ...
    TESSERA_ASSERT(file_ident != 0);
    TESSERA_ASSERT(file_ident != g_root_node_file_ident);
    TESSERA_ASSERT(m_acc->upstream_status.is_attached());
    TESSERA_ASSERT(m_local_file_ident == g_root_node_file_ident);
    using Instruction = tessera::sync::Instruction;

    auto promote_global_key = [&](GlobalKey& oid) {
        TESSERA_ASSERT(oid.hi() == 0); // client_file_ident == 0
...
void ServerHistory::record_current_schema_version(Array& schema_versions, version_type snapshot_version)
{
    ...
    TESSERA_ASSERT(schema_versions.size() == s_schema_versions_size);
    ...
    }
    {
        ...
        const char* library_version = TESSERA_VERSION_STRING;
