...
bool valid_virt_path_segment(const std::string& seg)
{
    ...
    // created by appending `.tess`, `.tess.lock`, or `.tess.management`
    // to the last component of client specified virtual paths.
    bool possible_clash = (StringData(seg).ends_with(".tess") || StringData(seg).ends_with(".tess.lock") ||
                           StringData(seg).ends_with(".tess.management"));
    if (possible_clash)
        return false;
...
_impl::VirtualPathComponents _impl::parse_virtual_path(const std::string& root_path, const std::string& virt_path)
{
    ...
    result.real_realm_path = real_path + ".tess";
    return result;
}
...
{
    TESSERA_ASSERT(!virt_path.empty());
    size_t prev_pos = 0;
    std::string real_path = root_path;
    if (virt_path.front() != '/') {
        real_path += '/';
    ...
    for (;;) {
        ...
        TESSERA_ASSERT(valid_virt_path_segment(name));
