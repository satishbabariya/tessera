...
TESSERA_NORETURN void throw_bad_transaction_log(std::string msg)
{
...
TESSERA_NORETURN void InstructionApplier::bad_transaction_log(const std::string& msg) const
{
    if (m_last_object_key) {
        // If the last_object_key is valid then we should have a changeset and a current table
        TESSERA_ASSERT(m_log);
        TESSERA_ASSERT(m_last_table_name);
        std::stringstream ss;
...
TESSERA_NORETURN void InstructionApplier::bad_transaction_log(const char* msg, Params&&... params) const
{
...
void InstructionApplier::operator()(const Instruction::EraseTable& instr)
{
    ...
    if (TESSERA_UNLIKELY(TESSERA_COVER_NEVER(!m_transaction.has_table(table_name)))) {
...
void InstructionApplier::visit_payload(const Instruction::Payload& payload, F&& visitor)
{
    ...
    switch (payload.type) {
        ...
        case Type::GlobalKey:
            return visitor(tessera::util::none); // FIXME: Not sure about this
        case Type::Null:
            return visitor(tessera::util::none);
        case Type::Int:
    ...
    if (instr.type != Type::Link) {
        ...
        if (target_table_name.size() != 0) {
            ...
            if (instr.collection_type == CollectionType::List) {
                ...
                TESSERA_ASSERT(instr.collection_type == CollectionType::Single);
            ...
            auto inserter = util::overload{
                ...
                [&](const Instruction::Payload::ObjectValue&) {
                    if (data_type == type_Link) {
                        ...
                        TESSERA_ASSERT(dynamic_cast<LnkLst*>(&list));
...
void InstructionApplier::operator()(const Instruction::Clear& instr)
{
    ...
    struct ClearResolver : public PathResolver {
        ...
        Status on_list_index(LstBase& list, uint32_t index) override
        {
            TESSERA_ASSERT(m_collection_type);
            TESSERA_ASSERT(dynamic_cast<Lst<Mixed>*>(&list));
            auto& mixed_list = static_cast<Lst<Mixed>&>(list);
...
void InstructionApplier::operator()(const Instruction::SetInsert& instr)
{
    struct SetInsertResolver : public PathResolver {
        ...
        Status on_property(Obj& obj, ColKey col) override
        {
            ...
            TESSERA_ASSERT(col.get_type() == col_type_Mixed);
        ...
        void on_set(SetBase& set) override
        {
            ...
            auto inserter = util::overload{
                [&](const ObjLink& link) {
                    if (data_type == type_TypedLink) {
                        TESSERA_ASSERT(dynamic_cast<Set<ObjLink>*>(&set));
                        auto& link_set = static_cast<Set<ObjLink>&>(set);
                        link_set.insert(link);
                    }
                    else if (data_type == type_Mixed) {
                        TESSERA_ASSERT(dynamic_cast<Set<Mixed>*>(&set));
                        auto& mixed_set = static_cast<Set<Mixed>&>(set);
                        mixed_set.insert(link);
                    }
                    else if (data_type == type_Link) {
                        TESSERA_ASSERT(dynamic_cast<Set<ObjKey>*>(&set));
    ...
    return mpark::visit(
        util::overload{
            [&](mpark::monostate) {
                ...
                ObjKey key = table.get_objkey_from_primary_key(tessera::util::none);
                return key;
            },
            [&](int64_t pk) {
                if (!pk_col) {
