...
public:
    HistoryCursorPtr create_history_cursor(util::Optional<file_ident_type> reciprocal) override
    {
        ...
        if (TESSERA_UNLIKELY(!cursor))
            return nullptr;
        if (reciprocal) {
            if (TESSERA_UNLIKELY(!cursor->reciprocal(*reciprocal))) // Throws
                return nullptr;
        }
        cursor->init(); // Throws
        return cursor;
    ...
    HistoryCursorPtr create_history_cursor(util::Optional<file_ident_type> reciprocal, version_type version) override
    {
        ...
        if (TESSERA_UNLIKELY(!cursor->init(version))) // Throws
            return nullptr;
        return cursor;
    }
                                           ...
                                           version_type end_version) override
    {
        ...
        if (TESSERA_UNLIKELY(!cursor->init(begin_version, end_version))) // Throws
            return nullptr;
        return cursor;
    }
...
public:
    ...
    {
        TESSERA_ASSERT(schema_version == 12);
...
public:
    ServerHistoryCursor_6_to_10(Allocator& alloc, ref_type root_ref, int schema_version)
        ...
        , m_root{alloc}
    {
        TESSERA_ASSERT(schema_version >= 6 && schema_version <= 10);
        ...
        TESSERA_ASSERT(m_origin_files->size() == history_size);
        TESSERA_ASSERT(m_client_versions->size() == history_size);
        TESSERA_ASSERT(m_timestamps->size() == history_size);
        TESSERA_ASSERT(m_changesets->size() == history_size);
...
public:
        ...
        : m_root{alloc}
    {
        ...
        TESSERA_ASSERT(m_client_versions->size() == m_size);
        TESSERA_ASSERT(m_rh_base_versions->size() == m_size);
        TESSERA_ASSERT(m_proxy_files->size() == m_size);
        TESSERA_ASSERT(!m_client_types || m_client_types->size() == m_size);
        TESSERA_ASSERT(m_last_seen_timestamps->size() == m_size);
        TESSERA_ASSERT(!m_locked_server_versions || m_locked_server_versions->size() == m_size);
        ...
        // Find the client file entry that corresponds to ourselves
        {
            ...
            TESSERA_ASSERT_RELEASE(upstream_status_ref == 0 || partial_sync_ref == 0);
    ...
    LogicalClientType get_logical_client_type() const override final
    {
        ...
        if (TESSERA_UNLIKELY(index == 0))
            return LogicalClientType::special;
        if (TESSERA_UNLIKELY(index == 1)) {
...
private:
                                ...
                                file_ident_type proxy_file) const
    {
        if (client_file_ident == 0) {
            TESSERA_ASSERT(client_type == ClientType(0));
                     ...
                     Format format, Summary summary, bool with_versions, std::ostream& out)
{
    ...
    while (cursor.next()) {
        ...
        if (TESSERA_UNLIKELY(origin_file)) {
            if (TESSERA_LIKELY(cursor.get_origin_file() != *origin_file))
                continue;
        }
        if (TESSERA_UNLIKELY(expression)) {
            buffer.clear();
            ...
            for (auto instr : changeset) {
                TESSERA_ASSERT(instr);
                bool did_match = instr->visit(matcher);
                if (TESSERA_LIKELY(!did_match))
                    continue;
                instr_was_found = true;
                break;
            }
            if (TESSERA_LIKELY(!instr_was_found))
                continue;
        }
        switch (format) {
            case Format::auto_: {
        ...
        ++num_history_entries;
        if (TESSERA_UNLIKELY(version < min_version))
            min_version = version;
        if (TESSERA_UNLIKELY(version > max_version))
            max_version = version;
        if (summary == Summary::full) {
            timestamp_type timestamp = cursor.get_origin_timestamp(); // Throws
            if (TESSERA_UNLIKELY(timestamp < min_timestamp))
                min_timestamp = timestamp;
            if (TESSERA_UNLIKELY(timestamp > max_timestamp))
                          ...
                          std::time_t max_last_seen_timestamp, version_type max_locked_version)
{
    ...
    while (cursor.next()) {
        ...
        ++num_client_files;
        if (is_unexpired) {
            if (TESSERA_UNLIKELY(last_seen_timestamp < min_timestamp))
                min_timestamp = last_seen_timestamp;
            if (TESSERA_UNLIKELY(last_seen_timestamp > max_timestamp))
