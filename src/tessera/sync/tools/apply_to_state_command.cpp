...
struct ServerIdentMessage {
    tessera::sync::session_ident_type session_ident;
    tessera::sync::SaltedFileIdent file_ident;
...
struct DownloadMessage {
    tessera::sync::session_ident_type session_ident;
    tessera::sync::SyncProgress progress;
    tessera::sync::SaltedVersion latest_server_version;
    uint64_t downloadable_bytes;
    tessera::sync::DownloadBatchState batch_state;
    int64_t query_version;

    Buffer<char> uncompressed_body_buffer;
    std::vector<tessera::sync::RemoteChangeset> changesets;
...
struct UploadMessage {
    tessera::sync::session_ident_type session_ident;
    tessera::sync::UploadCursor upload_progress;
    tessera::sync::version_type locked_server_version;

    Buffer<char> uncompressed_body_buffer;
    std::vector<tessera::sync::Changeset> changesets;
...
DownloadMessage DownloadMessage::parse(HeaderLineParser& msg, Logger& logger, bool is_flx_sync)
{
    ...
    while (!body.at_end()) {
        tessera::sync::RemoteChangeset cur_changeset;
        ...
        auto changeset_stream = tessera::util::SimpleInputStream(changeset_data);
...
UploadMessage UploadMessage::parse(HeaderLineParser& msg, Logger& logger)
{
    ...
    HeaderLineParser body(body_str);
    while (!body.at_end()) {
        tessera::sync::Changeset cur_changeset;
        ...
        tessera::util::SimpleInputStream changeset_stream(changeset_buffer);
        try {
            tessera::sync::parse_changeset(changeset_stream, cur_changeset);
        }
        catch (...) {
...
int main(int argc, const char** argv)
{
    ...
    tessera::DBOptions db_opts(encryption_key.empty() ? nullptr : encryption_key.c_str());
    tessera::sync::ClientReplication repl{};
    auto local_db = tessera::DB::create(repl, realm_path, db_opts);
    ...
    while (!msg.at_end()) {
        ...
        mpark::visit(tessera::util::overload{
                         [&](const DownloadMessage& download_message) {
                             tessera::sync::VersionInfo version_info;
                         ...
                         [&](const UploadMessage& upload_message) {
                             for (const auto& changeset : upload_message.changesets) {
                                 ...
                                 tessera::sync::InstructionApplier applier(*transaction);
                                 ...
                                 history.set_local_origin_timestamp_source(tessera::sync::generate_changeset_timestamp);
                             }
                         },
