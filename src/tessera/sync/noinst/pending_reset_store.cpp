...
std::optional<PendingReset> PendingResetStore::has_pending_reset(const Group& group)
{
    ...
    TESSERA_ASSERT(reset_store->m_pending_reset_table);
    ...
    if (reset_entry.get<String>(reset_store->m_version) != TESSERA_VERSION_STRING) {
...
{
    TESSERA_ASSERT(mode != ClientResyncMode::Manual);
    ...
    table->create_object(null_key, {
                                       {reset_store.m_version, Mixed(TESSERA_VERSION_STRING)},
