...
#ifndef TESSERA_NOINST_CLIENT_IMPL_BASE_HPP
#define TESSERA_NOINST_CLIENT_IMPL_BASE_HPP
...
{
    TESSERA_ASSERT(!was_voluntary(reason));
...
inline void ClientImpl::Connection::change_state_to_disconnected() noexcept
{
    ...
    TESSERA_ASSERT(m_state != ConnectionState::disconnected);
...
{
    TESSERA_ASSERT(m_num_active_unsuspended_sessions);
    if (--m_num_active_unsuspended_sessions != 0)
        return;
...
{
    TESSERA_ASSERT(m_state == Active);

    bool resume_upload = do_recognize_sync_version(version);
    if (TESSERA_LIKELY(resume_upload)) {
...
inline void ClientImpl::Session::request_download_completion_notification()
{
    ...
    TESSERA_ASSERT(m_error_message_received || !m_unbind_message_sent);
...
{
    if (TESSERA_LIKELY(version > m_last_version_available)) {
...
{
    TESSERA_ASSERT(m_state == Active || m_state == Deactivating);
    ...
    if (m_state == Deactivating) {
        ...
        TESSERA_ASSERT(m_state == Deactivated);
        return;
    }
...
inline void ClientImpl::Session::message_sent()
{
    ...
    TESSERA_ASSERT(!m_unbind_message_send_complete);
...
#endif // TESSERA_NOINST_CLIENT_IMPL_BASE_HPP
