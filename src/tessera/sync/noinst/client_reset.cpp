                    ...
                    bool allow_schema_additions)
{
    ...
    TESSERA_ASSERT_RELEASE(client_repl);
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        if (!table_dst) {
            ...
            if (table_src->is_embedded()) {
                TESSERA_ASSERT(!pk_col_src);
                group_dst.add_table(table_name, Table::Type::Embedded);
            }
            else {
                TESSERA_ASSERT(pk_col_src); // a sync table will have a pk
                auto pk_col_src = table_src->get_primary_key_column();
    ...
    size_t num_tables;
    {
        ...
        TESSERA_ASSERT_EX(allow_schema_additions || num_tables_src == num_tables_dst, num_tables_src, num_tables_dst);
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        TESSERA_ASSERT_DEBUG(pk_col); // sync realms always have a pk
    ...
    for (auto table_key : group_src.get_table_keys()) {
        ...
        StringData table_name = table_src->get_name();
        TableRef table_dst = group_dst.get_table(table_name);
        TESSERA_ASSERT_EX(allow_schema_additions || table_src->get_column_count() == table_dst->get_column_count(),
