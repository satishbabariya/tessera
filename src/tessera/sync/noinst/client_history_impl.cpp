    ...
    SaltedVersion server_version, const std::vector<_impl::client_reset::RecoveredChange>& recovered_changesets)
{
    ...
    TESSERA_ASSERT(client_version == current_version); // For now
    ...
    if (recovered_changesets.empty()) {
        ...
        TESSERA_ASSERT(first_version >= m_sync_history_base_version);
...
// Overriding member function in tessera::Replication
...
void ClientReplication::upgrade_history_schema(int stored_schema_version)
{
    ...
    // >= 1`).
    TESSERA_ASSERT(stored_schema_version < get_client_history_schema_version());
    TESSERA_ASSERT(stored_schema_version >= 11);
    ...
    TESSERA_ASSERT(schema_version == get_client_history_schema_version());
                               ...
                               SyncProgress& progress) const
{
    ...
    TESSERA_ASSERT(current_client_version >= s_initial_version + 0);
...
{
    TESSERA_ASSERT(client_file_ident.ident != 0);
    ...
    Array& root = m_arrays->root;
    TESSERA_ASSERT(wt->get_sync_file_id() == 0);
...
// Overriding member function in tessera::sync::ClientHistoryBase
...
{
    TESSERA_ASSERT(incoming_changesets.size() != 0);
    TESSERA_ASSERT(
    ...
    while (!changesets_to_integrate.empty()) {
        ...
        TESSERA_ASSERT(!m_applying_server_changeset);
    ...
    TESSERA_ASSERT(new_version.version > 0);
    TESSERA_ASSERT(
...
{
    TESSERA_ASSERT(transact->get_transact_stage() == DB::transact_Writing);
    ...
    try {
        for (auto& changeset : changesets_to_integrate) {
            TESSERA_ASSERT(changeset.last_integrated_remote_version <= local_version);
            TESSERA_ASSERT(changeset.origin_file_ident > 0 && changeset.origin_file_ident != sync_file_id);
...
{
    is_compressed = true;
    TESSERA_ASSERT(version > m_sync_history_base_version);

    std::size_t index = to_size_t(version - m_sync_history_base_version) - 1;
    TESSERA_ASSERT(index < sync_history_size());
                                            ...
                                            version_type& last_integrated_server_version) noexcept -> version_type
{
    ...
    TESSERA_ASSERT(begin_version >= base_version);
    TESSERA_ASSERT(end_version <= base_version + arrays.changesets.size());
                                                             ...
                                                             version_type end_version) const noexcept
{
    ...
    TESSERA_ASSERT(m_arrays->changesets.is_attached());
    TESSERA_ASSERT(m_arrays->origin_file_idents.is_attached());
    TESSERA_ASSERT(end_version <= m_sync_history_base_version + sync_history_size());
...
{
    if (m_arrays) {
        TESSERA_ASSERT(m_arrays->root.size() == s_root_size);
        return;
    }
...
Replication::version_type ClientHistory::add_changeset(BinaryData ct_changeset, BinaryData sync_changeset)
{
    ...
    TESSERA_ASSERT(!m_applying_server_changeset || !m_applying_client_reset);
    ...
    if (m_applying_server_changeset) {
        ...
        TESSERA_ASSERT(m_ct_history_base_version + ct_history_size() ==
                     m_sync_history_base_version + sync_history_size());
        TESSERA_ASSERT(sync_changeset.size() == 0);
...
{
    TESSERA_ASSERT(m_arrays->changesets.size() == sync_history_size());
    TESSERA_ASSERT(m_arrays->reciprocal_transforms.size() == sync_history_size());
    TESSERA_ASSERT(m_arrays->remote_versions.size() == sync_history_size());
    TESSERA_ASSERT(m_arrays->origin_file_idents.size() == sync_history_size());
    TESSERA_ASSERT(m_arrays->origin_timestamps.size() == sync_history_size());
    ...
    m_arrays->remote_versions.insert(tessera::npos, std::int_fast64_t(entry.remote_version));       // Throws
    m_arrays->origin_file_idents.insert(tessera::npos, std::int_fast64_t(entry.origin_file_ident)); // Throws
    m_arrays->origin_timestamps.insert(tessera::npos, std::int_fast64_t(entry.origin_timestamp));   // Throws
}
...
void ClientHistory::set_download_progress(Transaction& tr, DownloadableProgress p)
{
    ...
    TESSERA_ASSERT(root.size() > s_progress_uploadable_bytes_iip);
    root.set(s_progress_downloadable_bytes_iip,
...
void ClientHistory::trim_ct_history()
{
    ...
    // history empty.
    TESSERA_ASSERT(n < ct_history_size());
    ...
    TESSERA_ASSERT(m_ct_history_base_version + ct_history_size() == m_sync_history_base_version + sync_history_size());
}
...
void ClientHistory::do_trim_sync_history(std::size_t n)
{
    ...
    TESSERA_ASSERT(m_arrays->origin_file_idents.size() == sync_history_size());
    TESSERA_ASSERT(m_arrays->origin_timestamps.size() == sync_history_size());
    TESSERA_ASSERT(n <= sync_history_size());
...
// Overriding member function in tessera::_impl::History
...
{
    TESSERA_ASSERT(begin_version <= end_version);
    TESSERA_ASSERT(begin_version >= m_ct_history_base_version);
    TESSERA_ASSERT(end_version <= m_ct_history_base_version + ct_history_size());
    std::size_t n = to_size_t(end_version - begin_version);
    TESSERA_ASSERT(n == 0 || m_arrays);
...
{
    TESSERA_ASSERT(version >= m_version_of_oldest_bound_snapshot);
...
void ClientHistory::verify() const
{
    ...
    TESSERA_ASSERT(ct_history_size() != 0 || m_ct_history_base_version == s_initial_version + 0);

    if (!m_arrays) {
        TESSERA_ASSERT(m_progress_download.server_version == 0);
        TESSERA_ASSERT(m_progress_download.last_integrated_client_version == 0);
        return;
    }
    m_arrays->verify();
    ...
    version_type progress_download_client_version =
        version_type(root.get_as_ref_or_tagged(s_progress_download_client_version_iip).get_as_int());
    TESSERA_ASSERT(progress_download_server_version == m_progress_download.server_version);
    TESSERA_ASSERT(progress_download_client_version == m_progress_download.last_integrated_client_version);
    TESSERA_ASSERT(progress_download_client_version <= m_sync_history_base_version + sync_history_size());
    ...
    TESSERA_ASSERT(progress_download_server_version >= remote_version_of_last_entry);
    ...
    TESSERA_ASSERT(cooked_history.get_ref_from_parent() == 0);
#endif // TESSERA_DEBUG
}
...
{
    root.init_from_ref(ref);
    TESSERA_ASSERT(root.size() == s_root_size);
    {
...
void ClientHistory::Arrays::verify() const
{
    ...
    TESSERA_ASSERT(root.size() == s_root_size);
    TESSERA_ASSERT(reciprocal_transforms.size() == changesets.size());
    TESSERA_ASSERT(remote_versions.size() == changesets.size());
    TESSERA_ASSERT(origin_file_idents.size() == changesets.size());
    TESSERA_ASSERT(origin_timestamps.size() == changesets.size());
#endif // TESSERA_DEBUG
}
