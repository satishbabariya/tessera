...
#ifndef TESSERA_NOINST_CHANGESET_INDEX_HPP
#define TESSERA_NOINST_CHANGESET_INDEX_HPP
...
struct ChangesetIndex {
    using Changeset = tessera::sync::Changeset;
    using GlobalID = tessera::sync::GlobalID;
    using PrimaryKey = tessera::sync::PrimaryKey;
...
struct ChangesetIndex::RangeIterator {
        ...
        , m_outer(ranges->begin())
    {
        if (m_outer != ranges->end()) {
            ...
            m_pos = m_inner->begin;
            TESSERA_ASSERT(m_pos != m_inner->end); // empty ranges not allowed!
            check();
        }
    }
    ...
    {
        TESSERA_ASSERT_DEBUG(m_ranges);
        TESSERA_ASSERT_DEBUG(m_outer != m_ranges->end());
        TESSERA_ASSERT_DEBUG(m_inner >= m_outer->second.begin());
        TESSERA_ASSERT_DEBUG(m_inner < m_outer->second.end());
        TESSERA_ASSERT_DEBUG(m_pos >= m_inner->begin);
        TESSERA_ASSERT_DEBUG(m_pos < m_inner->end);
        TESSERA_ASSERT_DEBUG(m_pos.m_inner >= m_outer->first->begin().m_inner);
        TESSERA_ASSERT_DEBUG(m_pos.m_inner < m_outer->first->end().m_inner);
    }
    ...
    RangeIterator& operator++() noexcept
    {
        ...
        ++m_pos;
        if (TESSERA_UNLIKELY(m_pos == m_inner->end)) {
            // Slow path
            inc_inner();
        }
        return *this;
    ...
    {
        TESSERA_ASSERT(m_ranges == other.m_ranges);
...
#endif // TESSERA_NOINST_CHANGESET_INDEX_HPP
