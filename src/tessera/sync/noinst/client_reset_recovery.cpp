...
struct InternDictKey {
    bool is_null() const
    {
        return m_pos == tessera::npos && m_size == tessera::npos;
    }
...
private:
    ...
    size_t m_pos = tessera::npos;
    size_t m_size = tessera::npos;
};
...
private:
    ...
    TESSERA_NORETURN void handle_error(const std::string& message) const;
...
#define TESSERA_DECLARE_INSTRUCTION_HANDLER(X) void operator()(const Instruction::X&) override;
    TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_DECLARE_INSTRUCTION_HANDLER)
#undef TESSERA_DECLARE_INSTRUCTION_HANDLER
                       ...
                       uint32_t& remote_to_out)
{
    ...
    for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
        if (it->local == from) {
            TESSERA_ASSERT(target_from == m_indices_allowed.end());
            target_from = it;
        }
        else if (it->local == to) {
            TESSERA_ASSERT(target_to == m_indices_allowed.end());
            target_to = it;
        }
    }
    ...
    TESSERA_ASSERT_EX(target_from->remote <= lst_size, from, to, target_from->remote, target_to->remote, lst_size);
    TESSERA_ASSERT_EX(target_to->remote <= lst_size, from, to, target_from->remote, target_to->remote, lst_size);

    if (from < to) {
        for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
            if (it->local > from && it->local <= to) {
                TESSERA_ASSERT(it->local != 0);
                TESSERA_ASSERT(it->remote != 0);
                --it->local;
                --it->remote;
            }
        }
        ...
        for (auto it = m_indices_allowed.begin(); it != m_indices_allowed.end(); ++it) {
            if (it->local < from && it->local >= to) {
                TESSERA_ASSERT_EX(it->remote + 1 < lst_size, it->remote, lst_size);
                ++it->local;
                ++it->remote;
            }
        }
...
std::string_view InterningBuffer::get_key(const InternDictKey& key) const
{
    ...
    TESSERA_ASSERT(key.m_pos < m_dict_keys_buffer.size());
    TESSERA_ASSERT(key.m_pos + key.m_size <= m_dict_keys_buffer.size());
...
TESSERA_NORETURN void RecoverLocalChangesetsHandler::handle_error(const std::string& message) const
{
    ...
    throw tessera::sync::ClientResetFailed(full_message);
}
...
void RecoverLocalChangesetsHandler::copy_lists_with_unrecoverable_changes()
{
    ...
    using namespace tessera::converters;
                                                 ...
                                                 util::UniqueFunction<void(LstBase&, LstBase&)> callback)
{
    ...
    for (auto it = path.begin(); it != path.end();) {
        ...
        TESSERA_ASSERT(it->type != ListPath::Element::Type::ListIndex);
        ...
        if (col.is_list()) {
            ...
            TESSERA_ASSERT_EX(!col.is_collection(), col);
            TESSERA_ASSERT_EX(col.get_type() == col_type_Link, col);
...
RecoverLocalChangesetsHandler::RecoveryResolver::on_list_index_advance(uint32_t index)
{
    if (m_recovery_applier->m_lists.count(m_list_path) != 0) {
        ...
        TESSERA_ASSERT(cross_ndx->remote != uint32_t(-1));
...
{
    TESSERA_ASSERT(m_it_begin != m_path_instr.path.begin());
    size_t distance = (m_it_begin - m_path_instr.path.begin()) - 1;
    TESSERA_ASSERT_EX(distance < m_path_instr.path.size(), distance, m_path_instr.path.size());
    TESSERA_ASSERT(mpark::holds_alternative<uint32_t>(m_path_instr.path[distance]));
    ...
    try {
        ...
        if (m_replication && table) {
            if (table->is_embedded()) {
                ...
                TESSERA_ASSERT_EX(pk_col, class_name);
...
void RecoverLocalChangesetsHandler::operator()(const Instruction::ArrayInsert& instr)
{
    struct ArrayInsertResolver : public RecoveryResolver {
        ...
        Status on_list_index(LstBase& list, uint32_t index) override
        {
            TESSERA_ASSERT(index != uint32_t(-1));
