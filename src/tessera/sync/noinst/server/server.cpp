...
#include <tessera/util/scratch_allocator.hpp>
#include <tessera/util/thread.hpp>
#include <tessera/util/thread_exec_guard.hpp>
...
public:
    ...
    util::bind_ptr<ServerFile> get_file(const std::string& virt_path) noexcept
    {
        ...
        if (TESSERA_LIKELY(i != m_files.end()))
            return i->second;
        return {};
    }
    ...
    {
        if (TESSERA_UNLIKELY(m_config.token_expiration_clock))
...
public:
    ...
    bool websocket_binary_message_received(const char* data, size_t size) final override
    {
        ...
        if (TESSERA_LIKELY(!m_is_closing)) {
    ...
    void websocket_handshake_completion_handler(const HTTPHeaders&) final override
    {
        ...
        TESSERA_TERMINATE("websocket_handshake_completion_handler should not have been called");
    }
...
{
    TESSERA_ASSERT(ec != util::error::operation_aborted);
...
util::Optional<AccessToken> m_access_token;
    ...
    void initiate_http()
    {
        ...
        auto handler = [this](HTTPRequest request, std::error_code ec) {
            if (TESSERA_UNLIKELY(ec == util::error::operation_aborted))
                return;
            if (TESSERA_UNLIKELY(ec == HTTPParserError::MalformedRequest)) {
                logger.error("Malformed HTTP request");
                close_due_to_error(ec); // Throws
                return;
            }
            if (TESSERA_UNLIKELY(ec == HTTPParserError::BadRequest)) {
                logger.error("Bad HTTP request");
                const char* body = "The HTTP request was corrupted";
                handle_400_bad_request(body); // Throws
                return;
            }
            if (TESSERA_UNLIKELY(ec)) {
    ...
    void handle_request_for_sync(const HTTPRequest& request)
    {
            ...
            while (parser.next(elem)) {
                ...
                if (!prefix.empty()) {
                    auto parse_version = [&](std::string_view str) {
                        ...
                        int version = 0;
                        in >> version;
                        if (TESSERA_LIKELY(in && in.eof() && version >= 0))
                            return version;
                        return -1;
                    };
                    int min, max;
                    ...
                    if (TESSERA_LIKELY(min >= 0 && max >= 0 && min <= max)) {
        ...
        }
        {
            ...
            if (TESSERA_UNLIKELY(best_match == 0)) {
...
public:
    ...
    ~Session() noexcept
    {
        TESSERA_ASSERT(!is_enlisted_to_send());
    ...
    {
        TESSERA_ASSERT(is_session_level_error(error_code));
        TESSERA_ASSERT(!error_occurred()); // Must only be called once

        // If the UNBIND message has been received, then the client has
        // initiated the deactivation process already.
        if (TESSERA_LIKELY(!unbind_message_received())) {
            detach_from_server_file();
    ...
    void receive_file_ident(SaltedFileIdent file_ident) override final
    {
        ...
        if (!ident_message_received()) {
            TESSERA_ASSERT(need_client_file_ident());
            TESSERA_ASSERT(m_send_ident_message);
        }
        else {
            TESSERA_ASSERT(!m_send_ident_message);
        }
    ...
    {
        if (TESSERA_LIKELY(!unbind_message_received())) {
            if (TESSERA_LIKELY(!error_occurred())) {
                if (TESSERA_LIKELY(ident_message_received())) {
                    // State is WaitForUnbind.
                    bool relayed_alloc = (m_allocated_file_ident.ident != 0);
                    if (TESSERA_LIKELY(!relayed_alloc)) {
                              ...
                              bool is_subserver, ProtocolError& error)
    {
        ...
        if (TESSERA_LIKELY(!need_client_file_ident)) {
                               ...
                               ProtocolError& error)
    {
        ...
        TESSERA_ASSERT(!need_client_file_ident());
        TESSERA_ASSERT(!m_send_ident_message);
        TESSERA_ASSERT(!ident_message_received());
        TESSERA_ASSERT(!unbind_message_received());
        TESSERA_ASSERT(!error_occurred());
        TESSERA_ASSERT(!m_error_message_sent);
        ...
        if (TESSERA_UNLIKELY(config.session_bootstrap_callback)) {
                                ...
                                ProtocolError& error)
    {
        ...
        if (TESSERA_UNLIKELY(!is_consistent(upload_progress))) {
        ...
        // consistent.
        if (TESSERA_UNLIKELY(!are_mutually_consistent(upload_progress, m_upload_threshold))) {
        ...
        // consistent.
        if (TESSERA_UNLIKELY(!are_mutually_consistent(upload_progress, m_upload_progress))) {
        ...
        if (TESSERA_UNLIKELY(locked_server_version_2 < m_locked_server_version)) {
        ...
        // heard about.
        if (TESSERA_UNLIKELY(locked_server_version_2 > m_download_progress.server_version)) {
        ...
        if (!upload_changesets.empty()) {
            ...
            for (const ServerProtocol::UploadChangeset& uc : upload_changesets) {
                ...
                // UPLOAD message.
                if (TESSERA_UNLIKELY(uc.upload_cursor.client_version <= up.client_version)) {
                ...
                if (TESSERA_UNLIKELY(!is_consistent(uc.upload_cursor))) {
                ...
                if (TESSERA_UNLIKELY(!are_mutually_consistent(uc.upload_cursor, up))) {
                ...
                if (TESSERA_UNLIKELY(!consistent_with_threshold)) {
                    logger.error("Upload cursor of uploaded changeset (%1, %2) is mutually "
            ...
            // UPLOAD message.
            if (TESSERA_UNLIKELY(up.client_version > upload_progress.client_version)) {
            ...
            if (TESSERA_UNLIKELY(!are_mutually_consistent(up, upload_progress))) {
        ...
        // scheme.
        if (TESSERA_UNLIKELY(!m_server_file->can_add_changesets_from_downstream())) {
        ...
        TESSERA_ASSERT(have_real_upload_progress || num_changesets_to_integrate == 0);
    ...
    void receive_unbind_message()
    {
        ...
        TESSERA_ASSERT(!m_unbind_message_received);
...
private:
    ...
    void continue_history_scan()
    {
        ...
        TESSERA_ASSERT(last_server_version.version >= m_download_progress.server_version);

        ServerImpl& server = m_connection.get_server();
        const Server::Config& config = server.get_config();
        if (TESSERA_UNLIKELY(m_disable_download))
            return;
        ...
        if (have_more_to_scan) {
            ...
            if (fetch_from_cache) {
                ...
                auto fetch_and_compress = [&](std::size_t max_download_size) {
                    ...
                    TESSERA_ASSERT(upload_progress.client_version >= download_progress.last_integrated_client_version);
                    SyncConnection& conn = get_connection();
                    if (TESSERA_UNLIKELY(!not_expired)) {
                ...
                if (enable_cache) {
                    ...
                    TESSERA_ASSERT(upload_progress.client_version == 0);
    ...
    void send_ident_message()
    {
        ...
        TESSERA_ASSERT(m_allocated_file_ident.ident != 0);
    ...
    void send_error_message()
    {
        ...
        TESSERA_ASSERT(is_session_level_error(m_error_code));
...
{
    TESSERA_ASSERT(m_unidentified_sessions.empty());
    TESSERA_ASSERT(m_identified_sessions.empty());
    TESSERA_ASSERT(m_file_ident_request == 0);
}
...
void ServerFile::initialize()
{
    ...
    TESSERA_ASSERT(!has_upstream_sync_status);
...
void ServerFile::cancel_file_ident_request(file_ident_request_type request) noexcept
{
    ...
    TESSERA_ASSERT(i != m_file_ident_requests.end());
    FileIdentRequestInfo& info = i->second;
    TESSERA_ASSERT(info.receiver);
...
{
    TESSERA_ASSERT(m_unidentified_sessions.count(sess) == 0);
...
{
    TESSERA_ASSERT(m_unidentified_sessions.count(sess) == 1);
    TESSERA_ASSERT(m_identified_sessions.count(client_file_ident) == 0);
...
{
    TESSERA_ASSERT(m_identified_sessions.count(client_file_ident) == 1);
                                                ...
                                                std::size_t num_changesets)
{
    ...
    TESSERA_ASSERT(upload_progress.client_version >= list.upload_progress.client_version);
    TESSERA_ASSERT(are_mutually_consistent(upload_progress, list.upload_progress));
    if (upload_progress.client_version > list.upload_progress.client_version) {
        list.upload_progress = upload_progress;
        dirty = true;
    }

    TESSERA_ASSERT(locked_server_version >= list.locked_server_version);
    if (locked_server_version > list.locked_server_version) {
        list.locked_server_version = locked_server_version;
        dirty = true;
    }

    if (TESSERA_LIKELY(dirty)) {
        if (num_changesets > 0) {
                                                    ...
                                                    version_type& locked_server_version, Logger& logger)
{
    ...
    if (error == BootstrapError::no_error) {
        ...
        if (i != m_work.changesets_from_downstream.end()) {
            ...
            TESSERA_ASSERT(list.upload_progress.client_version >= upload_progress.client_version);
            upload_progress = list.upload_progress;
            TESSERA_ASSERT(list.locked_server_version >= locked_server_version);
...
void ServerFile::worker_process_work_unit(WorkerState& state)
{
    ...
    if (work.has_primary_work) {
        if (TESSERA_UNLIKELY(!m_work.file_ident_alloc_slots.empty()))
...
{
    TESSERA_ASSERT(!m_has_work_in_progress);
    if (TESSERA_LIKELY(!m_server.is_sync_stopped())) {
        unblock_work(); // Throws
        const Work& work = m_work;
        if (TESSERA_LIKELY(work.has_primary_work)) {
...
{
    TESSERA_ASSERT(m_has_blocked_work);
    ...
    TESSERA_ASSERT(m_unblocked_changesets_from_downstream_byte_size == 0);
...
void ServerFile::recognize_external_change()
{
    ...
    TESSERA_ASSERT(m_version_info.realm_version >= prev_version_info.realm_version);
    TESSERA_ASSERT(m_version_info.sync_version.version >= prev_version_info.sync_version.version);
...
{
    Work& work = m_work;
    TESSERA_ASSERT(!work.file_ident_alloc_slots.empty());
...
{
    TESSERA_ASSERT(!m_work.changesets_from_downstream.empty());
    ...
    TESSERA_ASSERT(!produced_new_sync_version || produced_new_realm_version);
...
void ServerFile::finalize_work_stage_1()
{
    ...
    TESSERA_ASSERT(num_changesets_removed <= m_num_changesets_from_downstream);
    TESSERA_ASSERT(num_bytes_removed <= m_blocked_changesets_from_downstream_byte_size);
...
void ServerFile::finalize_work_stage_2()
{
    ...
    TESSERA_ASSERT(m_work.produced_new_realm_version || m_work.version_info.realm_version == 0);
    if (m_work.version_info.realm_version > m_version_info.realm_version) {
        TESSERA_ASSERT(m_work.version_info.sync_version.version >= m_version_info.sync_version.version);
    ...
    TESSERA_ASSERT(m_file_ident_requests.size() >= m_work.file_ident_alloc_slots.size());
    auto begin = m_file_ident_requests.begin();
    auto i = begin;
    for (const FileIdentAllocSlot& slot : m_work.file_ident_alloc_slots) {
        FileIdentRequestInfo& info = i->second;
        TESSERA_ASSERT(info.proxy_file == slot.proxy_file);
        TESSERA_ASSERT(info.client_type == slot.client_type);
...
ServerImpl::~ServerImpl() noexcept
{
    ...
    TESSERA_ASSERT_RELEASE(!server_destroyed_while_still_running);
}
...
{
    TESSERA_ASSERT(byte_size <= m_pending_changesets_from_downstream_byte_size);
    m_pending_changesets_from_downstream_byte_size -= byte_size;
...
{
    TESSERA_ASSERT(m_send_trigger);
    TESSERA_ASSERT(!m_is_closing);
    TESSERA_ASSERT(!sess->is_enlisted_to_send());
                                          ...
                                          bool is_subserver)
{
    ...
    if (TESSERA_UNLIKELY(!was_inserted)) {
    ...
    bool success =
        sess.receive_bind_message(std::move(path), std::move(signed_user_token), need_client_file_ident, is_subserver,
                                  error); // Throws
    if (TESSERA_UNLIKELY(!success))         // Throws
                                           ...
                                           salt_type latest_server_version_salt)
{
    ...
    if (TESSERA_UNLIKELY(sess.ident_message_received())) {
...
{
    TESSERA_ASSERT(!m_is_sending);
    TESSERA_ASSERT(!m_sending_pong);
    if (m_send_pong) {
...
void SyncConnection::protocol_error(ProtocolError error_code, Session* sess)
{
    ...
    TESSERA_ASSERT(!session_level || sess);
    TESSERA_ASSERT(!sess || m_sessions.count(sess->get_session_ident()) == 1);
...
{
    TESSERA_ASSERT(get_protocol_error_message(int(error_code)));
    ...
    TESSERA_ASSERT(is_session_level_error(error_code) == (session_ident != 0));
    TESSERA_ASSERT(!is_session_level_error(error_code));
