...
#ifndef TESSERA_NOINST_SERVER_FILE_ACCESS_CACHE_HPP
#define TESSERA_NOINST_SERVER_FILE_ACCESS_CACHE_HPP
...
{
    TESSERA_ASSERT(m_max_open_files >= 1);
}
...
{
    TESSERA_ASSERT(!slot.m_next_open_file);
    TESSERA_ASSERT(!slot.m_prev_open_file);
...
{
    if (m_file) {
        TESSERA_ASSERT(m_prev_open_file);
        TESSERA_ASSERT(m_next_open_file);
        return true;
    }
...
#endif // TESSERA_NOINST_SERVER_FILE_ACCESS_CACHE_HPP
