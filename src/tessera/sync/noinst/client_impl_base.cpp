...
#include <tessera/sync/noinst/sync_schema_migration.hpp>
...
using namespace tessera::sync::websocket;
...
std::chrono::milliseconds ClientImpl::ReconnectInfo::delay_interval()
{
    ...
    switch (*m_backoff_state.triggering_error) {
        ...
        default:
            ...
            TESSERA_ASSERT(m_reconnect_mode == ReconnectMode::normal);
                                      ...
                                      port_type& port, std::string& path) const
{
    ...
    if (!port_2.empty()) {
        ...
        in >> port_3;
        if (TESSERA_UNLIKELY(!in || !in.eof() || port_3 < 1))
            return false;
    }
    ...
    , m_random{}
{
    ...
    TESSERA_ASSERT_EX(m_socket_provider, "Must provide socket provider in sync Client config");
...
{
    util::CheckedLockGuard lock(m_drain_mutex);
    TESSERA_ASSERT(m_outstanding_posts);
...
{
    TESSERA_ASSERT(m_socket_provider);
...
void Connection::initiate_session_deactivation(Session* sess)
{
    TESSERA_ASSERT(sess);
    TESSERA_ASSERT(&sess->m_conn == this);
    TESSERA_ASSERT(m_num_active_sessions);
    ...
    if (TESSERA_UNLIKELY(--m_num_active_sessions == 0)) {
...
{
    TESSERA_ASSERT(sess->m_state == Session::Deactivated);
...
void Connection::force_close()
{
    ...
    TESSERA_ASSERT_EX(m_state == ConnectionState::disconnected, m_state);
...
{
    TESSERA_ASSERT(m_activated);
    TESSERA_ASSERT(!m_reconnect_delay_in_progress);
    TESSERA_ASSERT(!m_disconnect_delay_in_progress);
...
{
    if (!status.is_ok()) {
        TESSERA_ASSERT(status != ErrorCodes::OperationAborted);
        throw Exception(status);
    }
...
void Connection::handle_connect_wait(Status status)
{
    ...
    TESSERA_ASSERT_EX(m_state == ConnectionState::connecting, m_state);
...
void Connection::schedule_urgent_ping()
{
    ...
    TESSERA_ASSERT_EX(m_state == ConnectionState::connecting || m_waiting_for_pong, m_state);
    if (!m_send_ping)
        m_minimize_next_ping_delay = true;
}
...
{
    TESSERA_ASSERT(!m_ping_delay_in_progress);
    TESSERA_ASSERT(!m_waiting_for_pong);
    TESSERA_ASSERT(!m_send_ping);
    ...
    if (!m_minimize_next_ping_delay) {
        ...
        TESSERA_ASSERT_3(now, >=, m_pong_wait_started_at);
...
{
    TESSERA_ASSERT_EX(m_state == ConnectionState::connected, m_state);
    TESSERA_ASSERT(!m_sending_session);
    TESSERA_ASSERT(!m_sending);
    if (m_send_ping) {
        send_ping(); // Throws
        return;
    }
...
void Connection::receive_pong(milliseconds_type timestamp)
{
    ...
    if (m_ping_after_scheduled_reset_of_reconnect_info) {
        TESSERA_ASSERT(m_reconnect_info.scheduled_reset);
        m_ping_after_scheduled_reset_of_reconnect_info = false;
...
void Connection::receive_error_message(const ProtocolErrorInfo& info, session_ident_type session_ident)
{
    ...
    if (TESSERA_LIKELY(known_error_code)) {
        ProtocolError error_code = ProtocolError(info.raw_error_code);
        if (TESSERA_LIKELY(!is_session_level_error(error_code))) {
...
{
    TESSERA_ASSERT_EX(m_state == Active, m_state);
                                                 ...
                                                 std::vector<ProtocolErrorInfo>* out)
{
    ...
    TESSERA_ASSERT_DEBUG(
        std::is_sorted(m_pending_compensating_write_errors.begin(), m_pending_compensating_write_errors.end(),
                       [](const ProtocolErrorInfo& lhs, const ProtocolErrorInfo& rhs) {
                           TESSERA_ASSERT_DEBUG(lhs.compensating_write_server_version.has_value());
                           TESSERA_ASSERT_DEBUG(rhs.compensating_write_server_version.has_value());
    ...
    while (!m_pending_compensating_write_errors.empty() &&
        ...
        TESSERA_ASSERT_3(*cur_error.compensating_write_server_version, >=, changesets.front().version);
...
{
    TESSERA_ASSERT_EX(m_state == Active, m_state);
    TESSERA_ASSERT(!m_client_error && !m_error_to_send);
    ...
    TESSERA_ASSERT(m_suspended || m_error_message_received || !m_unbind_message_sent);
...
{
    TESSERA_ASSERT_EX(m_state == Active, m_state);
    TESSERA_ASSERT_3(progress.download.server_version, >=, m_download_progress.server_version);
...
Session::~Session()
{
    //    TESSERA_ASSERT_EX(m_state == Unactivated || m_state == Deactivated, m_state);
}
...
{
    TESSERA_ASSERT_EX(m_state == Unactivated, m_state);
    ...
    TESSERA_ASSERT_3(m_last_version_available, >=, m_progress.upload.client_version);
...
void Session::initiate_deactivation()
{
    ...
    if (m_enlisted_to_send) {
        TESSERA_ASSERT(!unbind_process_complete());
        return;
    }
...
{
    TESSERA_ASSERT_EX(m_state == Deactivating, m_state);
    m_state = Deactivated;
...
{
    TESSERA_ASSERT_EX(m_state == Active || m_state == Deactivating, m_state);
    TESSERA_ASSERT(m_enlisted_to_send);
    ...
    // not been initiated
    TESSERA_ASSERT(!m_unbind_message_sent);
...
void Session::send_bind_message()
{
    ...
    // BIND carries no token. Upstream emptied this field deliberately in
    // realm-core #5151 (af504f10d, January 2022, "Discard token from bind and
    // refresh messages"), moving authentication to the transport: the client
    // sends its credential as ?baas_at= on the WebSocket handshake URL, which
    // get_http_request_path() builds.
    //
    // #23 briefly sent the connection's token here as well, on the belief that
    // nothing sent one anywhere. That was wrong -- see
    // docs/findings/0b-auth-belongs-at-the-handshake.md -- and it put a second
    // copy of the credential on the wire once per session.
    const std::string empty_access_token;
    if (m_is_flx_sync_session) {
...
{
    TESSERA_ASSERT_EX(m_state == Active, m_state);
    TESSERA_ASSERT(m_bind_message_sent);
    TESSERA_ASSERT(!m_unbind_message_sent);
    TESSERA_ASSERT(have_client_file_ident());
...
{
    TESSERA_ASSERT_EX(m_state == Active, m_state);
    TESSERA_ASSERT(m_ident_message_sent);
    TESSERA_ASSERT(!m_unbind_message_sent);
    TESSERA_ASSERT(m_pending_flx_sub_set);
    TESSERA_ASSERT_3(m_pending_flx_sub_set->query_version, >, m_last_sent_flx_query_version);

    if (TESSERA_UNLIKELY(get_client().is_dry_run())) {
        return;
    }
...
void Session::send_upload_message()
{
    ...
    if (m_pending_flx_sub_set) {
        TESSERA_ASSERT(m_is_flx_sync_session);
...
void Session::send_mark_message()
{
    ...
    TESSERA_ASSERT(m_ident_message_sent);
    TESSERA_ASSERT(!m_unbind_message_sent);
    TESSERA_ASSERT_3(m_target_download_mark, >, m_last_download_mark_sent);
...
{
    TESSERA_ASSERT_EX(m_state == Deactivating || m_error_message_received || m_suspended, m_state);
    TESSERA_ASSERT(m_bind_message_sent);
    TESSERA_ASSERT(!m_unbind_message_sent);
...
void Session::send_json_error_message()
{
    ...
    TESSERA_ASSERT(m_ident_message_sent);
    TESSERA_ASSERT(!m_unbind_message_sent);
    TESSERA_ASSERT(m_error_to_send);
    TESSERA_ASSERT(m_client_error);
...
void Session::send_test_command_message()
{
    ...
    TESSERA_ASSERT(it != m_pending_test_commands.end());
...
bool Session::client_reset_if_needed()
{
    ...
    TESSERA_ASSERT_EX(m_progress.download.last_integrated_client_version == 0,
                    m_progress.download.last_integrated_client_version);
    TESSERA_ASSERT_EX(m_progress.upload.client_version == 0, m_progress.upload.client_version);
...
Status Session::receive_ident_message(SaltedFileIdent client_file_ident)
{
    ...
    if (TESSERA_UNLIKELY(client_file_ident.ident < 1)) {
        return {ErrorCodes::SyncProtocolInvariantFailed, "Bad client file identifier in IDENT message"};
    }
    if (TESSERA_UNLIKELY(client_file_ident.salt == 0)) {
...
Status Session::receive_download_message(const DownloadMessage& message)
{
    ...
    if (auto status = check_received_sync_progress(progress); TESSERA_UNLIKELY(!status.is_ok())) {
    ...
    TESSERA_ASSERT_EX(hook_action == SyncClientHookAction::NoAction, hook_action);
...
Status Session::receive_mark_message(request_ident_type request_ident)
{
    ...
    if (TESSERA_UNLIKELY(!good_request_ident)) {
        return {
            ErrorCodes::SyncProtocolInvariantFailed,
            util::format(
...
Status Session::receive_unbound_message()
{
    ...
    TESSERA_ASSERT_EX(m_state == Deactivating || m_suspended, m_state);
...
Status Session::receive_error_message(const ProtocolErrorInfo& info)
{
    ...
    if (status != ErrorCodes::UnknownError && TESSERA_UNLIKELY(!is_session_level_error(protocol_error))) {
        return {ErrorCodes::SyncProtocolInvariantFailed,
    ...
    if (status == ErrorCodes::SyncCompensatingWrite) {
        ...
        if (m_state == Active) {
            TESSERA_ASSERT(info.compensating_write_server_version.has_value());
...
{
    TESSERA_ASSERT(!m_try_again_activation_timer);
...
{
    TESSERA_ASSERT_3(m_target_download_mark, >=, m_last_download_mark_received);
    TESSERA_ASSERT_3(m_last_download_mark_received, >=, m_last_triggering_download_mark);
    ...
    if (TESSERA_UNLIKELY(m_delay_uploads)) {
