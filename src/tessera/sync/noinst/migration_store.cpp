...
bool MigrationStore::load_data(bool read_only)
{
    ...
    TESSERA_ASSERT(m_migration_table);
...
void MigrationStore::complete_migration_or_rollback()
{
    ...
    TESSERA_ASSERT(!migration_table->is_empty());
    auto migration_store_obj = migration_table->get_object(0);
...
std::shared_ptr<tessera::SyncConfig> MigrationStore::convert_sync_config(std::shared_ptr<tessera::SyncConfig> config)
{
    TESSERA_ASSERT(config);
...
std::shared_ptr<tessera::SyncConfig>
MigrationStore::convert_sync_config_to_flx(std::shared_ptr<tessera::SyncConfig> config)
{
    if (config->flx_sync_requested) {
        return config;
    }

    auto flx_config = std::make_shared<tessera::SyncConfig>(*config); // deep copy
...
{
    TESSERA_ASSERT(!rql_query_string.empty());
    ...
    TESSERA_ASSERT(m_state != MigrationState::Migrated);
    ...
    if (migration_table->is_empty()) {
        ...
        TESSERA_ASSERT(m_state == state);
        TESSERA_ASSERT(m_query_string == query_string);
        TESSERA_ASSERT(m_migrated_partition == migrated_partition);
    }
}
...
void MigrationStore::rollback_to_pbs()
{
    ...
    TESSERA_ASSERT(m_state != MigrationState::NotMigrated);
    m_state = MigrationState::RollbackInProgress;
...
{
    TESSERA_ASSERT(!object_class_name.empty());
