...
struct Path {
    ...
    {
        TESSERA_ASSERT(idx < m_path.size());
        return m_path[idx];
    }
...
struct Payload {
    ...
    explicit Payload(tessera::util::None) noexcept
        : type(Type::Null)
    {
    }
...
struct Instruction {
#define TESSERA_DECLARE_INSTRUCTION_STRUCT(X) using X = instr::X;
    TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_DECLARE_INSTRUCTION_STRUCT)
#undef TESSERA_DECLARE_INSTRUCTION_STRUCT
...
#define TESSERA_INSTRUCTION_VARIANT_ALTERNATIVE(X) , X
                       TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_INSTRUCTION_VARIANT_ALTERNATIVE)
#undef TESSERA_INSTRUCTION_VARIANT_ALTERNATIVE
                   >
        m_instr;
...
{
    switch (type) {
#define TESSERA_INSTRUCTION_TYPE_TO_STRING(X)                                                                          \
    case Instruction::Type::X:                                                                                       \
        return #X;
        TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_INSTRUCTION_TYPE_TO_STRING)
#undef TESSERA_INSTRUCTION_TYPE_TO_STRING
    }
    return "(invalid)";
}
...
inline DataType get_data_type(Instruction::Payload::Type type) noexcept
{
    ...
    switch (type) {
        ...
        case Type::ObjectValue:
            [[fallthrough]];
        case Type::GlobalKey:
            TESSERA_TERMINATE(util::format("Invalid data type: %1", int8_t(type)).c_str());
    }
...
#define TESSERA_DEFINE_INSTRUCTION_GET_TYPE(X)                                                                         \
    template <>                                                                                                      \
...
TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_DEFINE_INSTRUCTION_GET_TYPE)
#undef TESSERA_DEFINE_INSTRUCTION_GET_TYPE
...
inline decltype(auto) Instruction::visit(F&& lambda, V&& instr)
{
    ...
    if (mpark::holds_alternative<Vector>(instr)) {
        TESSERA_TERMINATE("visiting instruction vector");
    }
#define TESSERA_VISIT_VARIANT(X)                                                                                       \
    else if (auto ptr = mpark::get_if<Instruction::X>(&instr))                                                       \
    {                                                                                                                \
        return lambda(*ptr);                                                                                         \
    }
    TESSERA_FOR_EACH_INSTRUCTION_TYPE(TESSERA_VISIT_VARIANT)
#undef TESSERA_VISIT_VARIANT
    else
    {
        TESSERA_TERMINATE("Unhandled instruction variant entry");
    }
}
...
template <class T>
TESSERA_NOINLINE T* Instruction::get_if() noexcept
{
...
{
    if (auto vec = mpark::get_if<Vector>(&m_instr)) {
        TESSERA_ASSERT(idx < vec->size());
        return (*vec)[idx];
    }
    TESSERA_ASSERT(idx == 0);
    return *this;
}
...
inline size_t Instruction::path_length() const noexcept
{
    ...
    switch (type()) {
        ...
        default:
            TESSERA_TERMINATE("Unhandled instruction type in Instruction::path_len()");
    }
}
...
#endif // TESSERA_IMPL_INSTRUCTIONS_HPP
