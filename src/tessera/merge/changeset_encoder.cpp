...
void ChangesetEncoder::operator()(const Instruction::AddColumn& instr)
{
    ...
    TESSERA_ASSERT(instr.type != Instruction::Payload::Type::Null || instr.nullable || is_dictionary);
...
InternString ChangesetEncoder::intern_string(StringData str)
{
    ...
    if (it == m_intern_strings_rev.end()) {
        ...
        TESSERA_ASSERT_RELEASE_EX(index <= std::numeric_limits<uint32_t>::max(), index);
        bool inserted;
        std::tie(it, inserted) = m_intern_strings_rev.insert({std::string{str}, uint32_t(index)});
        TESSERA_ASSERT_RELEASE_EX(inserted, str);
...
StringBufferRange ChangesetEncoder::add_string_range(StringData data)
{
    ...
    TESSERA_ASSERT(data.size() <= std::numeric_limits<uint32_t>::max());
...
{
    TESSERA_ASSERT(str.offset + str.size <= m_string_range.size());
...
{
    TESSERA_ASSERT(str != InternString::npos);
