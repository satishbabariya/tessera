#ifndef TESSERA_NOINST_INTEGER_CODEC_HPP
#define TESSERA_NOINST_INTEGER_CODEC_HPP
...
std::size_t decode_int(const char* buffer, std::size_t size, T& value) noexcept
{
    ...
    if (TESSERA_LIKELY(decode_int(input, value))) {
        TESSERA_ASSERT(input.ptr > buffer);
...
#endif // TESSERA_NOINST_INTEGER_CODEC_HPP
