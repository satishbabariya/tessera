...
#ifndef TESSERA_ARRAY_BIG_BLOBS_HPP
#define TESSERA_ARRAY_BIG_BLOBS_HPP
...
{
    TESSERA_ASSERT_DEBUG(!(!m_nullable && value.is_null()));
                                            ...
                                            bool nullable = true) noexcept
{
    ...
    TESSERA_ASSERT_DEBUG(!(!nullable && bin.is_null()));
    if (bin.is_null())
        return tessera::null();
    else
        return StringData(bin.data(), bin.size() - 1); // Do not include terminating zero
...
#endif // TESSERA_ARRAY_BIG_BLOBS_HPP
