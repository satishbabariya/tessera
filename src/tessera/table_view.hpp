...
public:
    ...
    /// within this view is returned, otherwise `tessera::not_found` is returned.
...
{
    TESSERA_ASSERT(m_collection_source);
    m_key_values.create();
    if (m_table) {
        m_last_seen_versions.emplace_back(m_table->get_key(), m_table->get_content_version());
    }
