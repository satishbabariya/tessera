...
#include <tessera/node.hpp>
#include <tessera/query_state.hpp>
#include <tessera/column_fwd.hpp>
...
public:
    size_t m_state = tessera::not_found;
    QueryStateFindFirst()
        : QueryStateBase(1)
    {
    }
...
public:
    ...
    {
        TESSERA_ASSERT_3(m_width, ==, get_width_from_header(get_header()));
        return m_width;
    }
    ...
    {
        TESSERA_ASSERT_3(m_width, ==, get_width_from_header(get_header()));
        TESSERA_ASSERT_3(m_size, ==, get_size_from_header(get_header()));
    ...
    {
        static cond c;
        TESSERA_ASSERT(start <= m_size && (end <= m_size || end == size_t(-1)) && start <= end);
        if (end - start == 1) {
            return c(get(start), value) ? start : tessera::not_found;
        }
...
inline Array::Type Array::get_type() const noexcept
{
    if (m_is_inner_bptree_node) {
        TESSERA_ASSERT_DEBUG(m_has_refs);
...
{
    TESSERA_ASSERT_DEBUG(is_attached());
    TESSERA_ASSERT_DEBUG_EX(ndx < m_size, ndx, m_size);
    return (this->*m_getter)(ndx);

    // Two ideas that are not efficient but may be worth looking into again:
    /*
        // Assume correct width is found early in TESSERA_TEMPEX, which is the case for B tree offsets that
        // are probably either 2^16 long. Turns out to be 25% faster if found immediately, but 50-300% slower
        // if found later
        TESSERA_TEMPEX(return get, (ndx));
    */
    /*
...
{
    TESSERA_ASSERT_DEBUG(is_attached());
    TESSERA_ASSERT_DEBUG_EX(m_has_refs, m_ref, ndx, m_size);
    int64_t v = get(ndx);
    return to_ref(v);
}
...
inline size_t Array::get_byte_size() const noexcept
{
    ...
    TESSERA_ASSERT_7(m_alloc.is_read_only(m_ref), ==, true, ||, num_bytes, <=, get_capacity_from_header(header));
